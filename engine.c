    ...
    for(int y = 0; y < 11; y++) {
        for(int x = 0; x < 6; x++) {
            ...
            if(piece == 2) {
                ...
                if(y <= 1) score -= 50;   // don't sit on starting 2 rows
                else if(y == 2) score -= 20; // smaller penalty for staying in 3rd row

                // Bonus for leaving start zone
                if(y > 0 && y <= 3) score += 10; // bonus for 4th row or greater

                if(late_game && y >= 3 && y <= 7) score -= 30; // mid-board camping late game -- needs to be implemented sooner(?)
            } else {
                int dist = 10 - y;
                score -= (dist * dist * 2);
                if(y <= 1) score -= 30;
                    ...
                    if((x+2 >= 6) || model->board_state[y][x+2] != piece) {
                        ...
                        for(int i = 0; i < 2 && isolated; i++) {
                            ...
                            for(int n = 0; n < 4; n++) {
                                ...
                                if((ny_ == coords[0][0] && nx_ == coords[0][1]) || (ny_ == coords[1][0] && nx_ == coords[1][1])) continue;
