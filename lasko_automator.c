...
static void send_lasko_signal(uint16_t code) {
  ...
  InfraredTxContext tx_ctx = {
      .timings = duration_buffer, .count = index, .index = 0};

  furi_hal_infrared_async_tx_set_data_isr_callback(infrared_tx_callback,
                                                   &tx_ctx);
...
static void render_callback(Canvas *canvas, void *ctx_ptr) {
  ...
  if (ctx->state == StateIdle) {
    ...
    snprintf(buffer, sizeof(buffer), "Off Time: %ld min",
    ...
    if (ctx->state == StateRunningOn) {
      ...
      snprintf(buffer, sizeof(buffer), "Time Left: %02ld:%02ld", rem_min,
               rem_sec);
    } else {
      canvas_draw_str(canvas, 10, 30, "State: FAN OFF");
      snprintf(buffer, sizeof(buffer), "Next ON in: %02ld:%02ld", rem_min,
               rem_sec);
    }
...
int32_t lasko_automator_app(void *p) {
  ...
  while (running) {
    FuriStatus status =
        furi_message_queue_get(event_queue, &event, 250); // 250ms tick
