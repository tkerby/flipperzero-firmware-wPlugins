...
static bool alloc_text_input(FlipRPIApp *app)
{
    ...
    if (!easy_flipper_set_uart_text_input(&app->uart_text_input, FlipRPIViewTextInput, "Enter command", app->uart_text_input_temp_buffer, app->uart_text_input_buffer_size, text_updated, callback_to_submenu, &app->view_dispatcher, app))
    {
        return false;
    }
...
static bool alloc_widget(FlipRPIApp *app)
{
    ...
    return easy_flipper_set_widget(&app->widget, FlipRPIViewAbout, "FlipRPI v1.0\n------ ------ ------\nControl your Raspberry Pi\nwith your Flipper.", callback_to_submenu, &app->view_dispatcher);
}
...
static void update_submenu(FlipRPIApp *app)
{
    ...
    for (uint32_t i = 0; i < MAX_COMMANDS; i++)
    {
        if (commands[i] != NULL)
        {
            submenu_add_item(app->submenu_commands, commands[i], FlipRPISubmenuIndexCommandStart + i, callback_submenu_choices, app);
        }
    }
}
...
static bool alloc_submenu_command(FlipRPIApp *app)
{
    ...
    if (!easy_flipper_set_submenu(&app->submenu_commands, FlipRPIViewSubmenuCommands, "Commands", callback_to_submenu, &app->view_dispatcher))
    {
        return false;
    }
    update_submenu(app);
...
static bool alloc_text_box(FlipRPIApp *app)
{
    ...
    if (!easy_flipper_set_text_box(
            &app->textbox,
            FlipRPIViewMain,
            app->fhttp->last_response_str && furi_string_size(app->fhttp->last_response_str) > 0 ? (char *)furi_string_get_cstr(app->fhttp->last_response_str) : "Awaiting data...",
            false,
            callback_to_submenu,
            &app->view_dispatcher))
    {
    ...
    default:
        ...
        if (index >= FlipRPISubmenuIndexCommandStart)
        {
            ...
            if (command_index != 0)
            {
                ...
                strncpy(app->uart_text_input_temp_buffer, command, app->uart_text_input_buffer_size);
...
static FlipRPIApp *flip_rpi_app_alloc()
{
    ...
    view_dispatcher_set_custom_event_callback(app->view_dispatcher, flip_rpi_custom_event_callback);
    // Submenu
    if (!easy_flipper_set_submenu(&app->submenu, FlipRPIViewSubmenu, VERSION_TAG, callback_exit_app, &app->view_dispatcher))
    {
        return NULL;
    }
