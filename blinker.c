...
static void timer_callback(void *context) {
  ...
  if (!app->time_out) {
    ...
    uint32_t elapsed_time =
        (furi_get_tick() - app->start_time) / 1000; // seconds
    ...
    if (app->last_check == 0 || app->last_check + 5 < current_time ||
      ...
      widget_add_string_element(app->widget, 64, 32, AlignCenter, AlignCenter,
                                FontPrimary, "Blinking");
      widget_add_string_element(app->widget, 64, 42, AlignCenter, AlignCenter,
                                FontSecondary, text);
...
static void number_picker_view(BlinkerApp *app, const char *header,
                               uint32_t current, uint32_t min, uint32_t max) {
  number_input_set_header_text(app->number_input, header);
  number_input_set_result_callback(app->number_input, number_picker_callback,
                                   app, current, min, max);
...
static void main_view(BlinkerApp *app) {
  dialog_ex_set_header(app->dialog, "Blinker", 64, 14, AlignCenter,
                       AlignCenter);

  char text[64];
  snprintf(text, sizeof(text), "Duration: %ld min\nInterval: %ld - %ld bpm",
           app->duration, app->max_interval, app->min_interval);
...
int32_t blinker_main(void *p) {
  ...
  // Add views
  view_dispatcher_add_view(app->view_dispatcher, Main,
                           dialog_ex_get_view(app->dialog));
  view_dispatcher_add_view(app->view_dispatcher, NumberPicker,
                           number_input_get_view(app->number_input));
  view_dispatcher_add_view(app->view_dispatcher, Exec,
