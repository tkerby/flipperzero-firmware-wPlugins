...
#define TAG         "FlipGemini"
#define VERSION     "1.0"
#define VERSION_TAG TAG " " VERSION
#define APP_ID      "flip_gemini"

typedef enum {
    FlipGeminiSubmenuRun = 0,
    FlipGeminiSubmenuAbout = 1,
    FlipGeminiSubmenuSettings = 2,
} FlipGeminiSubmenuIndex;

typedef enum {
    FlipGeminiViewMain = 0,
    FlipGeminiViewSubmenu = 1,
    FlipGeminiViewAbout = 2,
    FlipGeminiViewSettings = 3,
    FlipGeminiViewTextInput = 4,
} FlipGeminiView;

class FlipGeminiApp {
private:
    std::unique_ptr<FlipGeminiAbout> about; // About class instance
    FlipperHTTP* flipperHttp = nullptr; // FlipperHTTP instance for HTTP requests
    std::unique_ptr<FlipGeminiRun> run; // Run class instance
    std::unique_ptr<FlipGeminiSettings> settings; // Settings class instance
...
public:
    ...
    FuriString* httpRequest( // synchronous HTTP request
        ...
        const char* headers =
            "{\"Content-Type\": \"application/json\"}", // Headers to include in the request
    ...
    bool sendHTTPCommand(
        HTTPCommand command); // send a command to the board (e.g., ping, reset, etc.)
    bool sendWiFiCredentials(
        const char* ssid,
        const char* password); // send WiFi credentials to the board
    static void viewPortDraw(
        Canvas* canvas,
        void* context); // draw callback for the ViewPort (used in run instance)
    static void viewPortInput(
        InputEvent* event,
        void* context); // input callback for the ViewPort (used in run instance)
};
