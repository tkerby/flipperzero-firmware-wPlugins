...
FlipperHTTP* flipper_http_alloc() {
    if(memmgr_get_free_heap() < MAX_FILE_SHOW + sizeof(FlipperHTTP)) {
        FURI_LOG_E(HTTP_TAG, "Not enough heap to allocate FlipperHTTP.");
        return NULL;
    }
...
void flipper_http_rx_callback(const char* line, void* context) {
    ...
    if(trimmed_line != NULL && trimmed_line[0] != '\0') {
        // Check if the line is not an HTTP method end tag
