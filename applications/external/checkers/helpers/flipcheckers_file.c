#include "flipcheckers_file.h"
#include <storage/storage.h>
#include <loader/loader.h>

// #define FLIPCHECKERS_APP_BASE_FOLDER APP_BOARDA_PATH("flipcheckers")
#define FLIPCHECKERS_APP_BASE_FOLDER            EXT_PATH("apps_data/flipcheckers")
#define FLIPCHECKERS_APP_BASE_FOLDER_PATH(path) FLIPCHECKERS_APP_BASE_FOLDER "/" path
...
bool flipcheckers_has_file(
    ...
    if(file_type == FlipCheckersFileBoard) {
        path = FLIPCHECKERS_BOARD_PATH;
    } else {
        strcpy(path_buf, FLIPCHECKERS_APP_BASE_FOLDER); // 22
...
bool flipcheckers_save_file(
    ...
    if(file_type == FlipCheckersFileBoard) {
        path = FLIPCHECKERS_BOARD_PATH;
        path_bak = FLIPCHECKERS_BOARD_PATH_BAK;
    } else {
        strcpy(path_buf, FLIPCHECKERS_APP_BASE_FOLDER); // 22
        strcpy(path_buf + strlen(path_buf), "/");
        strcpy(path_buf + strlen(path_buf), file_name);
    ...
    storage_simply_mkdir(fs_api, FLIPCHECKERS_APP_BASE_FOLDER);
