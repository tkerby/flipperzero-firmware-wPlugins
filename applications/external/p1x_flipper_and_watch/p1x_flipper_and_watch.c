...
static void game_init(GameState* state) {
    ...
    for(int i = 0; i < 4; i++) {
        ...
        for(int j = 0; j < MAX_UPCOMING_PACKETS; j++) {
            state->upcoming_packets[i][j] =
                rand() % 2; // Randomly initialize as 0 (normal) or 1 (hacking attempt)
        }
    }
}
...
static void draw_callback(Canvas* canvas, void* ctx) {
    ...
    canvas_draw_str(
        canvas,
        SERVER_POSITION.x - canvas_string_width(canvas, buffer) / 2,
        SERVER_POSITION.y + 12,
        buffer);
    ...
    for(int i = 0; i < 4; i++) {
        ...
        for(int j = 0; j < MAX_UPCOMING_PACKETS; j++) {
            ...
            int packet_x =
                (i % 2 == 0) ?
                    x - 10 - j * 5 :
                    x + 10 + j * 5; // Left for even-indexed systems, right for odd-indexed systems
    ...
    snprintf(score_text, sizeof(score_text), "Score: %d", state->score);
    canvas_draw_str(
        canvas, SCREEN_WIDTH / 2 - canvas_string_width(canvas, score_text) / 2, 10, score_text);
}
...
static void update_game(GameState* state) {
    ...
    for(int i = 0; i < 4; i++) {
        if(state->warn_start[i] && !state->hacking[i] &&
           now - state->warn_start[i] >= HACK_WARN_TIME) {
