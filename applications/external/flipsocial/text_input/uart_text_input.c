...
static void
    text_input_keyboard_callback_line(UART_TextInput* text_input, const RpcKeyboardEvent* event) {
    with_view_model(
        text_input->view,
        UART_TextInputModel * model,
        {
                        ...
                        if((ch >= 0x20 && ch <= 0x7E) || ch == '\n' || ch == '\r') {
                            ...
                            if(ch == '\n' || ch == '\r') {
                                newline = event->data.newline_enabled &&
                                          !substitutions; // TODO: No min-length check?
                                break;
                            }
                        }
                    }
...
static void text_input_keyboard_type_key(UART_TextInput* text_input, char selected) {
    with_view_model(
        ...
        UART_TextInputModel * model,
        {
            ...
            char search_key = isupper(selected) ? tolower(selected) :
                              selected == ' '   ? '_' :
                                                  selected;
            bool found = false;
            ...
            if(selected == ENTER_KEY) {
                ...
                } else if(model->callback != 0) { // TODO: no min-length check
                    model->callback(model->callback_context);
                }
            } else if(selected == BACKSPACE_KEY) {
                uart_text_input_backspace_cb(model);
            ...
            with_view_model(
                ...
                UART_TextInputModel * model,
                {
                    model->keyboard_subscription = furi_pubsub_subscribe(
                        rpc_keyboard_pubsub, text_input_keyboard_callback, text_input);
                },
                false);
        }
