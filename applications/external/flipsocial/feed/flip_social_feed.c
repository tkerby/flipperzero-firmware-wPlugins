...
bool flip_social_get_feed() {
    ...
    snprintf(
        command,
        96,
        "https://www.flipsocial.net/api/feed/50/%s/extended/",
...
bool flip_social_load_feed_post(int post_id) {
    ...
    snprintf(
        file_path,
        sizeof(file_path),
        STORAGE_EXT_PATH_PREFIX "/apps_data/flip_social/feed_post_%d.json",
        post_id);
...
bool flip_social_load_initial_feed() {
    ...
    if(!easy_flipper_set_loading(
           &app_instance->loading,
           FlipSocialViewLoading,
