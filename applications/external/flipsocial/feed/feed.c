...
bool feed_load_post(int post_id) {
    ...
    snprintf(
        file_path,
        sizeof(file_path),
        STORAGE_EXT_PATH_PREFIX "/apps_data/flip_social/feed/feed_post_%d.json",
        post_id);
    ...
    if(username == NULL || message == NULL || flipped == NULL || flips == NULL ||
       date_created == NULL) {
        FURI_LOG_E(TAG, "Failed to parse item fields.");
        if(username) furi_string_free(username);
        if(message) furi_string_free(message);
        if(flipped) furi_string_free(flipped);
        if(flips) furi_string_free(flips);
        if(date_created) furi_string_free(date_created);
        furi_string_free(feed_data);
        return false;
    }
    ...
    snprintf(
        flip_feed_item->date_created, MAX_LINE_LENGTH, "%s", furi_string_get_cstr(date_created));
...
bool feed_load_initial_feed(FlipperHTTP* fhttp, int series_index) {
    ...
    view_dispatcher_add_view(
        app_instance->view_dispatcher, loading_view_id, loading_get_view(loading));
