...
bool messages_parse_json_message_user_choices(FlipperHTTP* fhttp) {
    ...
    for(int i = 0; i < MAX_MESSAGE_USERS; i++) {
        ...
        snprintf(
            flip_social_explore->usernames[i], MAX_USER_LENGTH, "%s", furi_string_get_cstr(user));
...
bool messages_parse_json_messages(FlipperHTTP* fhttp) {
    ...
    for(int i = 0; i < MAX_MESSAGES; i++) {
        ...
        snprintf(
            flip_social_messages->usernames[i],
            MAX_USER_LENGTH,
            "%s",
            furi_string_get_cstr(sender));
        snprintf(
            flip_social_messages->messages[i],
            MAX_MESSAGE_LENGTH,
            "%s",
            furi_string_get_cstr(content));
