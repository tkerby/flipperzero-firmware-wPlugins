...
FlipSocialRun::FlipSocialRun(void* appContext)
    ...
    , currentProfileElement(ProfileElementBio)
    ...
    , registrationStatus(RegistrationNotStarted)
    , shouldReturnToMenu(false)
    , userInfoStatus(UserInfoNotStarted)
...
void FlipSocialRun::drawCommentsView(Canvas* canvas) {
    ...
    case CommentsWaiting:
        ...
        if(!this->httpRequestIsFinished()) {
            ...
            if(!response || !feedSaveKey) {
                ...
                if(feedSaveKey) free(feedSaveKey);
                if(loading) {
                    loading->stop();
                }
                loadingStarted = false;
            ...
            if(strstr(response, "\"comments\":[{") != NULL) {
                ...
                for(int i = 0; i < MAX_COMMENTS; i++) {
                    ...
                    if(commentItem) {
                        ...
                        if(!username || !message || !flipped || !flips_str || !date_created ||
                           !id_str) {
                            if(username) free(username);
                            if(message) free(message);
                            if(flipped) free(flipped);
                            if(flips_str) free(flips_str);
                            if(date_created) free(date_created);
    ...
    case CommentsSending:
        ...
        if(!this->httpRequestIsFinished()) {
            ...
            if(response && app->loadChar("comment_post", response, 64) &&
...
void FlipSocialRun::drawExploreView(Canvas* canvas) {
    ...
    case ExploreWaiting:
        ...
        if(!this->httpRequestIsFinished()) {
            ...
            if(response && app->loadChar("explore", response, 1024) &&
               strstr(response, "users") != NULL) {
    ...
    case ExploreSending:
        ...
        if(!this->httpRequestIsFinished()) {
            ...
            char* response = (char*)malloc(64);
            if(response && app->loadChar("messages_post", response, 64) &&
               strstr(response, "[SUCCESS]") != NULL) {
    ...
    case ExploreAddingFriend:
        ...
        if(!this->httpRequestIsFinished()) {
            ...
            char* response = (char*)malloc(64);
            if(response && app->loadChar("add_friend", response, 64) &&
               strstr(response, "[SUCCESS]") != NULL) {
                canvas_clear(canvas);
...
void FlipSocialRun::drawFeedItem(
    Canvas* canvas,
    char* username,
    char* message,
    char* flipped,
    ...
    snprintf(
        flip_message, sizeof(flip_message), "%u %s", flipCount, flipCount == 1 ? "flip" : "flips");
                ...
                if(feedItem) {
                    ...
                    if(!username || !message || !flipped || !flips_str || !date_created ||
                       !id_str || !comments_str) {
                        ...
                        if(date_created) free(date_created);
                        if(id_str) free(id_str);
                        if(comments_str) free(comments_str);
                        free(feedItem);
                        feedItemIndex++;
                        continue;
                    }
                    ...
                    if(feedItemFlipOverrideActive[i]) {
                        ...
                        if(feedItemFlipOverride[i] != originalFlipped) {
                            // Flip status changed
                            newCount = feedItemFlipOverride[i] ? originalCount + 1 :
                                                                 originalCount - 1;
...
void FlipSocialRun::drawFriendsView(Canvas* canvas) {
    ...
    case FriendWaiting:
        ...
        if(!this->httpRequestIsFinished()) {
            ...
            char* response = (char*)malloc(1024);
            if(response && app->loadChar("friends", response, 1024) &&
               strstr(response, "friends") != NULL) {
    ...
    case FriendRemove:
        ...
        if(!this->httpRequestIsFinished()) {
            ...
            char* response = (char*)malloc(64);
            if(response && app->loadChar("remove_friend", response, 64) &&
               strstr(response, "[SUCCESS]") != NULL) {
                free(response);
...
void FlipSocialRun::drawMessagesView(Canvas* canvas) {
    ...
    case MessagesWaiting:
        ...
        if(!this->httpRequestIsFinished()) {
            ...
            char* response = (char*)malloc(1024);
            if(response && app->loadChar("messages_with_user", response, 1024) &&
               strstr(response, "conversations") != NULL) {
    ...
    case MessagesSuccess: {
        ...
        if(!messagesUserList) {
            FURI_LOG_E(
                TAG, "drawMessageUsersView: Failed to allocate memory for messagesUserList");
        ...
        if(!app || !app->loadChar("messages_with_user", messagesUserList, 1024)) {
            FURI_LOG_E(
                TAG, "drawMessageUsersView: Failed to load messages user list from storage");
        ...
        for(int i = 0; i < MAX_MESSAGES; i++) {
            ...
            if(messagesIndex < (MAX_MESSAGES - 1)) {
                ...
                char* nextMessage =
                    get_json_array_value("conversations", messagesIndex + 1, messagesUserList);
                if(nextMessage) {
...
void FlipSocialRun::drawMessageUsersView(Canvas* canvas) {
    ...
    case MessageUsersWaiting:
        ...
        if(!this->httpRequestIsFinished()) {
            ...
            char* response = (char*)malloc(1024);
            if(response && app->loadChar("messages_user_list", response, 1024) &&
               strstr(response, "users") != NULL) {
...
void FlipSocialRun::drawPostView(Canvas* canvas) {
    ...
    case PostWaiting:
        ...
        if(!this->httpRequestIsFinished()) {
            ...
            if(response && app->loadChar("post", response, 128) &&
               strstr(response, "[SUCCESS]") != NULL) {
    ...
    case PostChoose: {
        ...
        if(!preSavedLocation || !preSavedMessages) {
            FURI_LOG_E(
                TAG,
                "drawPostView: Failed to allocate memory for preSavedLocation or preSavedMessages");
        ...
        snprintf(
            preSavedLocation,
            128,
            "%s/apps_data/flip_social/pre_saved_messages.txt",
...
void FlipSocialRun::drawProfileView(Canvas* canvas) {
    ...
    if(!bio || !friendsCount || !dateCreated) {
        canvas_set_font(canvas, FontSecondary);
        canvas_draw_str_aligned(
            canvas, 64, 32, AlignCenter, AlignCenter, "Incomplete profile data.");
        if(bio) free(bio);
        if(friendsCount) free(friendsCount);
        if(dateCreated) free(dateCreated);
        return;
    }
...
void FlipSocialRun::drawWrappedBio(Canvas* canvas, const char* text, uint8_t x, uint8_t y) {
    ...
    if(textLen > breakPoint) {
        ...
        if(text[breakPoint] == ' ') breakPoint++; // Skip space at beginning of line2
...
void FlipSocialRun::drawBioEditView(Canvas* canvas) {
    ...
    case BioEditWaiting:
        ...
        if(!this->httpRequestIsFinished()) {
            ...
            char response[64];
            if(app->loadChar("update_bio", response, sizeof(response)) &&
               strstr(response, "[SUCCESS]") != NULL) {
        ...
        if(!this->httpRequestIsFinished()) {
            ...
            if(app && app->loadChar("register", response, sizeof(response))) {
                if(strstr(response, "[SUCCESS]") != NULL) // [SUCCESS]: User created
                {
                    ...
                    strstr(response, "Username or password is empty") !=
                    NULL) // [ERROR]: Username or password is empty.
                {
                ...
                } else if(strstr(response, "User already exists") != NULL) // [ERROR]: User already exists.
                {
                    ...
                    strstr(response, "Username is all one letter") !=
                    NULL) // [ERROR]: Username is all one letter.
                {
                    registrationStatus = RegistrationErrorAllOneLetter;
                } else if(
                    strstr(response, "Username cannot be all numbers") !=
                    NULL) // [ERROR]: Username cannot be all numbers.
                {
                    registrationStatus = RegistrationErrorAllNumbers;
                } else if(
                    strstr(response, "Username too short") !=
                    NULL) // [ERROR]: Username too short (5 or more).
                {
                    registrationStatus = RegistrationErrorUsernameTooShort;
                } else if(
                    strstr(response, "Password too short") !=
                    NULL) // [ERROR]: Password too short (5 or more).
                {
                    registrationStatus = RegistrationErrorPasswordTooShort;
                } else if(
                    strstr(response, "Username too long") !=
                    NULL) // [ERROR]: Username too long (32 or less).
                {
                    registrationStatus = RegistrationErrorUsernameTooLong;
                } else if(
                    strstr(response, "Password too long") !=
                    NULL) // [ERROR]: Password too long (32 or less).
                {
...
bool FlipSocialRun::getMessageUser(char* buffer, size_t buffer_size) {
    ...
    FlipSocialApp* app = static_cast<FlipSocialApp*>(appContext);
    if(!app || !messagesUserList ||
       !app->loadChar(
           currentMenuIndex == SocialViewExplore ? "explore" : "messages_user_list",
           messagesUserList,
           1024)) {
...
bool FlipSocialRun::getSelectedPost(char* buffer, size_t buffer_size) {
    ...
    if(!preSavedLocation || !preSavedMessages) {
        FURI_LOG_E(
            TAG,
            "getSelectedPost: Failed to allocate memory for preSavedLocation or preSavedMessages");
        return false;
    }
    snprintf(
        preSavedLocation,
...
void FlipSocialRun::updateFeedItemFlipStatus() {
    ...
    if(!feedItem) {
        FURI_LOG_E(
            TAG, "updateFeedItemFlipStatus: Failed to get feed item at index %d", feedItemIndex);
        free(response);
        free(feedSaveKey);
        return;
    }
    ...
    case SocialViewMenu: {
        ...
        case InputKeyOk:
            ...
            case SocialViewExplore:
                ...
                break;
            case SocialViewProfile:
                if(userInfoStatus == UserInfoNotStarted ||
                   userInfoStatus == UserInfoRequestError) {
    ...
    case SocialViewFriends: {
        if(friendStatus == FriendConfirmRemove) {
            ...
            case InputKeyOk: {
                ...
                if(friendsList) free(friendsList);
                break;
            }
            default:
                break;
...
void FlipSocialRun::userRequest(RequestType requestType) {
    ...
    case RequestTypeFeed: {
        ...
        if(!url || !feedSaveName) {
            ...
            free(password);
            free(payload);
            if(feedSaveName) free(feedSaveName);
            if(url) free(url);
            return;
        }
        snprintf(
        ...
        if(!app->httpRequestAsync(
               feedSaveName, url, GET, "{\"Content-Type\":\"application/json\"}")) {
            feedStatus = FeedRequestError;
        }
        free(url);
        free(feedSaveName);
    ...
    case RequestTypeFlipPost: {
        ...
        if(!feedPostPayload || !authHeader) {
            FURI_LOG_E(
                TAG, "userRequest: Failed to allocate memory for feedPostPayload or authHeader");
            feedStatus = FeedRequestError;
            free(username);
            free(password);
            free(payload);
            if(feedPostPayload) free(feedPostPayload);
        ...
        snprintf(
            ...
            password);
        snprintf(
            feedPostPayload, 256, "{\"username\":\"%s\",\"post_id\":\"%u\"}", username, feedItemID);
        if(!app->httpRequestAsync(
               "flip_post.txt",
               "https://www.jblanked.com/flipper/api/feed/flip/",
               POST,
               authHeader,
               feedPostPayload)) {
            feedStatus = FeedRequestError;
    ...
    case RequestTypeCommentFetch: {
        ...
        snprintf(
            url,
            128,
            "https://www.jblanked.com/flipper/api/feed/comments/%d/%s/%d/",
            MAX_FEED_ITEMS,
            username,
            feedItemID);
    ...
    case RequestTypeCommentFlip: {
        ...
        if(!commentFlipPayload || !authHeader) {
            FURI_LOG_E(
                TAG,
                "userRequest: Failed to allocate memory for commentFlipPayload or authHeader");
            feedStatus = FeedRequestError;
            free(username);
            free(password);
            free(payload);
            if(commentFlipPayload) free(commentFlipPayload);
            if(authHeader) free(authHeader);
            return;
        }
        snprintf(
    ...
    case RequestTypeMessagesWithUser: {
        ...
        if(!url || !authHeader || !messageUser) {
            FURI_LOG_E(
                TAG, "userRequest: Failed to allocate memory for url, authHeader or messageUser");
            messagesStatus = MessagesRequestError;
            free(username);
            free(password);
            free(payload);
            if(url) free(url);
            if(authHeader) free(authHeader);
            if(messageUser) free(messageUser);
            return;
        }
    ...
    case RequestTypeMessageSend: {
        ...
        if(!url || !authHeader || !message || !messageUser) {
            FURI_LOG_E(
                TAG,
                "userRequest: Failed to allocate memory for url, authHeader, message or messageUser");
            messagesStatus = MessagesRequestError;
            free(username);
            free(password);
            free(payload);
    ...
    case RequestTypeExplore: {
        ...
        if(!url || !authHeader || !keyword) {
            FURI_LOG_E(
                TAG, "userRequest: Failed to allocate memory for url, authHeader or keyword");
            exploreStatus = ExploreRequestError;
            free(username);
            free(password);
            free(payload);
        ...
        if(!app->loadChar("explore_keyword", keyword, 64) || strlen(keyword) == 0 ||
           strlen(keyword) > MAX_MESSAGE_LENGTH) {
    ...
    case RequestTypePost: {
        ...
        if(!url || !authHeader || !userMessage || !feedPost) {
            FURI_LOG_E(
                TAG,
                "userRequest: Failed to allocate memory for url, authHeader, userMessage or feedPost");
            postStatus = PostRequestError;
            free(username);
            free(password);
            if(url) free(url);
            if(authHeader) free(authHeader);
            if(userMessage) free(userMessage);
            if(feedPost) free(feedPost);
            return;
        }
        if(!app->loadChar("new_feed_post", userMessage, 128) || strlen(userMessage) == 0 ||
           strlen(userMessage) > MAX_MESSAGE_LENGTH) {
    ...
    case RequestTypeCommentPost: {
        ...
        if(!url || !authHeader || !userComment || !commentPost) {
            FURI_LOG_E(
                TAG,
                "userRequest: Failed to allocate memory for url, authHeader, userComment or commentPost");
            commentsStatus = CommentsRequestError;
            free(username);
            free(password);
            if(url) free(url);
            if(authHeader) free(authHeader);
            if(userComment) free(userComment);
            if(commentPost) free(commentPost);
            return;
        }
        if(!app->loadChar("new_comment", userComment, 128) || strlen(userComment) == 0 ||
           strlen(userComment) > MAX_MESSAGE_LENGTH) {
        ...
        snprintf(
            commentPost,
            256,
            "{\"username\":\"%s\",\"content\":\"%s\",\"post_id\":\"%u\"}",
            username,
            userComment,
            feedItemID);
    ...
    case RequestTypeFriendAdd: // api/user/add-friend/
    {
        ...
        if(!url || !authHeader || !friendUsername || !friendPayload) {
            FURI_LOG_E(
                TAG,
                "userRequest: Failed to allocate memory for url, authHeader, friendUsername or friendPayload");
            free(username);
            free(password);
            if(url) free(url);
            if(authHeader) {
        ...
        if(!app->loadChar("friend_to_add", friendUsername, MAX_USER_LENGTH) ||
           strlen(friendUsername) == 0 || strlen(friendUsername) > MAX_USER_LENGTH) {
    ...
    case RequestTypeFriendRemove: // api/user/remove-friend/
    {
        ...
        if(!url || !authHeader || !friendUsername || !friendRemovePayload) {
            FURI_LOG_E(
                TAG,
                "userRequest: Failed to allocate memory for url, authHeader, friendUsername or friendRemovePayload");
            free(username);
            free(password);
            if(url) free(url);
            if(authHeader) {
        ...
        if(!app->loadChar("friend_to_remove", friendUsername, MAX_USER_LENGTH) ||
           strlen(friendUsername) == 0 || strlen(friendUsername) > MAX_USER_LENGTH) {
            FURI_LOG_E(TAG, "Failed to load friend username");
            free(username);
            free(password);
        ...
        snprintf(
            ...
            friendUsername);
        if(!app->httpRequestAsync(
               "remove_friend.txt", url, POST, authHeader, friendRemovePayload)) {
            friendStatus = FriendRequestError;
        }
        free(url);
        free(authHeader);
    ...
    case RequestTypeBioUpdate: {
        ...
        if(!url || !authHeader || !bio || !payload) {
            FURI_LOG_E(
                TAG, "userRequest: Failed to allocate memory for url, authHeader, bio or payload");
            bioEditStatus = BioEditRequestError;
            free(username);
            free(password);
            if(url) free(url);
        ...
        if(!app->loadChar("new_bio", bio, 128) || strlen(bio) == 0 ||
           strlen(bio) > MAX_BIO_LENGTH) {
