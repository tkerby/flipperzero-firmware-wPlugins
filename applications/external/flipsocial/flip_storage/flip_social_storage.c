...
bool flip_social_save_post(const char* post_id, const char* json_feed_data) {
    ...
    snprintf(
        ...
        STORAGE_EXT_PATH_PREFIX "/apps_data/flip_social/feed");
    storage_common_mkdir(storage, directory_path);
    ...
    snprintf(
        file_path,
        sizeof(file_path),
        STORAGE_EXT_PATH_PREFIX "/apps_data/flip_social/feed/feed_post_%s.json",
        post_id);
    ...
    if(storage_file_write(file, json_feed_data, strlen(json_feed_data)) !=
       strlen(json_feed_data)) {
...
bool save_char(const char* path_name, const char* value) {
    ...
    snprintf(
        file_path,
        sizeof(file_path),
        STORAGE_EXT_PATH_PREFIX "/apps_data/flip_social/data/%s.txt",
        path_name);
