...
void flip_social_free_explore(void) {
    ...
    if(app_instance->submenu_explore) {
        ...
        view_dispatcher_remove_view(
            app_instance->view_dispatcher, FlipSocialViewLoggedInExploreSubmenu);
    }
...
bool flip_social_parse_json_explore() {
    ...
    for(size_t i = 0; i < MAX_EXPLORE_USERS; i++) {
        char* username = get_json_array_value(
            "users", i, data_cstr, 64); // currently its 53 tokens (with max explore users at 50)
        if(username == NULL) {
            break;
        }
