...
bool flip_social_get_explore(void) {
    ...
    char directory[128];
    snprintf(
        directory, sizeof(directory), STORAGE_EXT_PATH_PREFIX "/apps_data/flip_social/explore");

    // Create the directory
    Storage* storage = furi_record_open(RECORD_STORAGE);
    storage_common_mkdir(storage, directory);
    char* keyword = !app_instance->explore_logged_in ||
                            strlen(app_instance->explore_logged_in) == 0 ?
                        "a" :
                        app_instance->explore_logged_in;
    snprintf(
        fhttp.file_path,
...
bool flip_social_get_explore_2(void) {
    ...
    char* keyword = !app_instance->message_users_logged_in ||
                            strlen(app_instance->message_users_logged_in) == 0 ?
                        "a" :
                        app_instance->message_users_logged_in;
    snprintf(
        fhttp.file_path,
        sizeof(fhttp.file_path),
        STORAGE_EXT_PATH_PREFIX "/apps_data/flip_social/explore/%s.json",
...
bool flip_social_parse_json_explore() {
    ...
    for(size_t i = 0; i < MAX_EXPLORE_USERS; i++) {
        FuriString* username = get_json_array_value_furi(
            "users", i, user_data); // currently its 53 tokens (with max explore users at 50)
        if(username == NULL) {
            break;
        }
        snprintf(
