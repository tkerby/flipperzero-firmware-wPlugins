...
static void draw_callback(Canvas* c, void* ctx) {
    ...
    switch(gs->screen) {
    case SCREEN_TITLE:
        draw_title(c, app->anim_ms);
        break;
    case SCREEN_NAME_EDIT:
        draw_name_edit(c, gs, &app->name_edit);
        break;
    case SCREEN_TRAIL:
        draw_trail(c, gs, app->confirm_exit);
        break;
    case SCREEN_EVENT:
        event_draw(c, &app->active_event, gs);
        break;
    case SCREEN_HUNT:
        hunt_draw(c, &app->hunt, gs);
        break;
    case SCREEN_RIVER:
        river_draw(c, &app->river, gs);
        break;
    case SCREEN_RIVER_OUTCOME:
        river_draw_outcome(c, &app->river, gs);
        break;
    case SCREEN_MAP:
        draw_map(c, gs, app->map_page);
        break;
    case SCREEN_REST:
        draw_rest_card(c, gs, app->rest_food_used);
        break;
    case SCREEN_FORT_ARRIVAL:
        draw_fort_arrival(c, gs);
        break;
    case SCREEN_FORT:
        draw_fort_store(c, gs, app->fort_cursor);
        break;
    case SCREEN_DEATH:
        draw_death_card(c, gs, app->dead_player_idx);
        break;
...
static void draw_backdrop_forest(Canvas* c) {
    ...
    draw_pine(c, 58, 18, 9);
    draw_pine(c, 82, 21, 6);
    draw_pine(c, 102, 19, 8);
    draw_pine(c, 120, 23, 4);
}
static void draw_backdrop(Canvas* c, uint32_t miles) {
    ...
    case REGION_PRAIRIE:
    case REGION_PLAINS:
        draw_backdrop_plains(c);
        break;
    case REGION_HIGH_PLAINS:
        draw_backdrop_hills(c);
        break;
    case REGION_MOUNTAINS:
        draw_backdrop_mountains(c);
        break;
    case REGION_DESERT:
        draw_backdrop_desert(c);
        break;
    case REGION_FOREST:
        draw_backdrop_forest(c);
        break;
    default:
        draw_backdrop_plains(c);
        break;
...
static void draw_trail(Canvas* c, const GameState* gs, bool confirm_exit) {
    ...
    canvas_draw_str_aligned(
        c, 64, 1, AlignCenter, AlignTop, LANDMARKS[landmark_idx(t->miles)].name);
    char money_str[8];
    ...
    for(int x = 4; x < 38; x += 4)
        canvas_draw_box(c, x, 40, 2, 1);
    for(int x = 68; x < 124; x += 4)
        canvas_draw_box(c, x, 40, 2, 1);
...
static void draw_fort_arrival(Canvas* c, const GameState* gs) {
    ...
    canvas_set_font(c, FontSecondary);
    canvas_draw_str_aligned(
        c, 64, 1, AlignCenter, AlignTop, LANDMARKS[landmark_idx(gs->trail.miles)].name);
    canvas_set_color(c, ColorBlack);
    ...
    canvas_draw_str_aligned(c, 64, 53, AlignCenter, AlignTop, "Right:Trade  Back:Resume");
}
...
static void draw_fort_store(Canvas* c, const GameState* gs, int cursor) {
    ...
    // Scroll indicators
    canvas_set_font(c, FontSecondary);
    if(scroll_top > 0) canvas_draw_str_aligned(c, 126, 19, AlignRight, AlignTop, "^");
    if(scroll_top + visible < n) canvas_draw_str_aligned(c, 126, 46, AlignRight, AlignTop, "v");
    ...
    canvas_draw_str_aligned(c, 64, 57, AlignCenter, AlignTop, "OK:Buy  Up/Dn:Sel  Back:Go");
}
...
static void draw_map(Canvas* c, const GameState* gs, int page) {
    ...
    if(page == 0) {
        ...
        if(cur < NUM_LANDMARKS - 1) {
            ...
            char nxt[24];
            snprintf(nxt, sizeof(nxt), "%s %lu mi", LANDMARKS[cur + 1].abbr, (unsigned long)dist);
            canvas_draw_str_aligned(c, 126, 37, AlignRight, AlignTop, nxt);
        }

        canvas_draw_line(c, 0, 46, 127, 46);
        canvas_draw_str_aligned(c, 64, 48, AlignCenter, AlignTop, "Up/Dn: Page  Back: Return");
        ...
        canvas_draw_line(c, 0, 50, 127, 50);
        canvas_draw_str_aligned(c, 64, 53, AlignCenter, AlignTop, "Up/Dn: Page  Back: Return");
        ...
        canvas_draw_str_aligned(c, 2, 23, AlignLeft, AlignTop, pace_sel ? "\x15" : " ");
        canvas_draw_str_aligned(c, 10, 23, AlignLeft, AlignTop, pace_names[gs->trail.pace]);
        canvas_set_color(c, ColorBlack);
        canvas_draw_str_aligned(c, 10, 33, AlignLeft, AlignTop, pace_miles[gs->trail.pace]);
        ...
        canvas_draw_str_aligned(c, 66, 23, AlignLeft, AlignTop, ration_sel ? "\x15" : " ");
        canvas_draw_str_aligned(c, 74, 23, AlignLeft, AlignTop, ration_names[gs->trail.rations]);
        canvas_set_color(c, ColorBlack);
        canvas_draw_str_aligned(c, 74, 33, AlignLeft, AlignTop, ration_lbs[gs->trail.rations]);
        ...
        canvas_draw_line(c, 0, 55, 127, 55);
        canvas_draw_str_aligned(c, 64, 57, AlignCenter, AlignTop, "LR:Select  OK:Change  Up:Back");
    }
}
...
static void game_reset(App* app) {
    ...
    strncpy(name0, app->gs.players[0].name, NAME_LEN - 1);
    name0[NAME_LEN - 1] = '\0';
    strncpy(name1, app->gs.players[1].name, NAME_LEN - 1);
    name1[NAME_LEN - 1] = '\0';
...
int32_t oregon_trail_app(void* p) {
    ...
    while(app->gs.running) {
        ...
        while(furi_message_queue_get(app->input_queue, &ev, 0) == FuriStatusOk) {
            ...
            case SCREEN_TITLE:
                if(is_press && ev.key == InputKeyOk) app->gs.screen = SCREEN_NAME_EDIT;
                if(is_press && ev.key == InputKeyBack) app->gs.running = false;
                break;
                ...
                if(ne->editing) {
                    ...
                    if(ev.key == InputKeyUp) {
                        ...
                        if(ne->cursor == 2) {
                            ...
                            strncpy(ne->edit_buf, app->gs.players[ne->cursor].name, NAME_LEN - 1);
                ...
                if(ev.key == InputKeyOk && is_press) {
                    if(app->confirm_exit) {
                        ...
                        if(result == DAY_EVENT) {
                            ...
                            bool good =
                                (app->active_event.def && app->active_event.def->header[0] == 'G');
            ...
            case SCREEN_MAP:
                if(!is_press) break;
                if(app->map_page == 2) {
                    if(ev.key == InputKeyLeft) app->gs.settings_cursor = 0;
                    if(ev.key == InputKeyRight) app->gs.settings_cursor = 1;
                    if(ev.key == InputKeyOk) {
            ...
            case SCREEN_RIVER_OUTCOME:
                if(!is_press) break;
                if(ev.key == InputKeyOk || ev.key == InputKeyBack) app->gs.screen = SCREEN_TRAIL;
                break;

            case SCREEN_EVENT:
                if(!is_press) break;
                if(ev.key == InputKeyUp) {
                    if(app->active_event.choice_cursor > 0) app->active_event.choice_cursor--;
                } else if(ev.key == InputKeyDown) {
                    if(app->active_event.choice_cursor < app->active_event.def->num_choices - 1)
                        app->active_event.choice_cursor++;
                } else if(ev.key == InputKeyOk) {
                    event_apply_choice(
                        &app->gs, &app->active_event, app->active_event.choice_cursor);
                    app->gs.screen = SCREEN_TRAIL;
                } else if(ev.key == InputKeyBack) {
                    app->gs.screen = SCREEN_TRAIL;
                }
            ...
            case SCREEN_HUNT:
                ...
                if(ev.key == InputKeyBack) {
                    ...
                    if(!app->hunt.gored) app->gs.screen = SCREEN_TRAIL;
        ...
        if(app->ok_held && app->gs.screen == SCREEN_TRAIL) {
            ...
            while(app->hold_advance_ms >= 800) {
                ...
                // Fort check during held advance
                {
                    ...
                    bool intercepted = false;
                    for(int f = 0; f < 4; f++) {
                        if(li == FORT_IDX[f] && app->gs.trail.last_fort_visited != li) {
                            app->gs.trail.last_fort_visited = li;
                            app->fort_cursor = 0;
                            app->gs.screen = SCREEN_FORT_ARRIVAL;
                            app->ok_held = false;
