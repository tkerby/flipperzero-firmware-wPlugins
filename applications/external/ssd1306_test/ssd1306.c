...
static bool i2c_send(SSD1306* d, const uint8_t* data, size_t len) {
    ...
    bool ok = furi_hal_i2c_tx(&furi_hal_i2c_handle_external, d->i2c_addr, data, len, 50);
...
bool ssd1306_detect(uint8_t addr) {
    furi_hal_i2c_acquire(&furi_hal_i2c_handle_external);
    bool found = furi_hal_i2c_is_device_ready(&furi_hal_i2c_handle_external, addr, 10);
    furi_hal_i2c_release(&furi_hal_i2c_handle_external);
    return found;
}
...
void ssd1306_char(SSD1306* d, int16_t x, int16_t y, char c) {
    ...
    for(int8_t col = 0; col < 5; col++) {
        ...
        for(int8_t row = 0; row < 7; row++) {
            if(bits & (1 << row)) ssd1306_pixel(d, x + col, y + row, true);
        }
    }
}
...
void ssd1306_big_digit(SSD1306* d, int16_t x, int16_t y, uint8_t digit) {
    ...
    for(int s = 0; s < 7; s++) {
        if(segs & (1 << s)) {
            ssd1306_fill_rect(
                d, x + seg_pos[s][0], y + seg_pos[s][1], seg_pos[s][2], seg_pos[s][3]);
        }
    }
}
