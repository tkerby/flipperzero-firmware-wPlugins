...
static int
    run(const char* name, const WmbusDriver* drv, const char* hex, const char* must_contain) {
    uint8_t fr[300];
    ...
    size_t n =
        drv->decode_ex ?
            ...
            drv->decode(
                link.M, link.version, link.medium, link.apdu, link.apdu_len, out, sizeof(out));
