...
const InputKey input_keys[InputKeyMAX] =
...
static void put_input_event(InputConverter* input_converter, InputEvent* event) {
    FURI_LOG_D(
        GAME_NAME,
        "Input event: %s %s",
    ...
    FuriStatus status = furi_message_queue_put(input_converter->queue, event, 0);
    furi_check(status == FuriStatusOk);
}

static void process_pressed(
...
static void process_holded(
    InputConverter* input_converter,
    InputState* input_state,
    GameKey game_key,
    InputKey input_key) {
    size_t* holded_frames_for_key = &input_converter->holded_frames_for_key[input_key];
    ...
    if(*holded_frames_for_key == LONG_PRESS_IN_FRAMES) {
        ...
        put_input_event(input_converter, &event);
    } else if(*holded_frames_for_key == LONG_PRESS_IN_FRAMES + CHECK_PERIOD_IN_FRAMES) {
        // Pause between events
        input_converter->holded_frames_for_key[input_key] = LONG_PRESS_IN_FRAMES;
...
InputConverter* input_converter_alloc(void) {
    InputConverter* input_converter = malloc(sizeof(InputConverter));

    // Init queue
    input_converter->queue = furi_message_queue_alloc(2 * InputKeyMAX, sizeof(InputEvent));
...
void input_converter_free(InputConverter* input_converter) {
    furi_check(input_converter);
    furi_message_queue_free(input_converter->queue);
    free(input_converter);
}

void input_converter_reset(InputConverter* input_converter) {
...
void input_converter_process_state(InputConverter* input_converter, InputState* input_state) {
    furi_check(input_converter);
...
FuriStatus input_converter_get_event(InputConverter* input_converter, InputEvent* event) {
    furi_check(input_converter);
