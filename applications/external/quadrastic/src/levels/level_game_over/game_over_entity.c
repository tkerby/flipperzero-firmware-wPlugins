...
static void game_over_update(Entity* self, GameManager* manager, void* _entity_context) {
    UNUSED(self);
...
static void
    game_over_render(Entity* self, GameManager* manager, Canvas* canvas, void* _entity_context) {
    UNUSED(self);
    UNUSED(manager);
    ...
    snprintf(best_score_str, sizeof(best_score_str), "BEST: %ld", entity_context->max_score);
