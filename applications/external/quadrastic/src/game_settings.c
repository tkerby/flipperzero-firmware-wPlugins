...
static bool
    game_read_bool_setting(FlipperFormat* flipper_format, const char* key, State* setting) {
...
void game_read_settings(GameContext* game_context) {
    // Default settings
    ...
    do {
        // Check settings file
        if(!flipper_format_file_open_existing(flipper_format, APP_DATA_PATH(SETTINGS_FILE_NAME))) {
            FURI_LOG_D(GAME_NAME, "Settings file is not used");
            break;
        }
        if(!flipper_format_read_header(flipper_format, temp_string, &temp_uint32)) {
        ...
        if(!flipper_format_read_uint32(flipper_format, KEY_DIFFICULTY, &temp_uint32, 1)) {
        ...
        if(!flipper_format_read_uint32(flipper_format, KEY_BEST_SCORE, &temp_uint32, 1)) {
        ...
        // Read sound
        if(!game_read_bool_setting(flipper_format, KEY_SOUND, &game_context->sound)) {
            break;
        }

        // Read vibro
        if(!game_read_bool_setting(flipper_format, KEY_VIBRO, &game_context->vibro)) {
            break;
        }

        // Read led
        if(!game_read_bool_setting(flipper_format, KEY_LED, &game_context->led)) {
            break;
        }
...
void game_save_settings(GameContext* game_context) {
    Storage* storage = furi_record_open(RECORD_STORAGE);
    FlipperFormat* flipper_format = flipper_format_file_alloc(storage);
    ...
    do {
        // Write settings meta
        if(!flipper_format_file_open_always(flipper_format, APP_DATA_PATH(SETTINGS_FILE_NAME))) {
        ...
        if(!flipper_format_write_uint32(flipper_format, KEY_DIFFICULTY, &temp_uint32, 1)) {
        ...
        // Write best score
        temp_uint32 = game_context->best_score;
        if(!flipper_format_write_uint32(flipper_format, KEY_BEST_SCORE, &temp_uint32, 1)) {
        ...
        if(!flipper_format_write_bool(flipper_format, "Sound", &temp_bool, 1)) {
        ...
        if(!flipper_format_write_bool(flipper_format, "Vibro", &temp_bool, 1)) {
