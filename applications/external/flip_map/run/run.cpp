...
FlipMapRun::FlipMapRun(void* appContext)
    : appContext(appContext)
    , currentView(FlipMapRunViewLogin)
    , inCountryView(true)
    , inputHeld(false)
    , lastInput(InputKeyMAX)
    , locationStatus(LocationNotStarted)
    , loginStatus(LoginNotStarted)
    , mapDataStatus(MapDataNotStarted)
    , menuIndex(0)
    , menuMaxIndex(0)
    , menuStartIndex(0)
...
void FlipMapRun::drawMapDataMenu(Canvas* canvas) {
    ...
    if(currentItemCount == 0) {
        ...
        char no_items[32];
        snprintf(
            no_items, sizeof(no_items), "No %s found", inCountryView ? "countries" : "cities");
        ...
        for(uint8_t i = menuStartIndex; i < currentItemCount && displayCount < maxDisplayItems;
            i++) {
            char* item = get_json_array_value(arrayKey, i, response);
            if(!item) {
                break;
...
void FlipMapRun::userRequest(RequestType requestType) {
    ...
    case RequestTypeLocationUpdate: {
        ...
        if(!authHeaders && !url && !locationStatusStr) {
            if(authHeaders) free(authHeaders);
            if(url) free(url);
            if(locationStatusStr) free(locationStatusStr);
        ...
        snprintf(
            url,
            256,
            "https://www.jblanked.com/flipper/api/user/location/update/%s/%s/",
            username,
            locationStatusStr);
        snprintf(
            authHeaders,
