...
static void renfe_regular_parse_history_entry(
...
static bool renfe_regular_is_history_entry(const uint8_t* block_data) {
    ...
    if(block_data[0] == 0x3D && block_data[1] == 0x80 && block_data[2] == 0xA6 &&
       block_data[3] == 0xEF) {
        return true;
    }

    // Pattern 2: 3C 80 DE EF (Exit pattern)
    if(block_data[0] == 0x3C && block_data[1] == 0x80 && block_data[2] == 0xDE &&
       block_data[3] == 0xEF) {
        return true;
    }

    // Pattern 3: 3D 80 DE EF (Transfer pattern)
    if(block_data[0] == 0x3D && block_data[1] == 0x80 && block_data[2] == 0xDE &&
       block_data[3] == 0xEF) {
        return true;
    }

    // Pattern 4: 3C 80 A6 EF (Alternative pattern)
    if(block_data[0] == 0x3C && block_data[1] == 0x80 && block_data[2] == 0xA6 &&
       block_data[3] == 0xEF) {
        return true;
    }
    ...
    if((block_data[0] == 0x3D || block_data[0] == 0x3C) && (block_data[1] == 0x80) &&
...
static uint32_t renfe_regular_extract_timestamp(const uint8_t* block_data) {
    ...
    uint32_t timestamp = ((uint32_t)block_data[4] << 16) | ((uint32_t)block_data[5] << 8) |
...
static void
    renfe_regular_parse_travel_history(FuriString* parsed_data, const MfClassicData* data) {
    if(!parsed_data || !data) {
        return;
    }
        ...
        if(mfc_data) {
            ...
            if(!is_ida_vuelta) {
                ...
                widget_add_button_element(
                    widget, GuiButtonTypeLeft, "History", renfe_regular_widget_callback, app);
            }
                ...
                if(mfc_data) {
                    ...
                    if(widget) {
                        ...
                        widget_add_text_scroll_element(
                            widget, 0, 0, 128, 52, furi_string_get_cstr(parsed_data));
                        widget_add_button_element(
                            widget, GuiButtonTypeRight, "Back", renfe_regular_widget_callback, app);
                        view_dispatcher_switch_to_view(app->view_dispatcher, MetroflipViewWidget);
                    }
