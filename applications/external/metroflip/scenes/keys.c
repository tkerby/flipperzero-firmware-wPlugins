    ...
    do {
        if(!data_loaded) {
            ...
            if(error != MfClassicErrorNone) {
                ...
                if(error != MfClassicErrorNone) {
                    FURI_LOG_I(
                        TAG, "renfe_suma10_verify: RENFE sector 1 key also failed: %d", error);
            ...
            if(mf_classic_is_block_read(mfc_data, 12)) {
                ...
                FURI_LOG_I(
                    TAG,
                    "RENFE Suma 10 - Block 12 pattern: %02X %02X %02X %02X",
                    block12[0],
                    block12[1],
                    block12[2],
                    block12[3]);
                ...
                if(block12[0] == 0x81 && block12[1] == 0xF8 && block12[2] == 0x01 &&
                   block12[3] == 0x02) {
            ...
            if(has_mobilis_pattern) {
                FURI_LOG_I(
                    TAG, "RENFE Suma 10 - VERIFIED! Found Mobilis pattern (subtype of SUMA 10)");
                verified = true;
                ...
                const size_t num_suma_keys =
                    sizeof(renfe_suma10_1k_keys) / sizeof(renfe_suma10_1k_keys[0]);
                bool has_suma_key = false;

                for(uint8_t sector = 0; sector < 4 && !has_suma_key; sector++) {
                    MfClassicSectorTrailer* sec_tr =
                        mf_classic_get_sector_trailer_by_sector(mfc_data, sector);
                    if(sec_tr) {
                        uint64_t key = bit_lib_bytes_to_num_be(sec_tr->key_a.data, 6);
                        ...
                        for(size_t i = 0; i < num_suma_keys; i++) {
                            if(key == renfe_suma10_1k_keys[i].a) {
                                FURI_LOG_I(
                                    TAG, "RENFE Suma 10 - Found SUMA key in sector %d", sector);
                                has_suma_key = true;
                                verified = true;
                                break;
                            }
                ...
                if(!verified) {
                    FURI_LOG_I(
                        TAG,
                        "RENFE Suma 10 - NOT VERIFIED: missing RENFE/Mobilis patterns (sector0_key=%s, sector1_key=%s, block5_pattern=%s, mobilis_pattern=%s)",
...
static bool renfe_regular_verify(Nfc* nfc, MfClassicData* mfc_data, bool data_loaded) {
    FURI_LOG_I(
        TAG, "renfe_regular_verify: ENTRY - data_loaded=%s", data_loaded ? "true" : "false");
    bool verified = false;
    ...
    do {
        if(!data_loaded) {
            ...
            FURI_LOG_I(
                TAG, "renfe_regular_verify: LIVE MODE - trying RENFE Regular authentication...");
            ...
            if(error != MfClassicErrorNone) {
                FURI_LOG_I(
                    TAG, "renfe_regular_verify: RENFE Regular sector 0 key failed: %d", error);
                ...
                if(error != MfClassicErrorNone) {
                    FURI_LOG_I(
                        TAG,
                        "renfe_regular_verify: RENFE Regular sector 1 key also failed: %d",
                        error);
                    ...
                    if(error == MfClassicErrorNone) {
                        FURI_LOG_I(
                            TAG,
                            "renfe_regular_verify: Found default keys - but not specific enough for RENFE Regular");
                        ...
                        if(error == MfClassicErrorNone) {
                            FURI_LOG_I(
                                TAG,
                                "renfe_regular_verify: Found Mifare Classic with common keys - could be RENFE Regular");
                            verified = true;
                        } else {
                            FURI_LOG_I(
                                TAG,
                                "renfe_regular_verify: No common key access - checking if this could be protected RENFE Regular");
                            ...
                            for(int test_sector = 0; test_sector < 4; test_sector++) {
                                ...
                                if(test_error == MfClassicErrorAuth ||
                                   test_error == MfClassicErrorTimeout) {
                                    auth_failures++;
                                }
                            }
                            ...
                            if(auth_failures >= 3) {
                                FURI_LOG_I(
                                    TAG,
                                    "renfe_regular_verify: Multiple sectors protected - likely RENFE Regular with unique keys");
                                verified = true;
                            } else {
                                FURI_LOG_I(
                                    TAG,
                                    "renfe_regular_verify: Card behavior doesn't match RENFE Regular");
                            }
                        }
                    }
                } else {
                    FURI_LOG_I(
                        TAG,
                        "renfe_regular_verify: RENFE Regular sector 1 key authentication SUCCESS!");
                    verified = true;
                }
            } else {
                FURI_LOG_I(
                    TAG,
                    "renfe_regular_verify: RENFE Regular sector 0 key authentication SUCCESS!");
                verified = true;
            }
            ...
            if(mf_classic_is_block_read(mfc_data, 2)) {
                ...
                if(block2[2] == 0x5C && block2[3] == 0x9F) {
                    FURI_LOG_I(
                        TAG, "RENFE Regular - found Block 2 RENFE network signature (5C9F)");
                ...
                if(mf_classic_is_block_read(mfc_data, block)) {
                    ...
                    if((block_data[6] == 0x4D && block_data[7] == 0xDF) || // Common RENFE pattern
                       (block_data[0] > 0x50 && block_data[1] < 0x10)) { // Trip data pattern
                        FURI_LOG_I(
                            TAG, "RENFE Regular - found trip data pattern in block %d", block);
                        pattern_score += 1;
                    }
                }
            }
            ...
            for(int sector = 0; sector < 16; sector++) {
                ...
                if(sec_tr) {
                    ...
                    if(key != 0xFFFFFFFFFFFF && key != 0xA0A1A2A3A4A5 && key != 0x000000000000 &&
                       key != 0xB0B1B2B3B4B5) {
                        protected_sectors++;
                    }
                }
            ...
            if(pattern_score >= 3) {
                FURI_LOG_I(
                    TAG,
                    "RENFE Regular - VERIFIED! Pattern score: %d (high confidence)",
                    pattern_score);
                verified = true;
            } else if(pattern_score >= 1) {
                FURI_LOG_I(
                    TAG,
                    "RENFE Regular - PROBABLY VERIFIED! Pattern score: %d (medium confidence)",
                    pattern_score);
                verified = true;
            } else {
                FURI_LOG_I(
                    TAG,
                    "RENFE Regular - NOT VERIFIED: Pattern score: %d (too low)",
                    pattern_score);
                verified = false;
            }
        }
...
CardType determine_card_type(Nfc* nfc, MfClassicData* mfc_data, bool data_loaded) {
    ...
    if(bip_verify(nfc, mfc_data, data_loaded)) {
        ...
        if(!data_loaded && nfc) {
            FURI_LOG_I(
                TAG,
                "No known cards detected - checking if this could be a protected RENFE Regular");
            ...
            MfClassicAuthContext auth_context;
            MfClassicError error =
                mf_classic_poller_sync_auth(nfc, 0, &key, MfClassicKeyTypeA, &auth_context);
            ...
            if(error == MfClassicErrorTimeout || error == MfClassicErrorAuth) {
                ...
                if(failed_auths >= 3) {
                    FURI_LOG_I(
                        TAG, "Multiple sectors protected - likely RENFE Regular with unique keys");
                    return CARD_TYPE_RENFE_REGULAR;
                }
            }
        }
