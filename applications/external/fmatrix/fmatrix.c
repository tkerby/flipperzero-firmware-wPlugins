...
static void render_options_menu(Canvas* canvas, MatrixApp* app) {
    ...
    for(int i = 0; i < MENU_VISIBLE_ITEMS && (i + visible_start) < total_items; i++) {
        ...
        case 2:
            snprintf(
                value_str, sizeof(value_str), "Numbers: [%c]", settings->use_numbers ? 'X' : ' ');
            break;
        case 3:
            snprintf(
    ...
    case AppStateMatrix: {
        ...
        for(int i = 0; i < ctx->num_columns; i++) {
            ...
            if(ctx->columns[i].y - (ctx->columns[i].length * CHAR_HEIGHT) > SCREEN_HEIGHT) {
                init_matrix_column(
                    &ctx->columns[i], ctx->columns[i].x / CHAR__WIDTH, &ctx->settings);
            }
        }
    } break;
    }
...
static void handle_options_menu_input(MatrixApp* app, InputKey key) {
    ...
    case InputKeyOk:
        switch(app->selected_menu_item) {
        case 0:
            settings->use_uppercase = !settings->use_uppercase;
            break;
        case 1:
            settings->use_lowercase = !settings->use_lowercase;
            break;
        case 2:
            settings->use_numbers = !settings->use_numbers;
            break;
        case 3:
...
static void handle_main_menu_input(MatrixApp* app, InputKey key) {
    ...
    case InputKeyBack:
        ...
        break;
    default:
        break; // don't go over every single key, just put this here for your unused buttons
    }
}
...
static void matrix_input_callback(InputEvent* input_event, void* context) {
    ...
    if(input_event->type != InputTypePress && input_event->type != InputTypeRepeat) return;
