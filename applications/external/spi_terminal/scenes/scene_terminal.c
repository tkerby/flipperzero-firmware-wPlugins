...
void flipper_spi_terminal_scene_terminal_alloc(FlipperSPITerminalApp *app) {
  ...
  furi_string_reserve(app->terminalScreen.output_string_buffer,
...
static void flipper_spi_terminal_scene_terminal_dma_rx_isr(void *context) {
  ...
  if (startOfData != NULL) {
    furi_stream_buffer_send(app->terminalScreen.rx_buffer_stream, startOfData,
                            halfDataSize, 0);
  }
...
bool flipper_spi_terminal_scene_terminal_on_event(void *context,
  ...
  if (event.type == SceneManagerEventTypeTick) {
    ...
    for (size_t i = 0; i < 3; i++) {
      ...
      furi_string_cat_printf(app->terminalScreen.output_string_buffer, "%s",
                             buffer);
    }
