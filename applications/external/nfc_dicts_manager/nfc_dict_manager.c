...
static NfcDictManager* nfc_dict_manager_alloc() {
    ...
    view_dispatcher_set_navigation_event_callback(app->view_dispatcher, nfc_dict_manager_back_event_callback);
    
    app->scene_manager = scene_manager_alloc(&nfc_dict_manager_scene_handlers, app);
    
    app->submenu = submenu_alloc();
    view_dispatcher_add_view(app->view_dispatcher, NfcDictManagerViewSubmenu, submenu_get_view(app->submenu));
    
    app->file_browser_result = furi_string_alloc();
    app->file_browser = file_browser_alloc(app->file_browser_result);
    view_dispatcher_add_view(app->view_dispatcher, NfcDictManagerViewFileBrowser, file_browser_get_view(app->file_browser));
    
    app->text_box = text_box_alloc();
    view_dispatcher_add_view(app->view_dispatcher, NfcDictManagerViewTextBox, text_box_get_view(app->text_box));
    
    app->dialog_ex = dialog_ex_alloc();
    view_dispatcher_add_view(app->view_dispatcher, NfcDictManagerViewSelectType, dialog_ex_get_view(app->dialog_ex));
    
    app->popup = popup_alloc();
    view_dispatcher_add_view(app->view_dispatcher, NfcDictManagerViewPopup, popup_get_view(app->popup));
    ...
    app->backup_timer = furi_timer_alloc(nfc_dict_manager_backup_timer_callback, FuriTimerTypeOnce, app);
    app->success_timer = furi_timer_alloc(nfc_dict_manager_success_timer_callback, FuriTimerTypeOnce, app);
    app->merge_timer = furi_timer_alloc(nfc_dict_manager_merge_timer_callback, FuriTimerTypeOnce, app);
    app->optimize_timer = furi_timer_alloc(nfc_dict_manager_optimize_timer_callback, FuriTimerTypeOnce, app);
