...
static void CANTransceiver_SceneSetup_ChangeCallback_BaudRate(VariableItem* pVarItem)
{
	...
	view_dispatcher_send_custom_event(pInstance->pViewDispatcher, CANTransceiver_SceneSetup_BaudRate);
}
static void CANTransceiver_SceneSetup_ChangeCallback_XtalFreq(VariableItem* pVarItem)
{
	...
	view_dispatcher_send_custom_event(pInstance->pViewDispatcher, CANTransceiver_SceneSetup_XtalFreq);
}
#if 0
...
static void CANTransceiver_SceneSetup_ChangeCallback_ListenOnlyMode(VariableItem* pVarItem)
{
	...
	view_dispatcher_send_custom_event(pInstance->pViewDispatcher, CANTransceiver_SceneSetup_ListenOnlyMode);
}
...
static void CANTransceiver_SceneSetup_ChangeCallback_FrameIDLen(VariableItem* pVarItem)
{
	...
	view_dispatcher_send_custom_event(pInstance->pViewDispatcher, CANTransceiver_SceneSetup_FrameIDLen);
}
static void CANTransceiver_SceneSetup_ChangeCallback_NumDataBytes(VariableItem* pVarItem)
{
	...
	view_dispatcher_send_custom_event(pInstance->pViewDispatcher, CANTransceiver_SceneSetup_NumDataBytes);
}
static void CANTransceiver_SceneSetup_ChangeCallback_TxOneShot(VariableItem* pVarItem)
{
	...
	view_dispatcher_send_custom_event(pInstance->pViewDispatcher, CANTransceiver_SceneSetup_TxOneShot);
}
...
void CANTransceiver_SceneSetup_OnEnter(void* pContext)
{
	...
	variable_item_set_current_value_text(pCurItem, szpBaudRateTexts[pInstance->pGlobalConfig->u8BaudRate]);
	...
	variable_item_set_current_value_text(pCurItem, szpOscillatorFrequencyTexts[pInstance->pGlobalConfig->u8XtalFreq]);
	...
	variable_item_set_current_value_text(pCurItem, szpOffOrOnTexts[pInstance->pGlobalConfig->bListenOnly]);
	...
	variable_item_set_current_value_text(pCurItem, szpFrameIDBitsTexts[pInstance->pGlobalConfig->u8FrameIDLen]);
	...
	variable_item_set_current_value_text(pCurItem, szpNumDataBytesTexts[pInstance->pGlobalConfig->u8NumDataBytes]);
	...
	variable_item_set_current_value_text(pCurItem, szpOffOrOnTexts[pInstance->pGlobalConfig->bTxOneShot]);

	variable_item_list_set_selected_item(pInstance->pVarItemList, CANTransceiver_SceneSetup_BaudRate);
