...
CANTransceiverGlobals* CANTransceiver_Constructor(void) {
    ...
    view_dispatcher_set_navigation_event_callback(
        pInstance->pViewDispatcher, CANTransceiver_BackEventCallback);
    view_dispatcher_attach_to_gui(
        pInstance->pViewDispatcher, pInstance->pGUI, ViewDispatcherTypeFullscreen);

    // Add views to the view dispatcher.
    view_dispatcher_add_view(
        pInstance->pViewDispatcher,
        CANTransceiverViewSubmenu,
        submenu_get_view(pInstance->pSubmenu));
    view_dispatcher_add_view(
        pInstance->pViewDispatcher, CANTransceiverViewWidget, widget_get_view(pInstance->pWidget));
    view_dispatcher_add_view(
        pInstance->pViewDispatcher,
    ...
    view_dispatcher_add_view(
        ...
        byte_input_get_view(pInstance->pByteInput));
    view_dispatcher_add_view(
        pInstance->pViewDispatcher,
        CANTransceiverViewTextInput,
        text_input_get_view(pInstance->pTextInput));
    // TODO as needed
