...
void UFZ::View::free() noexcept {
    if(view != nullptr && bAllocated) view_free(view);
    view = nullptr;
    bAllocated = false;
}
...
const UFZ::View&
    UFZ::View::setPreviousCallback(const ViewNavigationCallback callback) const noexcept {
...
const UFZ::View&
    UFZ::View::allocateModel(const ViewModelType type, const size_t size) const noexcept {
...
const UFZ::Menu& UFZ::Menu::addItem(
    const char* label,
    const Icon* icon,
...
ButtonMenuItem* UFZ::ButtonMenu::addItem(
    const char* label,
    const int32_t index,
    const ButtonMenuItemCallback callback,
    const ButtonMenuItemType type,
    void* context) const noexcept {
...
const UFZ::ButtonPanel& UFZ::ButtonPanel::addItem(
    const uint32_t index,
    ...
    const ButtonItemCallback callback,
    void* context) const noexcept {
...
const UFZ::ButtonPanel& UFZ::ButtonPanel::addLabel(
    const uint16_t x,
    const uint16_t y,
    const Font font,
...
const UFZ::ButtonPanel&
    UFZ::ButtonPanel::addIcon(const uint16_t x, const uint16_t y, const Icon* icon) const noexcept {
...
void UFZ::ByteInput::setResultCallback(
    const ByteInputCallback inputCallback,
    const ByteChangedCallback changedCallback,
    void* context,
    uint8_t* bytes,
    const uint8_t bytesCount) const noexcept {
    byte_input_set_result_callback(
        byte_input, inputCallback, changedCallback, context, bytes, bytesCount);
}
...
void UFZ::NumberInput::setResultCallback(
    const NumberInputCallback inputCallback,
    void* context,
    const int32_t currentNumber,
    const int32_t min,
...
const UFZ::DialogEx&
    UFZ::DialogEx::setResultCallback(const DialogExResultCallback callback) const noexcept {
...
const UFZ::DialogEx& UFZ::DialogEx::setHeader(
    const char* text,
    const uint8_t x,
    const uint8_t y,
    const Align horizontal,
    const Align vertical) const noexcept {
    dialog_ex_set_header(dialog_ex, text, x, y, horizontal, vertical);
    return *this;
}

const UFZ::DialogEx& UFZ::DialogEx::setText(
    const char* text,
    const uint8_t x,
    const uint8_t y,
    const Align horizontal,
...
const UFZ::DialogEx&
    UFZ::DialogEx::setIcon(const uint8_t x, const uint8_t y, const Icon* icon) const noexcept {
...
const UFZ::Popup& UFZ::Popup::setHeader(
    const char* text,
    const uint8_t x,
    const uint8_t y,
    const Align horizontal,
...
const UFZ::Popup& UFZ::Popup::setText(
    const char* text,
    const uint8_t x,
    const uint8_t y,
    const Align horizontal,
...
const UFZ::Popup&
    UFZ::Popup::setIcon(const uint8_t x, const uint8_t y, const Icon* icon) const noexcept {
...
const UFZ::Submenu& UFZ::Submenu::addItem(
    const char* label,
    const uint32_t index,
    const SubmenuItemCallback callback,
    void* context) const noexcept {
...
void UFZ::TextInput::setResultCallback(
    const TextInputCallback callback,
    void* callbackContext,
    char* textBuffer,
    const size_t textBufferSize,
    const bool clearDefaultText) const noexcept {
    text_input_set_result_callback(
        text_input, callback, callbackContext, textBuffer, textBufferSize, clearDefaultText);
}

void UFZ::TextInput::setValidator(const TextInputValidatorCallback callback, void* context)
    const noexcept {
...
VariableItem* UFZ::VariableItemList::add(
    const char* label,
    const uint8_t values_count,
    const VariableItemChangeCallback callback,
    void* context) const noexcept {
    return variable_item_list_add(variable_item_list, label, values_count, callback, context);
}

void UFZ::VariableItemList::setEnterCallback(
    const VariableItemListEnterCallback callback,
    void* context) const noexcept {
...
const UFZ::Widget& UFZ::Widget::addStringMultilineElement(
    const uint8_t x,
    const uint8_t y,
    const Align horizontal,
...
const UFZ::Widget& UFZ::Widget::addStringElement(
    const uint8_t x,
    const uint8_t y,
    const Align horizontal,
    const Align vertical,
...
const UFZ::Widget& UFZ::Widget::addTextBoxElement(
    const uint8_t x,
    const uint8_t y,
    const uint8_t width,
    ...
    const Align vertical,
    const char* text,
    const bool stripToDots) const noexcept {
    widget_add_text_box_element(
        widget, x, y, width, height, horizontal, vertical, text, stripToDots);
    return *this;
}

const UFZ::Widget& UFZ::Widget::addTextScrollElement(
    const uint8_t x,
    const uint8_t y,
    const uint8_t width,
    const uint8_t height,
...
const UFZ::Widget& UFZ::Widget::addButtonElement(
    const GuiButtonType type,
    const char* text,
...
const UFZ::Widget& UFZ::Widget::addIconElement(const uint8_t x, const uint8_t y, const Icon* icon)
    const noexcept {
    widget_add_icon_element(widget, x, y, icon);
    return *this;
}

const UFZ::Widget& UFZ::Widget::addFrameElement(
    const uint8_t x,
    const uint8_t y,
    const uint8_t width,
    const uint8_t height,
    const uint8_t radius) const noexcept {
