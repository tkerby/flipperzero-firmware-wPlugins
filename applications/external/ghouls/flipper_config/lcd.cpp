...
void lcd_draw_pixel(uint16_t x, uint16_t y, uint16_t color) {
    if(canvas && x < LCD_WIDTH && y < LCD_HEIGHT) {
...
void lcd_fill_triangle(
    ...
    if(x1 >= LCD_WIDTH || x2 >= LCD_WIDTH || x3 >= LCD_WIDTH || y1 >= LCD_HEIGHT ||
       y2 >= LCD_HEIGHT || y3 >= LCD_HEIGHT) {
        return; // Out of bounds, do not attempt to draw
    }
    ...
    for(int scanY = iy1; scanY <= iy3; scanY++) {
        if(scanY < 0 || scanY >= LCD_HEIGHT) continue;

        float x_long = p1_x + (p3_x - p1_x) * (scanY - iy1) / (float)(iy3 - iy1);
        float x_short;

        if(scanY <= iy2)
            x_short = (iy2 != iy1) ? p1_x + (p2_x - p1_x) * (scanY - iy1) / (float)(iy2 - iy1) :
                                     p1_x;
        else
            x_short = (iy3 != iy2) ? p2_x + (p3_x - p2_x) * (scanY - iy2) / (float)(iy3 - iy2) :
                                     p2_x;
...
void lcd_fill_round_rectangle(
    ...
    if(ix + iw > LCD_WIDTH) iw = LCD_WIDTH - ix;
    if(iy + ih > LCD_HEIGHT) ih = LCD_HEIGHT - iy;
    if(iw <= 0 || ih <= 0) return;
    ...
    for(int py = iy; py < iy + ih; py++) {
        for(int px = ix; px < ix + iw; px++) {
            ...
            if(px < tl_cx && py < tl_cy) {
                ...
                if(dx * dx + dy * dy > rsq) in_corner = true;
...
void lcd_draw_triangle(
    ...
    if(canvas) {
        ...
        if(x2 < LCD_WIDTH && x3 < LCD_WIDTH && y2 < LCD_HEIGHT && y3 < LCD_HEIGHT) {
            canvas_draw_line(canvas, x2, y2, x3, y3);
        }
        if(x3 < LCD_WIDTH && x1 < LCD_WIDTH && y3 < LCD_HEIGHT && y1 < LCD_HEIGHT) {
