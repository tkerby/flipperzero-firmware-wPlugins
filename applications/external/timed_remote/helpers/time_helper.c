...
uint32_t time_helper_seconds_until(uint8_t target_h, uint8_t target_m, uint8_t target_s) {
    ...
    uint32_t now_seconds = time_helper_hms_to_seconds(now.hour, now.minute, now.second);
    uint32_t target_seconds = time_helper_hms_to_seconds(target_h, target_m, target_s);
