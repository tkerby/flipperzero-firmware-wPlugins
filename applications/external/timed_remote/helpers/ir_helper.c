...
static void ir_signal_list_add(IrSignalList* list, InfraredSignal* signal, const char* name) {
...
bool ir_helper_list_files(const char* dir_path, FuriString*** files, size_t* count) {
    ...
    *count = file_count;
    ...
    while(storage_dir_read(dir, &file_info, name_buf, sizeof(name_buf)) && idx < file_count) {
        if(!(file_info.flags & FSF_DIRECTORY)) {
            size_t len = strlen(name_buf);
            if(len > 3 && strcmp(name_buf + len - 3, ".ir") == 0) {
