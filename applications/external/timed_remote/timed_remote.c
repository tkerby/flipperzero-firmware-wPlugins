...
static bool timed_remote_custom_event_callback(void* context, uint32_t custom_event) {
    TimedRemoteApp* app = context;
...
TimedRemoteApp* timed_remote_app_alloc(void) {
    ...
    view_dispatcher_set_custom_event_callback(
        app->view_dispatcher, timed_remote_custom_event_callback);
    ...
    view_dispatcher_add_view(
        app->view_dispatcher, TimedRemoteViewSubmenu, submenu_get_view(app->submenu));
    ...
    view_dispatcher_add_view(
        app->view_dispatcher, TimedRemoteViewTextInput, text_input_get_view(app->text_input));

    app->widget = widget_alloc();
    view_dispatcher_add_view(
        app->view_dispatcher, TimedRemoteViewWidget, widget_get_view(app->widget));

    app->popup = popup_alloc();
    view_dispatcher_add_view(
        app->view_dispatcher, TimedRemoteViewPopup, popup_get_view(app->popup));
...
void timed_remote_app_free(TimedRemoteApp* app) {
    ...
    view_dispatcher_remove_view(app->view_dispatcher, TimedRemoteViewVariableItemList);
