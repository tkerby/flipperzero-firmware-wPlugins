...
static void timer_config_mode_change(VariableItem* item) {
    ...
    view_dispatcher_send_custom_event(app->view_dispatcher, TimedRemoteEventModeChanged);
}
...
static void timer_config_enter_callback(void* context, uint32_t index) {
    ...
    uint32_t confirm_index = app->timer_mode == TimerModeCountdown ? TimerConfigIndexConfirm :
                                                                     (TimerConfigIndexSeconds + 1);
    if(index == confirm_index) {
        view_dispatcher_send_custom_event(app->view_dispatcher, TimedRemoteEventTimerConfigured);
    }
}
...
static void build_timer_config_list(TimedRemoteApp* app) {
    ...
    item =
        variable_item_list_add(app->variable_item_list, "Mode", 2, timer_config_mode_change, app);
    variable_item_set_current_value_index(item, app->timer_mode == TimerModeCountdown ? 0 : 1);
    variable_item_set_current_value_text(
        item, app->timer_mode == TimerModeCountdown ? "Countdown" : "Scheduled");

    /* Hours: 0-23 */
    item = variable_item_list_add(
        app->variable_item_list, "Hours", 24, timer_config_hours_change, app);
    ...
    /* Minutes: 0-59 */
    item = variable_item_list_add(
        app->variable_item_list, "Minutes", 60, timer_config_minutes_change, app);
    ...
    /* Seconds: 0-59 */
    item = variable_item_list_add(
        app->variable_item_list, "Seconds", 60, timer_config_seconds_change, app);
    ...
    if(app->timer_mode == TimerModeCountdown) {
        ...
        item = variable_item_list_add(
            app->variable_item_list, "Repeat", 101, timer_config_repeat_change, app);
    ...
    /* Confirm button */
    variable_item_list_add(app->variable_item_list, ">> Start Timer <<", 0, NULL, NULL);

    variable_item_list_set_enter_callback(
        app->variable_item_list, timer_config_enter_callback, app);
}

void timed_remote_scene_timer_config_on_enter(void* context) {
    TimedRemoteApp* app = context;
    build_timer_config_list(app);
    view_dispatcher_switch_to_view(app->view_dispatcher, TimedRemoteViewVariableItemList);
}

bool timed_remote_scene_timer_config_on_event(void* context, SceneManagerEvent event) {
    TimedRemoteApp* app = context;
    bool consumed = false;
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        if(event.event == TimedRemoteEventModeChanged) {
            ...
            scene_manager_next_scene(app->scene_manager, TimedRemoteSceneTimerRunning);
            consumed = true;
        }
    }
