...
static void confirm_popup_callback(void* context) {
    ...
    view_dispatcher_send_custom_event(app->view_dispatcher, TimedRemoteEventConfirmDone);
}
...
void timed_remote_scene_confirm_on_enter(void* context) {
    ...
    popup_set_text(app->popup, app->signal_name, 64, 35, AlignCenter, AlignCenter);
...
bool timed_remote_scene_confirm_on_event(void* context, SceneManagerEvent event) {
    TimedRemoteApp* app = context;
    bool consumed = false;
