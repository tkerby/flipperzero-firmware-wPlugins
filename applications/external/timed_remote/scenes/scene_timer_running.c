...
static void timer_callback(void* context) {
    ...
    view_dispatcher_send_custom_event(app->view_dispatcher, TimedRemoteEventTimerTick);
}
...
static void update_display(TimedRemoteApp* app) {
    ...
    widget_add_string_element(
        app->widget, 64, 5, AlignCenter, AlignTop, FontSecondary, app->signal_name);
    widget_add_string_element(
        app->widget, 64, 25, AlignCenter, AlignTop, FontBigNumbers, time_str);
    ...
    if(app->repeat_count > 0 && app->repeat_count < 255) {
        ...
        snprintf(
            ...
            sizeof(repeat_str),
            "Repeat: %d/%d",
            app->repeat_count - app->repeats_remaining + 1,
            app->repeat_count);
        widget_add_string_element(
            app->widget, 64, 52, AlignCenter, AlignTop, FontSecondary, repeat_str);
...
void timed_remote_scene_timer_running_on_enter(void* context) {
    ...
    if(app->timer_mode == TimerModeCountdown) {
        ...
        app->seconds_remaining = time_helper_seconds_until(app->hours, app->minutes, app->seconds);
        if(app->seconds_remaining == 0) {
            /* Target time already passed - fire immediately */
            view_dispatcher_send_custom_event(app->view_dispatcher, TimedRemoteEventTimerFired);
            return;
        }
    }
...
bool timed_remote_scene_timer_running_on_event(void* context, SceneManagerEvent event) {
    TimedRemoteApp* app = context;
    bool consumed = false;
        ...
        if(event.event == TimedRemoteEventTimerTick) {
            ...
            if(app->seconds_remaining == 0) {
                ...
                view_dispatcher_send_custom_event(
                    app->view_dispatcher, TimedRemoteEventTimerFired);
            }
            consumed = true;
