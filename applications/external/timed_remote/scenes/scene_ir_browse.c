...
static void ir_browse_callback(void* context, uint32_t index) {
    ...
    if(index < file_count) {
        ...
        snprintf(
            ...
            sizeof(app->selected_file_path),
            "%s/%s",
            IR_DIR_PATH,
            furi_string_get_cstr(file_list[index]));
        view_dispatcher_send_custom_event(app->view_dispatcher, TimedRemoteEventFileSelected);
    }
}
    ...
    if(ir_helper_list_files(IR_DIR_PATH, &file_list, &file_count)) {
        if(file_count == 0) {
            ...
            for(size_t i = 0; i < file_count; i++) {
                submenu_add_item(
                    app->submenu, furi_string_get_cstr(file_list[i]), i, ir_browse_callback, app);
            }
        }
    } else {
...
bool timed_remote_scene_ir_browse_on_event(void* context, SceneManagerEvent event) {
    TimedRemoteApp* app = context;
    bool consumed = false;
