...
def update_site(env):
    ...
    for version in env.versions:
        ...
        build.build_docs(
            version,
            doc_dir=target_doc_dir,
            include_dir=include_dir,
            work_dir=env.build_dir,
        )
        ...
        for link, target in {"index": "contents", "api": "reference"}.items():
            ...
            target += ".html"
            if os.path.exists(os.path.join(html_dir, target)) and not os.path.exists(
                link
            ):
...
def release(args):
    ...
    for line in fileinput.input(os.path.join(fmt_repo.dir, cmakelists), inplace=True):
