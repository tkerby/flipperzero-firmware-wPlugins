...
// Input for the longitude of the NDEF Location record.
// Stored in ndef_buf2. Email keyboard: . and - available.

static void nfc_tools_scene_write_location_lon_callback(void* context) {
...
void nfc_tools_scene_write_location_lon_on_enter(void* context) {
    ...
    email_input_set_result_callback(
        ei,
        nfc_tools_scene_write_location_lon_callback,
        app,
        app->ndef_buf2,
...
void nfc_tools_scene_write_location_lon_on_exit(void* context) {
    NfcToolsApp* app = context;
