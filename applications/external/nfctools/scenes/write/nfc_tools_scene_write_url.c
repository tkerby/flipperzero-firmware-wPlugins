...
static void nfc_tools_scene_write_url_callback(void* context) {
    NfcToolsApp* app = context;
    view_dispatcher_send_custom_event(app->view_dispatcher, 0);
}

void nfc_tools_scene_write_url_on_enter(void* context) {
    NfcToolsApp* app = context;
    SpecialInput* input = app->special_input;
    special_input_set_header_text(input, NTS_INPUT_URL);
    special_input_set_result_callback(
        input,
        nfc_tools_scene_write_url_callback,
        app,
        app->ndef_buf1,
...
bool nfc_tools_scene_write_url_on_event(void* context, SceneManagerEvent event) {
    NfcToolsApp* app = context;
    bool consumed = false;
...
void nfc_tools_scene_write_url_on_exit(void* context) {
    NfcToolsApp* app = context;
