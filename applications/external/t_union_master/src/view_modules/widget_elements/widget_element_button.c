...
static void gui_button_draw(Canvas* canvas, WidgetElement* element) {
    ...
    if(model->button_type == GuiButtonTypeLeft) {
        elements_button_left_utf8(canvas, furi_string_get_cstr(model->text));
    } else if(model->button_type == GuiButtonTypeRight) {
        elements_button_right_utf8(canvas, furi_string_get_cstr(model->text));
    } else if(model->button_type == GuiButtonTypeCenter) {
        elements_button_center_utf8(canvas, furi_string_get_cstr(model->text));
    }
}
