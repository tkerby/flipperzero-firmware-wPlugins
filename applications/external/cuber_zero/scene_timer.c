...
void SceneTimerTick(const PCUBERZERO instance) {
    ...
    if(instance->scene.timer.waitForReady &&
       furi_get_tick() - instance->scene.timer.pressedTime >= 500) {
...
void SceneTimerEnter(const PCUBERZERO instance) {
    ...
    while(furi_message_queue_get(instance->scene.timer.queue, &event, FuriWaitForever) ==
          FuriStatusOk) {
        break;
    }
...
void SceneTimerDraw(Canvas* const canvas, const PCUBERZERO instance) {
    ...
    uint32_t tick =
        (instance->scene.timer.timing ? furi_get_tick() : instance->scene.timer.stopTimer) -
        instance->scene.timer.startTimer;
    uint32_t seconds = tick / 1000;
    furi_string_printf(
        instance->scene.timer.string, "%lu:%02lu.%03lu", seconds / 60, seconds % 60, tick % 1000);
    canvas_draw_str_aligned(
        ...
        AlignCenter,
        AlignCenter,
        furi_string_get_cstr(instance->scene.timer.string));
}
