...
void Sprite3D::addTriangle(
    ...
    if(triangle_count < MAX_TRIANGLES_PER_SPRITE) {
        triangles[triangle_count] =
            ENGINE_MEM_NEW Triangle3D(x1, y1, z1, x2, y2, z2, x3, y3, z3, color);
        triangle_count++;
    }
}
...
void Sprite3D::createCube(
    ...
    addTriangle(
        Triangle3D(x - hw, y - hh, z + hd, x + hw, y - hh, z + hd, x + hw, y + hh, z + hd, color));
    addTriangle(
        Triangle3D(x - hw, y - hh, z + hd, x + hw, y + hh, z + hd, x - hw, y + hh, z + hd, color));
...
void Sprite3D::createCylinder(
    ...
    for(uint8_t i = 0; i < segments; i++) {
        ...
        addTriangle(Triangle3D(x1, y - hh, z1, x2, y - hh, z2, x2, y + hh, z2, color));
        addTriangle(Triangle3D(x1, y - hh, z1, x2, y + hh, z2, x1, y + hh, z1, color));
    }
}
...
void Sprite3D::createHumanoid(float height, uint16_t color) {
    ...
    createCube(
        0, leg_height + torso_height / 2, 0, torso_width, torso_height, torso_width * 0.8f, color);
...
void Sprite3D::createPillar(float height, float radius, uint16_t color) {
    ...
    createCylinder(
        0, pillar_radius * 0.4f, 0, pillar_radius * 1.4f, pillar_radius * 0.8f, 4, color);

    // Top - 4 segments = 8 triangles
    createCylinder(
        0, height - pillar_radius * 0.4f, 0, pillar_radius * 1.4f, pillar_radius * 0.8f, 4, color);
}
...
void Sprite3D::createTree(float height, uint16_t color) {
    ...
    createCube(
        0, trunk_height + crown_height / 2, 0, crown_width, crown_height, crown_width, color);
}
...
void Sprite3D::createTriangularPrism(
    ...
    // Front triangle
    addTriangle(
        Triangle3D(x - hw, y - hh, z + hd, x + hw, y - hh, z + hd, x, y + hh, z + hd, color));
    ...
    // Side faces
    addTriangle(Triangle3D(x - hw, y - hh, z + hd, x, y + hh, z + hd, x, y + hh, z - hd, color));
    addTriangle(
        Triangle3D(x - hw, y - hh, z + hd, x, y + hh, z - hd, x - hw, y - hh, z - hd, color));

    addTriangle(
        Triangle3D(x, y + hh, z + hd, x + hw, y - hh, z + hd, x + hw, y - hh, z - hd, color));
    addTriangle(Triangle3D(x, y + hh, z + hd, x + hw, y - hh, z - hd, x, y + hh, z - hd, color));
}
