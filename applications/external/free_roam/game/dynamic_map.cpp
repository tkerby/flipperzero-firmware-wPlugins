...
DynamicMap::DynamicMap(const char* name, uint8_t w, uint8_t h, bool addBorder)
    : width(w)
    , height(h)
    , name(name)
    , renderWallCount(0) {
...
void DynamicMap::addHorizontalWall(
    uint8_t x1,
    uint8_t x2,
    uint8_t y,
    float height,
...
void DynamicMap::addVerticalWall(
    uint8_t x,
    uint8_t y1,
    uint8_t y2,
    float height,
...
void DynamicMap::renderMiniMap(
    ...
    uint16_t background_color) const {
