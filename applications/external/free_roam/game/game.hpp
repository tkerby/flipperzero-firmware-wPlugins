...
private:
    ...
    bool httpRequestIsFinished() {
        return this->getHttpState() != INACTIVE && this->getHttpState() != RECEIVING;
    }
    ...
    float atof_furi(const FuriString* nptr) {
        return atof(furi_string_get_cstr(nptr));
    }
    int atoi(const char* nptr) {
        return (int)strtol(nptr, NULL, 10);
    }
    int atoi_furi(const FuriString* nptr) {
        return atoi(furi_string_get_cstr(nptr));
    }
