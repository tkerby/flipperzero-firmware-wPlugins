...
public:
    ...
    void addHorizontalWall(
        ...
        float depth = 0.2f,
        TileType type =
            TILE_WALL); // Add a horizontal wall and create a corresponding Sprite3D for rendering if it's a solid wall
    void addRoom(
        uint8_t x1,
        uint8_t y1,
        uint8_t x2,
    ...
    void addVerticalWall(
        ...
        float depth = 0.2f,
        TileType type =
            TILE_WALL); // Add a vertical wall and create a corresponding Sprite3D for rendering if it's a solid wall
    ...
    const Sprite3D* getRenderWall(uint8_t i) const {
        return renderWalls[i];
    } // Get the Sprite3D object for the i-th render wall, returns nullptr if index is out of bounds
    uint8_t getRenderWallCount() const {
        return renderWallCount;
    } // Get the number of render walls currently in the map
    uint8_t releaseRenderWalls(
        Sprite3D** out,
        uint8_t
            maxCount); // Transfer ownership of all render-wall Sprite3D pointers to caller; clears internal list so destructor won't delete them
    void getMiniMap(uint8_t output[MAX_MAP_HEIGHT][MAX_MAP_WIDTH])
    ...
    void renderMiniMap(
        ...
        Vector size,
        Vector player_pos,
        Vector player_dir = Vector(0, 0),
        uint16_t foreground_color = 0x0000,
        uint16_t background_color = 0xFFFF) const;
