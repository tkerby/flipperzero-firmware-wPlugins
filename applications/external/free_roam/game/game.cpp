...
bool FreeRoamGame::startGame() {
    ...
    auto game = std::make_unique<Game>(
        "Free Roam", draw->getDisplaySize(), draw.get(), 0x0000, 0xFFFF, camera.release());
    if(!game) {
    ...
    std::unique_ptr<Level> level1 =
        std::make_unique<Level>("Tutorial", draw->getDisplaySize(), game.get());
    std::unique_ptr<Level> level2 =
        std::make_unique<Level>("First", draw->getDisplaySize(), game.get());
    std::unique_ptr<Level> level3 =
        std::make_unique<Level>("Second", draw->getDisplaySize(), game.get());
    ...
    // add some 3D sprites
    std::unique_ptr<Entity> tutorialGuard1 = std::make_unique<Sprite>(
        "Tutorial Guard 1", Vector(3, 7), SPRITE_3D_HUMANOID, 1.7f, M_PI / 4, 0.f, Vector(9, 7));
    std::unique_ptr<Entity> tutorialGuard2 = std::make_unique<Sprite>(
        "Tutorial Guard 2", Vector(6, 2), SPRITE_3D_HUMANOID, 1.7f, M_PI / 4, 0.f, Vector(1, 2));
    ...
    std::unique_ptr<Entity> house1 =
        std::make_unique<Sprite>("House 1", Vector(14, 13), SPRITE_3D_HOUSE, 10.0f, 10.0f, 0.0f);
    std::unique_ptr<Entity> house2 = std::make_unique<Sprite>(
        "House 2", Vector(48, 13), SPRITE_3D_HOUSE, 10.0f, 10.0f, (float)(M_PI / 2.0));
    std::unique_ptr<Entity> house3 = std::make_unique<Sprite>(
        "House 3", Vector(14, 43), SPRITE_3D_HOUSE, 10.0f, 10.0f, (float)(M_PI));
    std::unique_ptr<Entity> house4 = std::make_unique<Sprite>(
        "House 4", Vector(48, 43), SPRITE_3D_HOUSE, 10.0f, 10.0f, (float)(M_PI * 1.5));
    ...
    for(uint8_t i = 0; i < 16; i++) {
        level3->entity_add(
            new Sprite(treeNames[i], treePositions[i], SPRITE_3D_TREE, 3.0f, 1.0f, 0.0f));
    }
...
bool FreeRoamGame::startGameOnline() {
    ...
    std::unique_ptr<Level> level_online =
        std::make_unique<Level>("Online", draw->getDisplaySize(), game.get());
    level_online->entity_add(player.get());

    // 4 houses in the four quadrants (mirrors town layout)
    level_online->entity_add(
        new Sprite("House 1", Vector(14, 13), SPRITE_3D_HOUSE, 10.0f, 10.0f, 0.0f));
    level_online->entity_add(
        new Sprite("House 2", Vector(48, 13), SPRITE_3D_HOUSE, 10.0f, 10.0f, (float)(M_PI / 2.0)));
    level_online->entity_add(
        new Sprite("House 3", Vector(14, 43), SPRITE_3D_HOUSE, 10.0f, 10.0f, (float)(M_PI)));
    level_online->entity_add(
        new Sprite("House 4", Vector(48, 43), SPRITE_3D_HOUSE, 10.0f, 10.0f, (float)(M_PI * 1.5)));
    ...
    for(uint8_t i = 0; i < 16; i++) {
        level_online->entity_add(new Sprite(
            onlineTreeNames[i], onlineTreePositions[i], SPRITE_3D_TREE, 3.0f, 1.0f, 0.0f));
    }
...
void FreeRoamGame::switchToLevel(int levelIndex) {
    if(!isGameRunning || !engine || !engine->getGame()) return;
