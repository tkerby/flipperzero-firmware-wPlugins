...
static const struct usb_string_descriptor dev_security_desc = USB_ARRAY_DESC(
    0x58,
    0x62,
    0x6f,
    0x78,
...
static usbd_respond pof_usb_ep_config(usbd_device* dev, uint8_t cfg) {
    ...
    case 1: // config
        ...
        usbd_ep_config(
            dev, POF_USB_X360_PLUGIN_MODULE_EP_IN, USB_EPTYPE_INTERRUPT, POF_USB_EP_OUT_SIZE);
        return usbd_ack;
    }
    return usbd_fail;
}
...
static const struct PoFUsbDescriptorXbox360 usb_pof_cfg_descr_x360 = {
    ...
    .audio_desc =
        {0x1B,
         ...
         0x01,
         0x01,
         POF_USB_X360_AUDIO_EP_IN1,
         0x40,
         0x01,
         POF_USB_X360_AUDIO_EP_OUT1,
         0x20,
         0x16,
         POF_USB_X360_AUDIO_EP_IN2,
         0x00,
         0x00,
         0x00,
         0x00,
    ...
    .security_desc = {0x06, 0x41, 0x00, 0x01, 0x01, 0x03},
};
...
static usbd_respond
    ...
    if(req->bmRequestType == 0x41 && req->bRequest == 00 &&
       (req->wValue == 0x1F || req->wValue == 0x1E)) {
        return usbd_ack;
    }
    switch(req->bRequest) {
    case 0x81:
