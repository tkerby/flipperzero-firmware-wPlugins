...
#define ASSERT_EQ(expected, actual) \
    if((uint32_t)(expected) != (uint32_t)(actual)) { \
        FURI_LOG_E(TEST_TAG, "ASSERT FAILED: %s != %s (%lu != %lu)", #expected, #actual, (uint32_t)(expected), (uint32_t)(actual)); \
        return false; \
    }
...
int32_t test_uid_brute_smarter_app(void* p) {
    ...
    if(dialogs) {
        dialog_message_show_storage_error(dialogs, "Test suite completed. Check logs for results.");
