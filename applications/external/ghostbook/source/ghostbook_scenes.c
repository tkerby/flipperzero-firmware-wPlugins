...
typedef enum { MenuView=0, MenuEdit, MenuShare, MenuReceive, MenuContacts, MenuExport, MenuAbout } MainIdx;
typedef enum { EditHandle=0, EditName, EditEmail, EditDiscord, EditSignal, EditTelegram, EditNotes, EditFlair } EditIdx;
    ...
    if(e.type == SceneManagerEventTypeCustom && e.event == EvPasscodeComplete) {
        if(ghost_auth_verify(app, app->passcode_input)) {
            ...
            if(app->auth.failed_attempts >= app->auth.max_attempts) {
                ...
                char msg[64];
                snprintf(msg, sizeof(msg), "%d attempts remaining\nAll data will be WIPED", 
...
bool ghost_scene_setup_confirm_event(void* ctx, SceneManagerEvent e) {
    ...
    if(e.type == SceneManagerEventTypeCustom && e.event == EvPasscodeComplete) {
        ...
        if(memcmp(app->setup_passcode, app->passcode_input, app->setup_length) == 0) {
            ...
            popup_set_text(app->popup, "Passcodes didn't match\nTry again", 64, 35, AlignCenter, AlignTop);
...
void ghost_scene_wiped_enter(void* ctx) {
    ...
    for(int frame = 0; frame < GHOST_MELT_FRAMES; frame++) {
        ...
        widget_add_string_multiline_element(app->widget, 64, 22, AlignCenter, AlignTop, FontSecondary,
            ghost_melt_frames[frame]);
    ...
    widget_add_string_element(app->widget, 64, 22, AlignCenter, AlignTop, FontSecondary,
        ". . .");
    widget_add_string_element(app->widget, 64, 36, AlignCenter, AlignTop, FontSecondary,
        "All data destroyed.");
    widget_add_string_element(app->widget, 64, 48, AlignCenter, AlignTop, FontSecondary,
        "Too many failed attempts.");
    widget_add_string_element(app->widget, 64, 62, AlignCenter, AlignBottom, FontSecondary, 
        "[BACK] Exit");
...
bool ghost_scene_main_event(void* ctx, SceneManagerEvent e) {
    ...
    if(e.type == SceneManagerEventTypeCustom) {
        switch(e.event) {
            case EvView: ghost_scene_view_enter(app); return true;
            case EvEdit: ghost_scene_edit_enter(app); return true;
            case EvShare: ghost_scene_share_enter(app); return true;
            case EvReceive: ghost_scene_receive_enter(app); return true;
            case EvContacts: ghost_scene_contacts_enter(app); return true;
            case EvExport: ghost_scene_export_enter(app); return true;
            case EvAbout: ghost_scene_about_enter(app); return true;
        }
    }
    return false;
}
...
void ghost_scene_view_enter(void* ctx) {
    ...
    furi_string_printf(t, "========================\n  @%s %s\n========================\n", c->handle, ghost_flair_icon(c->flair));
...
bool ghost_scene_view_event(void* ctx, SceneManagerEvent e) {
    GhostApp* app = ctx;
    if(e.type == SceneManagerEventTypeBack) { ghost_scene_main_enter(app); return true; }
    return false;
}

void ghost_scene_view_exit(void* ctx) { GhostApp* app = ctx; widget_reset(app->widget); }
...
void ghost_scene_edit_enter(void* ctx) {
    ...
    furi_string_printf(s, "@Handle: %s", strlen(c->handle)?c->handle:"---"); submenu_add_item(app->submenu, furi_string_get_cstr(s), EditHandle, edit_cb, app);
    furi_string_printf(s, "Name: %s", strlen(c->name)?c->name:"(optional)"); submenu_add_item(app->submenu, furi_string_get_cstr(s), EditName, edit_cb, app);
    furi_string_printf(s, "Email: %s", strlen(c->email)?c->email:"---"); submenu_add_item(app->submenu, furi_string_get_cstr(s), EditEmail, edit_cb, app);
    furi_string_printf(s, "Discord: %s", strlen(c->discord)?c->discord:"---"); submenu_add_item(app->submenu, furi_string_get_cstr(s), EditDiscord, edit_cb, app);
    furi_string_printf(s, "Signal: %s", strlen(c->signal)?c->signal:"---"); submenu_add_item(app->submenu, furi_string_get_cstr(s), EditSignal, edit_cb, app);
    furi_string_printf(s, "Telegram: %s", strlen(c->telegram)?c->telegram:"---"); submenu_add_item(app->submenu, furi_string_get_cstr(s), EditTelegram, edit_cb, app);
    furi_string_printf(s, "Notes: %s", strlen(c->notes)?c->notes:"---"); submenu_add_item(app->submenu, furi_string_get_cstr(s), EditNotes, edit_cb, app);
    furi_string_printf(s, "Flair: %s %s", ghost_flair_icon(c->flair), ghost_flair_name(c->flair)); submenu_add_item(app->submenu, furi_string_get_cstr(s), EditFlair, edit_cb, app);
    furi_string_free(s);
    view_dispatcher_switch_to_view(app->view, GhostViewSubmenu);
}
...
bool ghost_scene_edit_event(void* ctx, SceneManagerEvent e) {
    GhostApp* app = ctx;
    if(e.type == SceneManagerEventTypeBack) { ghost_save_profile(app); ghost_scene_main_enter(app); return true; }
    if(e.type == SceneManagerEventTypeCustom) {
        switch(e.event) {
            case EvHandle: case EvName: case EvEmail: case EvDiscord: case EvSignal: case EvTelegram: case EvNotes: ghost_scene_text_enter(app); return true;
            case EvFlair: ghost_scene_flair_enter(app); return true;
        }
    }
    return false;
}

void ghost_scene_edit_exit(void* ctx) { GhostApp* app = ctx; submenu_reset(app->submenu); }

// ============================================================================
// TEXT INPUT SCENE
// ============================================================================

static void text_cb(void* ctx) { GhostApp* app = ctx; view_dispatcher_send_custom_event(app->view, EvTextDone); }
...
void ghost_scene_text_enter(void* ctx) {
    ...
    switch(app->edit_field) {
        case EditHandle: hdr = "Your @handle"; cur = c->handle; len = GHOST_HANDLE_LEN; break;
        case EditName: hdr = "Name (optional)"; cur = c->name; len = GHOST_NAME_LEN; break;
        case EditEmail: hdr = "Email"; cur = c->email; len = GHOST_EMAIL_LEN; break;
        case EditDiscord: hdr = "Discord"; cur = c->discord; len = GHOST_DISCORD_LEN; break;
        case EditSignal: hdr = "Signal"; cur = c->signal; len = GHOST_SIGNAL_LEN; break;
        case EditTelegram: hdr = "Telegram"; cur = c->telegram; len = GHOST_TELEGRAM_LEN; break;
        case EditNotes: hdr = "Notes"; cur = c->notes; len = GHOST_NOTES_LEN; break;
    }
...
bool ghost_scene_text_event(void* ctx, SceneManagerEvent e) {
    GhostApp* app = ctx;
    GhostCard* c = &app->my_card;
    if(e.type == SceneManagerEventTypeBack) { ghost_scene_edit_enter(app); return true; }
    if(e.type == SceneManagerEventTypeCustom && e.event == EvTextDone) {
        switch(app->edit_field) {
            case EditHandle: strncpy(c->handle, app->text_buf, GHOST_HANDLE_LEN-1); break;
            case EditName: strncpy(c->name, app->text_buf, GHOST_NAME_LEN-1); break;
            case EditEmail: strncpy(c->email, app->text_buf, GHOST_EMAIL_LEN-1); break;
            case EditDiscord: strncpy(c->discord, app->text_buf, GHOST_DISCORD_LEN-1); break;
            case EditSignal: strncpy(c->signal, app->text_buf, GHOST_SIGNAL_LEN-1); break;
            case EditTelegram: strncpy(c->telegram, app->text_buf, GHOST_TELEGRAM_LEN-1); break;
            case EditNotes: strncpy(c->notes, app->text_buf, GHOST_NOTES_LEN-1); break;
        }
...
void ghost_scene_text_exit(void* ctx) { GhostApp* app = ctx; text_input_reset(app->text_input); }
...
bool ghost_scene_flair_event(void* ctx, SceneManagerEvent e) {
    GhostApp* app = ctx;
    if(e.type == SceneManagerEventTypeBack || (e.type == SceneManagerEventTypeCustom && e.event == EvFlairDone)) {
        ghost_scene_edit_enter(app);
        return true;
    }
    return false;
...
void ghost_scene_flair_exit(void* ctx) { GhostApp* app = ctx; submenu_reset(app->submenu); }
...
void ghost_scene_share_enter(void* ctx) {
    ...
    widget_add_string_element(app->widget, 64, 14, AlignCenter, AlignTop, FontPrimary, "BROADCASTING");
    
    // Handle
    FuriString* s = furi_string_alloc();
    furi_string_printf(s, "@%s", app->my_card.handle);
    widget_add_string_element(app->widget, 64, 32, AlignCenter, AlignTop, FontPrimary, furi_string_get_cstr(s));
    furi_string_free(s);
    
    widget_add_string_element(app->widget, 64, 48, AlignCenter, AlignTop, FontSecondary, "Place near receiver");
    widget_add_string_element(app->widget, 64, 62, AlignCenter, AlignBottom, FontSecondary, "[BACK] Stop");
...
void ghost_scene_receive_enter(void* ctx) {
    ...
    widget_add_string_element(app->widget, 64, 2, AlignCenter, AlignTop, FontSecondary, "(o_o)");
    widget_add_string_element(app->widget, 64, 14, AlignCenter, AlignTop, FontPrimary, "LISTENING");
    widget_add_string_element(app->widget, 64, 34, AlignCenter, AlignTop, FontSecondary, "Place near sender");
    widget_add_string_element(app->widget, 64, 62, AlignCenter, AlignBottom, FontSecondary, "[BACK] Cancel");
...
void ghost_scene_contacts_enter(void* ctx) {
    ...
    if(app->contact_count == 0) {
        ...
        for(uint32_t i = 0; i < app->contact_count; i++) {
            furi_string_printf(s, "@%s %s", app->contacts[i].card.handle, ghost_flair_icon(app->contacts[i].card.flair));
...
bool ghost_scene_contacts_event(void* ctx, SceneManagerEvent e) {
    GhostApp* app = ctx;
    if(e.type == SceneManagerEventTypeBack) { ghost_scene_main_enter(app); return true; }
    if(e.type == SceneManagerEventTypeCustom && e.event == EvContactView) { ghost_scene_contact_view_enter(app); return true; }
    return false;
}

void ghost_scene_contacts_exit(void* ctx) { GhostApp* app = ctx; submenu_reset(app->submenu); }
...
void ghost_scene_contact_view_enter(void* ctx) {
    ...
    if(app->selected_contact >= app->contact_count) { ghost_scene_contacts_enter(app); return; }
...
bool ghost_scene_contact_view_event(void* ctx, SceneManagerEvent e) {
    GhostApp* app = ctx;
    if(e.type == SceneManagerEventTypeBack) { ghost_scene_contacts_enter(app); return true; }
    return false;
}

void ghost_scene_contact_view_exit(void* ctx) { GhostApp* app = ctx; widget_reset(app->widget); }
...
static void export_cb(void* ctx, uint32_t idx) {
    ...
    switch(idx) {
        case 0: ok = ghost_export_vcard(app, &app->my_card, "/ext/ghostbook_me.vcf"); msg = ok ? "ghostbook_me.vcf" : "Failed"; break;
        case 1: ok = ghost_export_all_vcard(app, "/ext/ghostbook_all.vcf"); msg = ok ? "ghostbook_all.vcf" : "Failed"; break;
        case 2: ok = ghost_export_csv(app, "/ext/ghostbook.csv"); msg = ok ? "ghostbook.csv" : "Failed"; break;
    }
    popup_reset(app->popup);
...
void ghost_scene_export_exit(void* ctx) { GhostApp* app = ctx; submenu_reset(app->submenu); }
...
void ghost_scene_about_exit(void* ctx) { GhostApp* app = ctx; widget_reset(app->widget); }
