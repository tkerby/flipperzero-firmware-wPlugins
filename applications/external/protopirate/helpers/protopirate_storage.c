           ...
           uint32_array_size > 0) {
            ...
            if(raw_array) {
                if(flipper_format_read_int32(
                       flipper_format, "RAW_Data", raw_array, uint32_array_size))
                    flipper_format_write_int32(
                        save_file, "RAW_Data", raw_array, uint32_array_size);
                free(raw_array);
            }
        }
...
bool protopirate_storage_get_file_by_index(
    ...
    if(out_path)
        furi_string_printf(
            ...
            PROTOPIRATE_APP_EXTENSION);
    if(out_name) furi_string_set_str(out_name, g_file_entries[index].name);
    return true;
}
