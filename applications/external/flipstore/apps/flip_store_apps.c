...
bool flip_store_process_app_list(FlipperHTTP* fhttp) {
    ...
    for(int i = 0; i < MAX_APP_COUNT; i++) {
        ...
        snprintf(
            flip_catalog[i].app_name, MAX_APP_NAME_LENGTH, "%s", furi_string_get_cstr(app_name));
        ...
        snprintf(
            flip_catalog[i].app_description,
            MAX_APP_DESCRIPTION_LENGTH,
            "%s",
            furi_string_get_cstr(app_description));
        ...
        snprintf(
            flip_catalog[i].app_version,
            MAX_APP_VERSION_LENGTH,
            "%s",
            furi_string_get_cstr(app_version));
...
bool flip_store_install_app(FlipperHTTP* fhttp, char* category) {
    ...
    return flip_store_get_fap_file(
        fhttp, flip_catalog[app_selected_index].app_build_id, target, api_major, api_minor);
}
