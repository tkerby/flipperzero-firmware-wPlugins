...
bool FlipDownloaderApp::hasWiFiCredentials() {
    ...
    return load_char("wifi_ssid", ssid, sizeof(ssid)) &&
           load_char("wifi_pass", password, sizeof(password)) && strlen(ssid) > 0 &&
           strlen(password) > 0;
}
...
FlipDownloaderApp::FlipDownloaderApp() {
    ...
    if(!easy_flipper_set_submenu(
           &submenu, FlipDownloaderViewSubmenu, VERSION_TAG, callbackExitApp, &viewDispatcher)) {
    ...
    submenu_add_item(
        submenu, "Settings", FlipDownloaderSubmenuSettings, submenu_choices_callback, this);
