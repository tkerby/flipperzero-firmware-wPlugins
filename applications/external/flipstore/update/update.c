...
static bool update_save_char(const char* path_name, const char* value) {
    if(!value) {
        return false;
    }
...
static bool update_load_char(const char* path_name, char* value, size_t value_size) {
    if(!value) {
        return false;
    }
    Storage* storage = furi_record_open(RECORD_STORAGE);
...
static bool update_get_fap_file(FlipperHTTP* fhttp, bool flipper_server) {
    ...
    if(strlen(MOM_FOLDER) == 0)
        ...
        snprintf(
            fhttp->file_path,
            sizeof(fhttp->file_path),
            STORAGE_EXT_PATH_PREFIX "/apps/%s/%s/%s.fap",
            APP_FOLDER,
            MOM_FOLDER,
            FAP_ID);
#endif
