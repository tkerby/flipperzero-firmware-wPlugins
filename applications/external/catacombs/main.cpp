...
static void input_events_callback(const void* value, void* ctx) {
    ...
    if(event->type == InputTypeRepeat && event->key == InputKeyBack && state->back_hold_active &&
...
extern "C" int32_t arduboy3d_app(void* p) {
    ...
    if(g_state->input_sub) furi_pubsub_unsubscribe(g_state->input_events, g_state->input_sub);
