...
static void framebuffer_commit_callback(
    ...
    if(Game::IsInMenu()) {
        memcpy(data, state->screen_buffer, BUFFER_SIZE);
    } else {
        for(size_t i = 0; i < BUFFER_SIZE; i++) {
            data[i] = (uint8_t)(state->screen_buffer[i] ^ 0xFF);
        }
    }
...
extern "C" int32_t arduboy3d_app(void* p) {
    ...
    gui_add_view_port(g_state->gui, g_state->view_port, GuiLayerFullscreen);
