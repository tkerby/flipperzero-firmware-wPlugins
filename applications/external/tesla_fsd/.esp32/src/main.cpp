...
static void button_tick() {
    ...
    if(g_pending_clicks > 0 && !g_btn_down && (now - g_last_release_ms) >= DOUBLE_CLICK_MS) {
...
static void process_frame(const CanFrame& frame) {
    ...
    if(frame.id == CAN_ID_GTW_CAR_CONFIG) {
        ...
        if(hw != TeslaHW_Unknown && g_state.hw_version == TeslaHW_Unknown) {
            ...
            const char* hw_str = (hw == TeslaHW_HW4) ? "HW4" :
                                 (hw == TeslaHW_HW3) ? "HW3" :
                                                       "Legacy";
    ...
    if(frame.id == CAN_ID_EPAS_STATUS) {
        CanFrame echo;
        if(fsd_handle_nag_killer(&g_state, &frame, &echo) && tx) g_can->send(echo);
        return;
    }
    ...
    if(frame.id == CAN_ID_AP_LEGACY && g_state.hw_version == TeslaHW_Legacy) {
        CanFrame f = frame;
        if(fsd_handle_legacy_autopilot(&g_state, &f) && tx) g_can->send(f);
        return;
    }

    // ISA speed chime (0x399, HW4 only)
    if(frame.id == CAN_ID_ISA_SPEED && g_state.hw_version == TeslaHW_HW4 &&
       g_state.suppress_speed_chime) {
        CanFrame f = frame;
        if(fsd_handle_isa_speed_chime(&f) && tx) g_can->send(f);
        return;
    }
    ...
    if(frame.id == CAN_ID_AP_CONTROL) {
        CanFrame f = frame;
        if(fsd_handle_autopilot_frame(&g_state, &f) && tx) g_can->send(f);
        return;
    }
}
...
void loop() {
    ...
    if(g_state.bms_output && g_state.bms_seen && (now - last_bms_ms) >= BMS_PRINT_MS) {
        float kw = g_state.pack_voltage_v * g_state.pack_current_a / 1000.0f;
        Serial.printf(
            "[BMS] %.1fV  %.1fA  %.2fkW  SoC:%.1f%%  Temp:%d~%d°C\n",
    ...
    if(g_state.op_mode == OpMode_Active && (now - last_status_ms) >= STATUS_PRINT_MS) {
        const char* hw_str = (g_state.hw_version == TeslaHW_HW4)    ? "HW4" :
    ...
    if(g_state.rx_count == 0 && now > WIRING_WARN_MS && (now - last_warn_ms) >= 2000u) {
