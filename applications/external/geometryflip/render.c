...
void draw_ufo(Canvas* canvas, float y) {
    ...
    canvas_draw_line(
        canvas, CUBE_X_POS, ufo_y + UFO_SIZE / 2, CUBE_X_POS + UFO_SIZE, ufo_y + UFO_SIZE / 2);
}
    ...
    for(int i = 0; i < 3; i++) {
        ...
        if(i == 1) {
            ...
            case ObstacleTypePortalBall:
                canvas_draw_circle(
                    canvas, screen_x + OBSTACLE_WIDTH / 2, screen_y + OBSTACLE_HEIGHT / 2, 3);
                break;
        ...
        for(int j = 0; j < app->current_level.length; j++) {
            ...
            if(obs->grid_x == grid_x) {
                if(obs->type == ObstacleTypeBlock) {
                    ...
                    canvas_draw_box(
                        canvas, obstacle_screen_x, screen_y, platform_width, OBSTACLE_HEIGHT);
                } else {
