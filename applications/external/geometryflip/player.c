...
void player_update(PlayerState* player, uint32_t dt_ms, bool input_pressed, GameState state) {
    ...
    case PlayerModeShip:
        ...
        if(input_pressed) {
            float target_velocity =
                (player->gravity_dir == GravityDirectionDown) ? SHIP_FLY_FORCE : -SHIP_FLY_FORCE;
            // Плавное приближение к целевой скорости
            if(fabsf(player->cube_velocity_y - target_velocity) > 1.0f) {
                player->cube_velocity_y +=
                    (target_velocity - player->cube_velocity_y) * SHIP_ACCELERATION * dt;
            } else {
                player->cube_velocity_y = target_velocity;
            }
        } else {
            // Используем гравитацию для определения целевой скорости
            float target_velocity = (player->gravity_dir == GravityDirectionDown) ? SHIP_GRAVITY :
                                                                                    -SHIP_GRAVITY;
            if(fabsf(player->cube_velocity_y - target_velocity) > 1.0f) {
                player->cube_velocity_y +=
                    (target_velocity - player->cube_velocity_y) * SHIP_ACCELERATION * dt;
        ...
        if(player->gravity_dir == GravityDirectionDown) {
            if(player->cube_velocity_y > SHIP_MAX_FALLSPEED)
                player->cube_velocity_y = SHIP_MAX_FALLSPEED;
            if(player->cube_velocity_y < SHIP_MAX_RISESPEED)
                player->cube_velocity_y = SHIP_MAX_RISESPEED;
        } else {
            if(player->cube_velocity_y < -SHIP_MAX_FALLSPEED)
                player->cube_velocity_y = -SHIP_MAX_FALLSPEED;
            if(player->cube_velocity_y > -SHIP_MAX_RISESPEED)
    ...
    case PlayerModeBall:
        ...
        float ball_gravity = (player->gravity_dir == GravityDirectionDown) ? BALL_GRAVITY :
                                                                             -BALL_GRAVITY;
        ...
        if(player->cube_y >= GROUND_Y - BALL_SIZE) {
            ...
            player->cube_velocity_y =
                fabsf(player->cube_velocity_y) * 0.8f; // Отскок с потерей энергии
            player->is_on_ground = true;
            player->ground_lost_time = furi_get_tick();
        }
    ...
    if(input_pressed) {
        ...
        case PlayerModeCube:
            // Only jump if on ground or within coyote time
            {
                ...
                bool can_jump = player->is_on_ground ||
                                (!player->is_on_ground && time_since_ground_lost < COYOTE_TIME_MS);
        ...
        case PlayerModeBall:
            ...
            if(player->ball_flip_timer == 0) {
                ...
                player->cube_velocity_y =
                    (player->gravity_dir == GravityDirectionDown) ? BALL_JUMP_VEL : -BALL_JUMP_VEL;
