...
bool co2_logger_read_gas_concentration(co2_logger* instance, uint32_t* value) {
    ...
    FURI_LOG_D(
        "UART",
        "Sending command: FF %02X %02X 00 00 00 00 00 %02X",
        buffer[1],
        buffer[2],
        buffer[8]);
    ...
    do {
        ...
        FURI_LOG_D(
            "UART", "Received %zu bytes (expected %d)", read_size, co2_logger_UART_EXCHANGE_SIZE);

        if(read_size != co2_logger_UART_EXCHANGE_SIZE) {
            FURI_LOG_D(
                "UART", "RX failed %zu (expected %d)", read_size, co2_logger_UART_EXCHANGE_SIZE);
            break;
        }
        ...
        FURI_LOG_D(
            "UART",
            "RX Data: %02X %02X %02X %02X %02X %02X %02X %02X %02X",
            buffer[0],
            buffer[1],
            buffer[2],
            buffer[3],
        ...
        *value = (uint32_t)buffer[2] * 256 + buffer[3];
        FURI_LOG_D(
            "UART",
            "Successfully read CO2: %lu ppm (bytes[2]=%02X, bytes[3]=%02X)",
            *value,
            buffer[2],
            buffer[3]);
