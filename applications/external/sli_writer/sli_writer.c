...
static bool magic_write_uid(Iso15693_3Poller* iso, const uint8_t uid_new[8]) {
    uint8_t frame_high[8] = {
        0x02, 0xE0, 0x09, SLI_MAGIC_SET_UID_HIGH, uid_new[7], uid_new[6], uid_new[5], uid_new[4]};
    uint8_t frame_low[8] = {
        0x02, 0xE0, 0x09, SLI_MAGIC_SET_UID_LOW, uid_new[3], uid_new[2], uid_new[1], uid_new[0]};

    FURI_LOG_I(
        TAG, "Magic UID HIGH: %02X%02X%02X%02X", uid_new[0], uid_new[1], uid_new[2], uid_new[3]);
    bool ok = false;
    for(int i = 0; i < 3; i++) {
    ...
    FURI_LOG_I(
        TAG, "Magic UID LOW:  %02X%02X%02X%02X", uid_new[4], uid_new[5], uid_new[6], uid_new[7]);
    ok = false;
    for(int i = 0; i < 3; i++) {
...
static NfcCommand sli_poller_callback(NfcGenericEvent event, void* context) {
    ...
    if(uid_err == Iso15693_3ErrorNone) {
        ...
        FURI_LOG_I(
            TAG,
            "Card UID (MSB first): %02X%02X%02X%02X%02X%02X%02X%02X",
            app->detected_uid[0],
            app->detected_uid[1],
            app->detected_uid[2],
            app->detected_uid[3],
