...
class Decryptor:
    def __init__(self, data: bytes, priv: int):
        ...
        eph_key = ec.EllipticCurvePublicKey.from_encoded_point(
            ec.SECP224R1(), data[5 + offsetAdjustment : 62 + offsetAdjustment]
        )
        shared_key = ec.derive_private_key(
            priv, ec.SECP224R1(), default_backend()
        ).exchange(ec.ECDH(), eph_key)
        symmetric_key = self.__sha256(
            shared_key
            + b"\x00\x00\x00\x01"
            + data[5 + offsetAdjustment : 62 + offsetAdjustment]
        )
    ...
    def Decrypt(self):
        decryptor = Cipher(
            self.algorithm_dkey, self.mode, default_backend()
        ).decryptor()
