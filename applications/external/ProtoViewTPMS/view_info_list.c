...
static void render_tyre_info(Canvas* const canvas, ProtoViewApp* app) {
    ...
    for (i = firstrow; i < app->tyre_list_count; i++) {
        ...
        if (app->tyre_list[i].favorite) {
            if (show_in_bar) {
                snprintf(buf, sizeof(buf), "*: %s %s: %s %sC", app->tyre_list[i].serial, "Bar", app->tyre_list[i].pressure_bar, app->tyre_list[i].temperature);
            } else {
                snprintf(buf, sizeof(buf), "*: %s %s: %s %sC", app->tyre_list[i].serial, app->tyre_list[i].uom, app->tyre_list[i].pressure, app->tyre_list[i].temperature);
            }
