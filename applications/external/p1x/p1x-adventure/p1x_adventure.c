...
static GameState game_state = {
    ...
    .player = {.x = 10, .y = 30, .health = 100, .direction = DIR_RIGHT, .weapon_active = false, .weapon_timer = 0},
    .score = 0,
    .selected_upgrade = 0,
...
static void get_weapon_end_point(int* end_x, int* end_y) {
    ...
    *end_y = game_state.player.y + 4;
    switch(game_state.player.direction) {
        case DIR_UP: *end_y -= game_state.weapon_length; break;
        case DIR_RIGHT: *end_x += game_state.weapon_length; break;
        case DIR_DOWN: *end_y += game_state.weapon_length; break;
        case DIR_LEFT: *end_x -= game_state.weapon_length; break;
    }
}
...
static bool check_castle_reached() {
    ...
    return (game_state.player.x >= castle_x - 2 && game_state.player.y >= castle_door_y - 4 && game_state.player.y <= castle_door_y + 20);
}
...
static void draw_gameplay_screen(Canvas* canvas) {
    ...
    if(game_state.player.weapon_active) {
        ...
        canvas_draw_line(canvas, game_state.player.x + 4, game_state.player.y + 4, weapon_end_x, weapon_end_y);
    }
    for(int i = 0; i < MAX_ENEMIES; i++) {
        if(game_state.enemies[i].active) {
            canvas_draw_circle(canvas, game_state.enemies[i].x + 4, game_state.enemies[i].y + 4, 4);
        }
    }
    canvas_set_font(canvas, FontSecondary);
    char health_str[12];
...
static void app_draw_callback(Canvas* canvas, void* ctx) {
    ...
    switch(game_state.state) {
        case GameStateTitle: draw_title_screen(canvas); break;
        case GameStateGameplay: draw_gameplay_screen(canvas); break;
        case GameStateGameOver: draw_game_over_screen(canvas); break;
        case GameStateWin: draw_win_screen(canvas); break;
        case GameStateUpgrade: draw_upgrade_screen(canvas); break;
        case GameStateBossFight: draw_boss_fight_screen(canvas); break;
    }
}
...
static void check_weapon_collisions() {
    ...
    for (int i = 0; i < MAX_ENEMIES; i++) {
        if (game_state.enemies[i].active) {
            if (line_circle_collision(game_state.player.x + 4, game_state.player.y + 4, weapon_end_x, weapon_end_y, game_state.enemies[i].x + 4, game_state.enemies[i].y + 4, 4)) {
...
static void check_boss_weapon_collision() {
    ...
    int weapon_end_x, weapon_end_y;
    get_weapon_end_point(&weapon_end_x, &weapon_end_y);
    if(line_circle_collision(game_state.player.x + 4, game_state.player.y + 4, weapon_end_x, weapon_end_y, game_state.boss.x + 4, game_state.boss.y + 4, 8)) {
        ...
        if(furi_message_queue_get(event_queue, &event, 10) == FuriStatusOk) {
            if(event.type == InputTypePress) {
                if(event.key == InputKeyBack) {
                    ...
                    switch(game_state.state) {
                        case GameStateTitle: handle_title_screen(&event); break;
                        case GameStateGameOver: handle_game_over_screen(&event); break;
