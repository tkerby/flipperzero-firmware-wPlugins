...
print(
    "Arduboy Uploader GUI/FX activator/flasher"
    + VERSION
    + VERSION_DATE
    + " by Mr.Blinky running\n"
)
...
if len(sys.argv) > 1:
    if sys.argv[1].lower() == "uploader":
        fxActivator = False
    elif sys.argv[1].lower() == "activator":
        fxActivator = True
    ...
    for device in devicelist:
        for vidpid in compatibledevices:
            if vidpid in device[2]:
                ...
                if verbose:
                    addLog("Found {} at port {}".format(device[1], port))
                return port
    if verbose:
        addLogRed(
            "Arduboy not found! Please Check Arduboy is switched on or try a different USB cable."
        )
...
def getJedecID():
    ...
    if (
        jedec_id2 != jedec_id
        or jedec_id == b"\x00\x00\x00"
        or jedec_id == b"\xFF\xFF\xFF"
    ):
...
def uploadHexfile():
    ...
    if bootloader.read(2) == b"10":  # original caterina 1.0 bootloader
        ...
        if (ord(bootloader.read(1)) & 0x10 != 0) and caterina_overwrite:
            addLogRed(
                "This upload will most likely corrupt the catarina bootloader. Upload aborted."
            )
            bootloaderExit()
            enableButtons()
            return
    ...
    ## Flash ##
    addLog(
        "Flashing {} bytes. ({} flash pages)".format(
            flash_page_count * 128, flash_page_count
        )
    )
    for i in range(256):
        if flash_page_used[i]:
    ...
    ## Verify ##
    addLog(
        "Verifying {} bytes. ({} flash pages)".format(
            flash_page_count * 128, flash_page_count
        )
    )
    for i in range(256):
        if flash_page_used[i]:
            ...
            if bootloader.read(128) != flash_data[i * 128 : (i + 1) * 128]:
                addLogRed(
                    "Verify failed at address {:04X}. Upload unsuccessful.\n".format(
                        i * 128
                    )
                )
                bootloaderExit()
...
def flashImage():
    ...
    if getVersion() < 13:
        addLogRed(
            "Bootloader does not support writing to flash. Write aborted!\nPlease update bootloader first."
        )
        enableButtons()
        return
    ...
    if len(flashdata) % BLOCKSIZE:
        ...
        bootloader.read(1)
        bootloader.write(
            bytearray([ord("g"), (blocklen >> 8) & 0xFF, blocklen & 0xFF, ord("C")])
        )
    ...
    log.insert(
        END, "writing {} blocks/{}KB to flash".format(blocks, len(flashdata) // 1024)
    )
    if verifyAfterWrite:
        addLog(" with verify")
    else:
    ...
    for block in range(blocks):
        ...
        bootloader.read(1)
        bootloader.write(
            bytearray([ord("B"), (blocklen >> 8) & 0xFF, blocklen & 0xFF, ord("C")])
        )
        ...
        if verifyAfterWrite:
            ...
            if (
                bootloader.read(blocklen)
                != flashdata[block * BLOCKSIZE : block * BLOCKSIZE + blocklen]
            ):
    ...
    if not fxFlasher:
        addLog(
            "Press LEFT or RIGHT on Arduboy to browse through the game catagogories."
        )
        addLog(
...
def viewEEPROM():
    ...
    for addr in range(0, 1024, 16):
        ...
        for i in range(16):
            if eepromdata[addr + i] < 32:
                s += "."
            else:
                s += eepromdata[addr + i : addr + i + 1].decode("cp1252")
...
def eraseEEPROM():
    if (
        messagebox.showwarning(
            "Erase EEPROM", "Are you sure you want to erase the EEPROM?", type="yesno"
        )
        != "yes"
    ):
        return
...
def selectHexFile():
    ...
    selectHexFilename = filedialog.askopenfilename(
        initialdir=selectAppInitialDir,
...
def selectFlashFile():
    ...
    selectFlashFilename = filedialog.askopenfilename(
        initialdir=selectFlashInitialDir,
        title="Select Flash image",
...
def selectDevDataFile():
    ...
    selectDevDataFilename = filedialog.askopenfilename(
        initialdir=selectDevDataInitialDir,
        title="Select development data",
        filetypes=(("bin files", "*.bin"), ("all files", "*.*")),
    )
...
def selectEEPROMbackupFile():
    ...
    filename = filedialog.asksaveasfilename(
        initialdir=selectEEPROMinitialDir,
        initialfile=time.strftime("eeprom-backup-%Y%m%d-%H%M%S.eep", time.localtime()),
        title="Select EEPROM backup file",
        filetypes=(
            ("eep files", "*.eep"),
            ("bin files", "*.bin"),
            ("all files", "*.*"),
...
def selectEEPROMrestoreFile():
    ...
    filename = filedialog.askopenfilename(
        initialdir=selectEEPROMinitialDir,
        title="Select EEPROM restore file",
        filetypes=(
            ("eep files", "*.eep"),
            ("bin files", "*.bin"),
            ("all files", "*.*"),
...
if not fxFlasher:
    ...
    hexButton = Button(
        appFrame, text="Upload Hex file", width=23, command=uploadHexfileThread
    )
    hexButton.pack(side=LEFT)
    hexDirButton = Button(appFrame, text="...", width=2, command=selectHexFile).pack(
        side=RIGHT
    )
    hexEntry = Entry(appFrame, textvariable=appFilename).pack(
...
flashButton = Button(
    flashFrame, text="Upload Flash image", width=23, command=flashImageThread
)
flashButton.pack(side=LEFT)
flashDirButton = Button(flashFrame, text="...", width=2, command=selectFlashFile).pack(
    side=RIGHT
)
flashEntry = Entry(flashFrame, textvariable=flashFilename).pack(
    side=LEFT, expand=True, fill=X
)
...
if not fxFlasher and not fxActivator:
    ...
    devDataButton = Button(
        devDataFrame,
        text="Upload development data",
        width=23,
        command=devDataImageThread,
    )
    devDataButton.pack(side=LEFT)
    devDataDirButton = Button(
        devDataFrame, text="...", width=2, command=selectDevDataFile
    ).pack(side=RIGHT)
    devDataEntry = Entry(devDataFrame, textvariable=devDataFilename).pack(
        side=LEFT, expand=True, fill=X
    )
...
if not fxFlasher:
    filemenu.add_command(
        label="Select Hex file",
        underline=7,
        accelerator="Ctrl + H",
        command=selectHexFile,
    )
filemenu.add_command(
    label="Select Flash image",
    underline=7,
    accelerator="Ctrl + F",
    command=selectFlashFile,
)
if not fxFlasher and not fxActivator:
    filemenu.add_command(
        label="Select development data",
        underline=0,
        accelerator="Ctrl + D",
        command=selectDevDataFile,
    )
if not (fxActivator or fxFlasher):
    ...
    filemenu.add_command(
        ...
        underline=0,
        accelerator="Ctrl + B",
        command=backupEEPROMThread,
    )
    filemenu.add_command(
        label="Restore EEPROM",
        underline=0,
        accelerator="Ctrl + R",
        command=restoreEEPROMThread,
    )
    filemenu.add_command(
        label="View EEPROM",
        underline=0,
        accelerator="Ctrl + L",
        command=viewEEPROMThread,
    )
    filemenu.add_command(label="Erase EEPROM", underline=0, command=eraseEEPROMThread)
    filemenu.add_separator()
filemenu.add_command(
    label="Exit", underline=1, accelerator="Ctrl + X", command=root.quit
)
optionmenu = Menu(mainmenu, tearoff=0)
if not fxFlasher:
    optionmenu.add_checkbutton(
        label="Verify Hex file after upload",
        onvalue=True,
        offvalue=False,
        variable=appVerify,
    )
    appVerify.set(True)
optionmenu.add_checkbutton(
    label="Verify flash data", onvalue=True, offvalue=False, variable=flashVerify
)
# flashVerify.set(True)
optionmenu.add_checkbutton(
    label="Apply SSD1309 display patch",
    onvalue=True,
    offvalue=False,
    variable=applySsd1309patch,
)
