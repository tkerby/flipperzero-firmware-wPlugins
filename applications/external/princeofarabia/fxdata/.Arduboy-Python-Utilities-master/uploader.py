...
try:
    ...
    for file in zip.namelist():
        if file.lower().endswith(".hex"):
            ...
            print(
                '\nLoading "{}" from Arduboy file "{}"'.format(
...
if os.path.basename(sys.argv[0]).lower().find("micro") >= 0:
    for i in range(0, 32768 - 4, 2):
        if flash_data[i : i + 2] == b"\x28\x98":  # RXLED1
            ...
            flash_data[i : i + 4] == b"\x81\xef\x85\xb9"
        ):  # Arduboy core init RXLED port
            flash_data[i] = 0x80
        elif (
            flash_data[i : i + 4] == b"\x84\xe2\x8b\xb9"
        ):  # Arduboy core init TXLED port
            flash_data[i + 1] = 0xE0
...
for i in range(256):
    if flash_page_used[i]:
        ...
        if bootloader.read(128) != flash_data[i * 128 : (i + 1) * 128]:
            print(
                "\nVerify failed at address {:04X}. Upload unsuccessful.".format(
                    i * 128
                )
            )
            bootloaderExit()
