...
classic_patch = [
    ...
    (0x11A0, "Activate display reset patch", [0x5F, 0x98], [0x59, 0x98]),
    (0x11B2, "Deactivate display reset patch", [0x5F, 0x9A], [0x59, 0x9A]),
    (
        0x2882,
        "Sound channel 1 patch",
        [0x30, 0x91, 0x5A, 0x06, 0x63, 0x23],
        [0x0C, 0x94, 0x48, 0x00, 0x60, 0x74],
    ),
    (
        0x0090,
        "Sound channel 2 patch (extension in int vector area)",
        [0x0C, 0x94, 0x6D, 0x06, 0x0C, 0x94, 0x6D, 0x06, 0x0C, 0x94, 0x6D, 0x06],
        [0x67, 0xFD, 0x5F, 0x9A, 0x67, 0xFF, 0x5F, 0x98, 0x0C, 0x94, 0x43, 0x14],
    ),
    (0x00B7, "Display contrast patch", [0x81, 0xFF], [0x81, CONTRAST]),
]
...
if len(sys.argv) != 2:
    print(
        "Usage: {} game.hex | starduino-turbo.hex\n".format(
            os.path.basename(sys.argv[0])
        )
    )
    print(
        "This script creates a patched version of Starduino hex file for use on Homemade"
    )
    print(
        "Arduboys that use a Pro Micro with alternate wiring and a SSD1306 or SSD1309"
    )
    print("displays. The original Starduino hex files can be downloaded from:\n")
    print(
        "https://rv6502.ca/post/2019/01/02/starduino-for-arduboy-3d-gaming-inside-28kb/"
    )
    delayedExit()
...
# patch data
if checkPatch(classic_patch):
    applyPatch(classic_patch)
elif checkPatch(turbo_patch):
    applyPatch(turbo_patch)
else:
