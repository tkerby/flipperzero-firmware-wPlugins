    ...
    for rcd in records:
        ...
        if rcd[0] == ":":
            ...
            if (rcd_typ == 0) and (rcd_len > 0):
                ...
                if checksum != 0:
                    print(
                        "Error: Hex file '{}' contains errors.".format(
...
def checkNoUSB(program):
    if len(program) < 256:
        return True
    vector_10 = (program[0x2A] << 1) | (
        program[0x2B] << 9
    )  # USB General interrupt vector
    if program[vector_10 : vector_10 + 4] == b"\x0c\x94\x00\x00":
        return True  # bad  interrupts jumps to 0
    return False
...
for filenumber in range(1, len(sys.argv)):  # support multiple files
    ...
    if checkNoUSB(program):
        print("!!! No USB support !!! in {}".format(filename))
    else:
        print("USB support found in {}".format(filename))
