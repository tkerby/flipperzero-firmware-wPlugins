...
def ledControl(
    b,
):  # Bit 7 set: OLED display off (v1.1) Disable bootloader menu buttons (v1.3+)
    bootloader.write(
        bytearray([ord("x"), b])
    )  # Bit 6 set: RGB Breathing function off (v1.1 to v1.3 only)
...
def readButtons():
    bootloader.write(b"v")
    buttons = ((ord(bootloader.read(1)) - ord("1") << 2)) | (
        (ord(bootloader.read(1)) - ord("A") << 4)
    )
    return buttons
...
def usage():
    ...
    print()
    print(
        "Displays a 128x64 pixel image on Arduboy's display in bootloader mode until a "
    )
