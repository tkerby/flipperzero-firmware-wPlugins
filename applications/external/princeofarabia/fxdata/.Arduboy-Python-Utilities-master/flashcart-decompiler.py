...
def DecompileHexFileData(byteData):
    ...
    if byteLength % 16 > 0:
        ...
        hexLine = (
            ":"
            + IntToHex(byteLength % 16, 2)
            + IntToHex(byteLength - byteLength % 16, 4)
            + "00"
        )
...
def GetProgramSections(fullBinaryData):
    ...
    while currentHeaderIndex < len(fullBinaryData) - 1:
        if (
            HEADER_START_BYTES
            != fullBinaryData[
                currentHeaderIndex : currentHeaderIndex + len(HEADER_START_BYTES)
            ]
        ):
            break
        ...
        slotByteSize = (
            (
                (fullBinaryData[currentHeaderIndex + SLOT_SIZE_HEADER_INDEX] << 8)
                + fullBinaryData[currentHeaderIndex + SLOT_SIZE_HEADER_INDEX + 1]
            )
            * 2
            * 128
        )
        ...
        titleImageData = fullBinaryData[
            currentHeaderIndex
            + HEADER_LENGTH : currentHeaderIndex
            + HEADER_LENGTH
            + TITLE_IMAGE_LENGTH
        ]
        programData = fullBinaryData[
            currentHeaderIndex
            + HEADER_LENGTH
            + TITLE_IMAGE_LENGTH : currentHeaderIndex
            + HEADER_LENGTH
            + TITLE_IMAGE_LENGTH
            + fullBinaryData[currentHeaderIndex + PROGRAM_SIZE_HEADER_INDEX] * 128
        ]
        datafileData = fullBinaryData[
            currentHeaderIndex
            + HEADER_LENGTH
            + TITLE_IMAGE_LENGTH
            + fullBinaryData[currentHeaderIndex + PROGRAM_SIZE_HEADER_INDEX]
            * 128 : currentHeaderIndex
            + slotByteSize
        ]
        ...
        programDataArray.append(
            [programCategory, programIndex, titleImageData, programData, datafileData]
        )
...
def DecompileProgramParts(programSections):
    ...
    for programAbsoluteIndex in range(0, len(programSections)):
        ...
        decompiledProgramData.append(
            [
                programData[CATEGORY_INDEX],
                programData[PROGRAM_INDEX],
                DecompileTitleScreenData(programData[TITLE_IMAGE_INDEX]),
                DecompileHexFileData(programData[PROGRAM_HEXFILE_INDEX]),
                programData[DATAFILE_INDEX],
            ]
        )

        print(
            "{:7} {:7} {:9} {:6} {:13}".format(
                str(round(100 * programAbsoluteIndex / (len(programSections) - 1), 2))
                + "%",
                programAbsoluteIndex,
                programData[CATEGORY_INDEX],
                programData[PROGRAM_INDEX],
                len(programData[PROGRAM_HEXFILE_INDEX]),
            )
        )
...
def WriteDecompiledProgramData(decompiledProgramData):
    ...
    for programDataIndex in range(0, len(decompiledProgramData)):
        ...
        programDirPath = os.path.join(
            os.path.join(decompiledPath, categoryIndex), programIndex
        )
        ...
        shouldIncludeHexFile = (
            len(decompiledProgramData[programDataIndex][PROGRAM_HEXFILE_INDEX]) > 0
        )
        if shouldIncludeHexFile:
            hexFilePath = os.path.join(programDirPath, programIndex + ".hex")
            hexFile = open(hexFilePath, "w")
            hexFile.write(
                decompiledProgramData[programDataIndex][PROGRAM_HEXFILE_INDEX]
            )
            hexFile.close()

        shouldIncludeDataFile = (
            len(decompiledProgramData[programDataIndex][DATAFILE_INDEX]) > 0
        )
        ...
        indexCSVString += (
            os.path.join(
                os.path.join(categoryIndex, programIndex), programIndex + ".png"
            )
            + ";"
        )
        indexCSVString += (
            os.path.join(
                os.path.join(categoryIndex, programIndex), programIndex + ".hex"
            )
            if shouldIncludeHexFile
            else ""
        ) + ";"
