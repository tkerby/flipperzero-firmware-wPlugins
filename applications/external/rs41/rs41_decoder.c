...
static void
    ecef_to_lla(float x, float y, float z, float* lat_out, float* lon_out, float* alt_out) {
...
static int parse_block(const uint8_t* buf, int offset, int total, Rs41Frame* out) {
    ...
    if(crc16(data, block_len) != expected) return 2 + (int)block_len + 2; /* bad CRC — skip */
    ...
    case RS41_BLOCK_STATUS:
        ...
        if(block_len >= 40) {
            ...
            if(calfr >= 3 && calfr <= 7) {
                ...
                if(!s_cal_valid && (s_cal_ok & 0x0Fu) == 0x0Fu) assemble_cal();
                if(!s_pres_cal_valid && (s_cal_ok & 0x18u) == 0x18u) assemble_pres_cal();
            }
        }
        break;
    ...
    case RS41_BLOCK_GPS:
        ...
        if(block_len >= 21) {
            ...
            ecef_to_lla(
                ...
                &alt);
            out->sats = data[18];
            if(alt > -1000.0f && alt < 50000.0f && (fabsf(lat) > 0.001f || fabsf(lon) > 0.001f)) {
                out->lat = lat;
                out->lon = lon;
                out->alt = alt;
                out->has_gps = true;
    ...
    case RS41_BLOCK_PTU:
        ...
        if(block_len >= 9 && s_cal_valid) {
            ...
            if(fabsf(dRf) > 1.0f && fabsf(df) > 1.0f) {
                ...
                out->temp = (s_co1[0] + s_co1[1] * R + s_co1[2] * R * R + s_calT1[1]) *
                            (1.0f + s_calT1[2]);
            }
        }
        if(block_len >= 18 && s_pres_cal_valid) {
            uint32_t m3 = (uint32_t)data[9] | ((uint32_t)data[10] << 8) |
                          ((uint32_t)data[11] << 16);
            uint32_t m4 = (uint32_t)data[12] | ((uint32_t)data[13] << 8) |
                          ((uint32_t)data[14] << 16);
            uint32_t m5 = (uint32_t)data[15] | ((uint32_t)data[16] << 8) |
                          ((uint32_t)data[17] << 16);
            float fp = (float)m3;
            float fp1 = (float)m4;
            float fp2 = (float)m5;
            ...
            if(fabsf(dFp) > 1.0f && fabsf(dfp) > 1.0f) {
                ...
                if(P > 10.0f && P < 1200.0f) out->pres = P;
            }
        }
        break;
