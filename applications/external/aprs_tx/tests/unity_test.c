...
int main(void) {
    ...
    sh = 1;
    sh <<= 7;
    TEST_ASSERT_EQUAL_HEX8(0x80, sh);
    sh >>= 3;
    TEST_ASSERT_EQUAL_HEX8(0x10, sh);
    x = (uint8_t)((0x55 << 1) ^ 0xAA);
    TEST_ASSERT_EQUAL_HEX8(0x00, x);
    x = (uint8_t)(1u << 5);
    TEST_ASSERT_EQUAL_UINT8(32, x);
    ...
    for(i = 0; i < 25; i++)
        want_ff[i] = (uint8_t)(stuff_ff[i] == '1');
