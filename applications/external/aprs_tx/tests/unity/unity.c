...
void UnityPrintFloat(const UNITY_DOUBLE input_number) {
    ...
    if(number == 0.0f) {
        ...
        decimals = ((exponent <= 0) && (exponent >= -(sig_digits + 3))) ? (-exponent) :
                                                                          (sig_digits - 1);
        exponent += decimals;
            ...
            if((label[0] == '#') && (label[1] != 0)) {
                ...
                if((label[1] & UNITY_DISPLAY_RANGE_INT) == UNITY_DISPLAY_RANGE_INT) {
                    ...
                    UnityPrintUintNumberByStyle(
                        (UNITY_UINT)Unity.CurrentDetailStackValues[c], label[1]);
                }
...
/*-----------------------------------------------*/
static int UnityIsOneArrayNull(
    ...
    while(elements--) {
        ...
        if(!UnityFloatsWithin(in_delta + current_element_delta, *ptr_expected, *ptr_actual)) {
            ...
            UNITY_PRINT_EXPECTED_AND_ACTUAL_FLOAT(
                (UNITY_DOUBLE)*ptr_expected, (UNITY_DOUBLE)*ptr_actual);
            UnityAddMsgIfSpecified(msg);
            UNITY_FAIL_AND_BAIL;
        }
        if(flags == UNITY_ARRAY_TO_ARRAY) {
