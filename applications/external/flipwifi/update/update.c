...
static bool update_last_app_update(FlipperHTTP* fhttp, bool flipper_server) {
    ...
    if(flipper_server) {
        ...
        snprintf(
            ...
            STORAGE_EXT_PATH_PREFIX "/apps_data/flip_store");
        ...
        snprintf(
            directory_path,
            sizeof(directory_path),
            STORAGE_EXT_PATH_PREFIX "/apps_data/flip_store/data/last_update_request.txt");
        ...
        snprintf(
            url,
            sizeof(url),
            "https://catalog.flipperzero.one/api/v0/0/application/%s?is_latest_release_version=true",
            BUILD_ID);
        ...
        snprintf(
            url,
            sizeof(url),
            "https://www.jblanked.com/flipper/api/app/last-updated/flip_downloader/");
        return flipper_http_request(
            fhttp, GET, url, "{\"Content-Type\":\"application/json\"}", NULL);
    }
}
...
static bool
    update_parse_last_app_update(FlipperHTTP* fhttp, DateTime* time_current, bool flipper_server) {
    if(!fhttp) {
        FURI_LOG_E(TAG, "fhttp is NULL");
        return false;
    }
    ...
    if(!flipper_server) {
        ...
        FuriString* app_data = flipper_http_load_from_file_with_limit(
            fhttp->file_path, memmgr_heap_get_max_free_block());
        if(!app_data) {
...
static bool update_get_fap_file(FlipperHTTP* fhttp, bool flipper_server) {
    ...
    if(flipper_server) {
        ...
        snprintf(
            url,
            sizeof(url),
            "https://www.jblanked.com/flipper/api/app/download/flip_downloader/");
    }
    return flipper_http_request(
