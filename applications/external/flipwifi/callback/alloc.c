    ...
    case FlipWiFiViewSubmenuCommands:
        if(!app->submenu_wifi) {
            ...
            submenu_add_item(
                app->submenu_wifi,
                "[CUSTOM]",
                FlipWiFiSubmenuIndexFastCommandStart + 0,
                callback_submenu_choices,
                app);
            submenu_add_item(
                app->submenu_wifi,
                "PING",
                FlipWiFiSubmenuIndexFastCommandStart + 1,
                callback_submenu_choices,
                app);
            submenu_add_item(
                app->submenu_wifi,
                "LIST",
                FlipWiFiSubmenuIndexFastCommandStart + 2,
                callback_submenu_choices,
                app);
            submenu_add_item(
                app->submenu_wifi,
                "IP/ADDRESS",
                FlipWiFiSubmenuIndexFastCommandStart + 3,
                callback_submenu_choices,
                app);
            submenu_add_item(
                app->submenu_wifi,
                "WIFI/IP",
                FlipWiFiSubmenuIndexFastCommandStart + 4,
                callback_submenu_choices,
                app);
            submenu_add_item(
                app->submenu_wifi,
                "WIFI/AP",
                FlipWiFiSubmenuIndexFastCommandStart + 5,
                callback_submenu_choices,
                app);
        }
        return true;
    ...
    case FlipWiFiViewTextInputSaved:
        if(!app->uart_text_input) {
            if(!easy_flipper_set_uart_text_input(
                   ...
                   app->uart_text_input_temp_buffer,
                   app->uart_text_input_buffer_size,
                   callback_text_updated_password_saved,
    ...
    case FlipWiFiViewTextInputSavedAddSSID:
        if(!app->uart_text_input) {
            if(!easy_flipper_set_uart_text_input(
                   ...
                   app->uart_text_input_temp_buffer,
                   app->uart_text_input_buffer_size,
                   callback_text_updated_add_ssid,
                   callback_to_submenu_saved,
                   &app->view_dispatcher,
                   app)) {
    ...
    case FlipWiFiSubmenuIndexFastCommandStart:
        if(!app->uart_text_input) {
            if(!easy_flipper_set_uart_text_input(
                   ...
                   app->uart_text_input_temp_buffer,
                   app->uart_text_input_buffer_size,
                   callback_custom_command_updated,
                   callback_to_submenu_saved,
                   &app->view_dispatcher,
                   app)) {
    ...
    case FlipWiFiViewWiFiScan:
        if(!app->view_wifi) {
            if(!easy_flipper_set_view(
                   &app->view_wifi,
                   FlipWiFiViewGeneric,
                   callback_view_draw_callback_scan,
                   callback_view_input_callback_scan,
                   callback_to_submenu_scan,
                   &app->view_dispatcher,
                   app)) {
                return false;
    ...
    case FlipWiFiViewWiFiSaved:
        if(!app->view_wifi) {
            if(!easy_flipper_set_view(
                   &app->view_wifi,
                   FlipWiFiViewGeneric,
                   callback_view_draw_callback_saved,
                   callback_view_input_callback_saved,
                   callback_to_submenu_saved,
                   &app->view_dispatcher,
                   app)) {
                return false;
    ...
    case FlipWiFiViewWiFiDeauth:
        if(!app->view_wifi) {
            if(!easy_flipper_set_view(
                   &app->view_wifi,
                   FlipWiFiViewGeneric,
                   callback_view_draw_callback_deauth,
                   callback_view_input_callback_deauth,
                   callback_to_submenu_main,
                   &app->view_dispatcher,
                   app)) {
                return false;
