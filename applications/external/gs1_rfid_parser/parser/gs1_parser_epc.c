...
static ParsingResult parse_gtin_normal(const UhfEpcTagData* epc_data, ParsedTagInformation* tag_info) {
    ...
    if(epc_data->epc_size < min_byte_count){
        FURI_LOG_E(LOG_TAG, "Too few bytes in EPC. Expected at least %d bytes, but got %d bytes.", min_byte_count, epc_data->epc_size);
    ...
    uint64_t company_prefix = read_next_n_bits(epc_data->epc_data, bit_index, company_prefix_length);
    snprintf_with_padding(company_prefix_str, COMPANY_PREFIX_MAX_LENGTH, company_prefix, 12 - partition);
    ...
    uint64_t item_reference = read_next_n_bits(epc_data->epc_data, bit_index + company_prefix_length, 44 - company_prefix_length);
    snprintf_with_padding(item_reference_str, ITEM_REFERENCE_MAX_LENGTH, item_reference, 1 + partition);
    ...
    for(uint8_t i = 0; i < partition; i++){
        ...
        checksum = (checksum + ((item_reference_str[i + 1] - '0') * (gtin_index & 0x01 ? 1 : 3))) % 10;
        gtin_index++;
    }
...
static ParsingResult parse_gtin_plus(const UhfEpcTagData* epc_data, ParsedTagInformation* tag_info) {
    FURI_LOG_T(LOG_TAG, __func__);
    ...
    if(epc_data->epc_data[0] == DSGTIN_PLUS_HEADER) {
        ...
        switch(read_next_n_bits(epc_data->epc_data, 12, 4)) {
            case 0x00: tag_info->ai_list[2].ai_identifier = 0x11; break;
            case 0x01: tag_info->ai_list[2].ai_identifier = 0x13; break;
            case 0x02: tag_info->ai_list[2].ai_identifier = 0x15; break;
            case 0x03: tag_info->ai_list[2].ai_identifier = 0x16; break;
            case 0x04: tag_info->ai_list[2].ai_identifier = 0x17; break;
            case 0x05: tag_info->ai_list[2].ai_identifier = 0x7006; break;
            case 0x06: tag_info->ai_list[2].ai_identifier = 0x7007; break;
            default:
                return PARSING_MALFORMED_DATA;
        }
    ...
    tag_info->ai_list[0].ai_identifier = 0x01;
    parse_fixed_length_numeric(epc_data->epc_data, gtin_offset, GTIN_LENGTH, tag_info->ai_list[0].ai_value);
    
    FURI_LOG_I(LOG_TAG, "GTIN: %s", tag_info->ai_list[0].ai_value);
    
    int8_t serial_bit_count = parse_variable_length_alphanumeric(epc_data->epc_data, serial_offset, 5, tag_info->ai_list[1].ai_value);
