    ...
    while(ui->scan_scene->thread_state == ScanThreadRunning) {
        ...
        if(result > 0){
            ...
            if(ui->epc_data->pc & UHF_EPC_USER_MEM_FLAG){
                ...
                if(result < 0){
                    FURI_LOG_E(LOG_TAG, "Error returned by UHF reader while reading user memory: %ld", result);
                }
            }
            
            scene_manager_next_scene(ui->scene_manager, ParseDisplay);
        }else{
            FURI_LOG_E(LOG_TAG, "Error returned by UHF reader while polling for tags: %ld", result);
        }
...
void scan_scene_on_enter(void* context) {
    ...
    widget_add_string_element(ui->scan_scene->scan_widget, 0, 0, AlignLeft, AlignTop, FontPrimary, "Looking for UHF Tags");
