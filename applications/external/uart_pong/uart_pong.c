...
static void game_tick(void* ctx) {
    ...
    if(app->controls.up && app->me.y > 0)
        app->me.y--;

    if(app->controls.down && app->me.y < 63-12)
        app->me.y++;
        
    if(prev_y != app->me.y)
        furi_hal_serial_tx(app->handle, &app->me.y, 1);

    app->ball.y += app->ball.dy;
    app->ball.x += app->ball.dx;

    if(app->ball.y < 1 || app->ball.y > 63-3)
        app->ball.dy *= -1;

    if(app->ball.x < 1 || app->ball.x > 127-3)
        app->ball.dx *= -1;
