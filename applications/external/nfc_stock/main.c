...
static void nfc_stock_scene_menu_on_enter(void* context) {
    ...
    storage_helper_get_active_db(app->active_db_path, sizeof(app->active_db_path));
    nfc_stock_ui_apply_main_menu(app->ui, app->scene_manager, app->active_db_path);
...
static bool nfc_stock_scene_menu_on_event(void* context, SceneManagerEvent event) {
    UNUSED(context);
    UNUSED(event);
    return false;
}

static bool nfc_stock_scene_scan_on_event_main(void* context, SceneManagerEvent event) {
    return nfc_stock_scene_scan_on_event(context, event);
}

static bool nfc_stock_scene_stub_on_event(void* context, SceneManagerEvent event) {
    UNUSED(context);
    UNUSED(event);
    return false;
}

static void nfc_stock_scene_stub_on_exit(void* context) {
    UNUSED(context);
}
...
int32_t nfc_stock_app(void* p) {
    ...
    view_dispatcher_set_custom_event_callback(
        app->view_dispatcher, nfc_stock_custom_event_callback);
