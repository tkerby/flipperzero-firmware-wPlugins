...
void blackhat_console_output_handle_rx_data_cb(uint8_t* buf, size_t len, void* context) {
    furi_assert(context);
...
void blackhat_scene_console_output_on_enter(void* context) {
    ...
    scene_manager_set_scene_state(app->scene_manager, BlackhatSceneConsoleOutput, 0);

    view_dispatcher_switch_to_view(app->view_dispatcher, BlackhatAppViewConsoleOutput);

    // Register callback to receive data
    blackhat_uart_set_handle_rx_data_cb(app->uart, blackhat_console_output_handle_rx_data_cb);

    blackhat_uart_tx(app->uart, app->text_store, strlen(app->text_store));
}

bool blackhat_scene_console_output_on_event(void* context, SceneManagerEvent event) {
    UNUSED(context);
