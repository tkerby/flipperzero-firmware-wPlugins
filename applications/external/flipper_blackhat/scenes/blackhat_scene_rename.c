...
void blackhat_text_input_callback(void* context) {
    ...
    view_dispatcher_send_custom_event(app->view_dispatcher, BlackhatEventTextInput);
}
...
void blackhat_scene_rename_on_enter(void* context) {
    ...
    while(app->script_text[i++]) {
        if(app->script_text[i] == '\n') {
            if(!str_start)
                ...
                app->script_text[i] = '\0';
                memcpy(app->text_input_ch, &app->script_text[str_start], i - str_start - 1);
                break;
            }
        }
    }
    ...
    view_dispatcher_switch_to_view(app->view_dispatcher, BlackhatAppViewTextInput);
}
