...
BlackhatApp* blackhat_app_alloc() {
    ...
    view_dispatcher_add_view(app->view_dispatcher, BlackhatAppViewTui, app->tui_view);
...
void blackhat_app_free(BlackhatApp* app) {
    ...
    for(int i = 0; i < app->num_scripts; i++) {
        if(app->cmd[i]) free(app->cmd[i]);
    }

    // Views
    view_dispatcher_remove_view(app->view_dispatcher, BlackhatAppViewVarItemList);

    view_dispatcher_remove_view(app->view_dispatcher, BlackhatAppViewScriptItemList);
