...
static bool alloc_variable_item_list(WebCrawlerApp* app, uint32_t view) {
    ...
    case WebCrawlerViewVariableItemListWifi:
        if(!easy_flipper_set_variable_item_list(
               ...
               WebCrawlerViewVariableItemList,
        ...
        if(!app->ssid_item) {
            app->ssid_item =
                variable_item_list_add(app->variable_item_list, "SSID", 0, NULL, NULL); // index 0
        ...
        if(!app->password_item) {
            app->password_item = variable_item_list_add(
                app->variable_item_list, "Password", 0, NULL, NULL); // index 1
    ...
    case WebCrawlerViewVariableItemListFile:
        ...
        if(!app->file_read_item) {
            app->file_read_item = variable_item_list_add(
                app->variable_item_list, "Read File", 0, NULL, NULL); // index 0
        ...
        if(!app->file_type_item) {
            app->file_type_item = variable_item_list_add(
                app->variable_item_list, "Set File Type", 0, NULL, NULL); // index 1
        ...
        if(!app->file_rename_item) {
            app->file_rename_item = variable_item_list_add(
                app->variable_item_list, "Rename File", 0, NULL, NULL); // index 2
        ...
        if(!app->file_delete_item) {
            app->file_delete_item = variable_item_list_add(
                app->variable_item_list, "Delete File", 0, NULL, NULL); // index 3
    ...
    case WebCrawlerViewVariableItemListRequest:
        ...
        if(!app->path_item) {
            app->path_item =
                variable_item_list_add(app->variable_item_list, "Path", 0, NULL, NULL);
        ...
        if(!app->http_method_item) {
            app->http_method_item = variable_item_list_add(
                app->variable_item_list, "HTTP Method", 6, web_crawler_http_method_change, app);
        ...
        if(!app->headers_item) {
            app->headers_item =
                variable_item_list_add(app->variable_item_list, "Headers", 0, NULL, NULL);
            variable_item_set_current_value_text(app->headers_item, ""); // Initialize
        }
        if(!app->payload_item) {
            app->payload_item =
                variable_item_list_add(app->variable_item_list, "Payload", 0, NULL, NULL);
        ...
        if(settings_loaded) {
            ...
            variable_item_set_current_value_index(
                ...
                strstr(http_method, "GET") != NULL      ? 0 :
                strstr(http_method, "POST") != NULL     ? 1 :
...
static bool alloc_text_input(WebCrawlerApp* app, uint32_t view) {
    ...
    case WebCrawlerViewTextInputSSID:
        ...
        if(!easy_flipper_set_buffer(&app->temp_buffer_ssid, app->temp_buffer_size_ssid) ||
           !easy_flipper_set_buffer(&app->ssid, app->temp_buffer_size_ssid)) {
            return false;
        }
        if(!easy_flipper_set_uart_text_input(
               &app->uart_text_input,
    ...
    case WebCrawlerViewTextInputPassword:
        ...
        if(!easy_flipper_set_buffer(&app->temp_buffer_password, app->temp_buffer_size_password) ||
           !easy_flipper_set_buffer(&app->password, app->temp_buffer_size_password)) {
            return false;
        }
        if(!easy_flipper_set_uart_text_input(
               &app->uart_text_input,
        ...
        if(load_char("password", app->password, app->temp_buffer_size_password)) {
            snprintf(
                app->temp_buffer_password, app->temp_buffer_size_password, "%s", app->password);
        }
        break;
    case WebCrawlerViewTextInputFileType:
        ...
        if(!easy_flipper_set_buffer(&app->temp_buffer_file_type, app->temp_buffer_size_file_type) ||
           !easy_flipper_set_buffer(&app->file_type, app->temp_buffer_size_file_type)) {
            return false;
        }
        if(!easy_flipper_set_uart_text_input(
               &app->uart_text_input,
        ...
        if(load_char("file_type", app->file_type, app->temp_buffer_size_file_type)) {
            snprintf(
                app->temp_buffer_file_type, app->temp_buffer_size_file_type, "%s", app->file_type);
        }
        break;
    case WebCrawlerViewTextInputFileRename:
        app->temp_buffer_size_file_rename = 128;
        if(!easy_flipper_set_buffer(
               &app->temp_buffer_file_rename, app->temp_buffer_size_file_rename) ||
           !easy_flipper_set_buffer(&app->file_rename, app->temp_buffer_size_file_rename)) {
            return false;
        }
        if(!easy_flipper_set_uart_text_input(
               &app->uart_text_input,
    ...
    case WebCrawlerViewTextInputPayload:
        ...
        if(!easy_flipper_set_buffer(&app->temp_buffer_payload, app->temp_buffer_size_payload) ||
           !easy_flipper_set_buffer(&app->payload, app->temp_buffer_size_payload)) {
            return false;
        }
        if(!easy_flipper_set_uart_text_input(
               &app->uart_text_input,
...
static void save_simply() {
    ...
    if(load_char("path", path, 128) && load_char("ssid", ssid, 64) &&
       load_char("password", password, 64) && load_char("file_rename", file_rename, 128) &&
       load_char("file_type", file_type, 16) && load_char("http_method", http_method, 16) &&
       load_char("headers", headers, 256) && load_char("payload", payload, 256)) {
...
static bool web_crawler_fetch(DataLoaderModel* model) {
    ...
    if(strstr(http_method, "GET") != NULL) {
        ...
        return flipper_http_get_request_bytes(
            model->fhttp, url, "{\"Content-Type\": \"application/octet-stream\"}");
    } else // BROWSE
    {
        model->fhttp->save_received_data = false;
        model->fhttp->is_bytes_request = true;
...
void web_crawler_set_file_rename_update(void* context) {
    ...
    snprintf(
        app->file_rename, app->temp_buffer_size_file_rename, "%s", app->temp_buffer_file_rename);
    if(app->file_rename_item) {
...
void web_crawler_setting_item_path_clicked(void* context, uint32_t index) {
    ...
    view_set_previous_callback(
        uart_text_input_get_view(app->uart_text_input), web_crawler_back_to_request_callback);
...
void web_crawler_setting_item_ssid_clicked(void* context, uint32_t index) {
    ...
    // Set the previous callback to return to Configure screen
    view_set_previous_callback(
        uart_text_input_get_view(app->uart_text_input), web_crawler_back_to_wifi_callback);
...
void web_crawler_setting_item_file_type_clicked(void* context, uint32_t index) {
    ...
    // Set the previous callback to return to Configure screen
    view_set_previous_callback(
        uart_text_input_get_view(app->uart_text_input), web_crawler_back_to_file_callback);
...
void web_crawler_setting_item_file_delete_clicked(void* context, uint32_t index) {
    ...
    // Set the previous callback to return to Configure screen
    view_set_previous_callback(widget_get_view(app->widget), web_crawler_back_to_file_callback);
...
void web_crawler_setting_item_file_read_clicked(void* context, uint32_t index) {
    ...
    if(load_char("file_rename", file_rename, 128) && load_char("file_type", file_type, 16)) {
        snprintf(
            file_path, sizeof(file_path), "%s%s%s", RECEIVED_DATA_PATH, file_rename, file_type);
    } else {
        snprintf(
            file_path, sizeof(file_path), "%s%s%s", RECEIVED_DATA_PATH, "received_data", ".txt");
    }
    ...
    if(received_data == NULL) {
        ...
        if(app->widget) {
            widget_add_text_scroll_element(app->widget, 0, 0, 128, 64, "File is empty.");
            view_dispatcher_switch_to_view(app->view_dispatcher, WebCrawlerViewWidget);
        }
        return;
    }
    widget_add_text_scroll_element(
        app->widget, 0, 0, 128, 64, furi_string_get_cstr(received_data));
        ...
        if(loader_model->fhttp->state == IDLE && loader_model->fhttp->last_response != NULL) {
            if(strstr(loader_model->fhttp->last_response, "[PONG]") != NULL) {
                ...
                FURI_LOG_DEV(
                    TAG,
                    "SUCCESS received. %s",
                    loader_model->fhttp->last_response ? loader_model->fhttp->last_response :
                                                         "NULL");
            ...
            FURI_LOG_DEV(
                ...
                loader_model->fhttp->state,
                loader_model->fhttp->last_response ? loader_model->fhttp->last_response : "NULL");
            DEV_CRASH();
        }
        ...
        with_view_model(
            ...
            DataLoaderModel * model,
            {
                ...
                FURI_LOG_DEV(
                    TAG,
                    "Parsed data: %s\r\ntext: %s",
                    model->fhttp->last_response ? model->fhttp->last_response : "NULL",
        ...
        with_view_model(
            ...
            DataLoaderModel * model,
            {
                if(++model->request_index < model->request_count) {
                    ...
                    web_crawler_widget_set_text(
                        model->data_text != NULL ? model->data_text : "", &app->widget_result);
        ...
        DataLoaderModel * model,
        {
            ...
            if(model->timer == NULL) {
                model->timer = furi_timer_alloc(
                    web_crawler_loader_timer_callback, FuriTimerTypePeriodic, app);
            }
