...
#if ZF_HAS_TRANSPORT_SETTING || ZF_AUTO_ACCEPT_REQUESTS || ZF_DEV_FIDO2_1 || \
    ZF_HAS_ATTESTATION_SETTING
...
static bool
    zerofido_pin_input_validator_callback(const char* text, FuriString* error, void* context);
...
#if ZF_DEV_SCREENSHOT
static void zerofido_dev_screenshot_framebuffer_callback(
    uint8_t* data,
    size_t size,
...
_Static_assert(
    sizeof(ZfCredentialDisplayScratch) <= ZF_UI_SCRATCH_SIZE,
...
static void zerofido_refresh_pin_menu(ZerofidoApp* app) {
    ...
    if(!pin_is_set) {
        submenu_add_item(
            app->pin_menu, "Set PIN", ZfPinMenuItemSet, zerofido_pin_menu_callback, app);
    ...
    submenu_add_item(
        app->pin_menu, "Change PIN", ZfPinMenuItemChange, zerofido_pin_menu_callback, app);
    submenu_add_item(
        app->pin_menu, "Remove PIN", ZfPinMenuItemRemove, zerofido_pin_menu_callback, app);
...
static void zerofido_open_pin_input(
    ZerofidoApp* app,
    ZfPinInputState state,
    const char* header,
    size_t min_length) {
    ...
    text_input_set_result_callback(
        app->pin_input_view,
        zerofido_pin_input_result_callback,
        app,
        pin_buffers->input,
        sizeof(pin_buffers->input),
        false);
    zerofido_ui_switch_to_view(app, ZfViewPinInput);
}

static void zerofido_open_pin_confirm_dialog(
    ZerofidoApp* app,
...
static void zerofido_open_pin_confirm(ZerofidoApp* app) {
    zerofido_open_pin_confirm_dialog(
        app,
...
static void zerofido_refresh_settings_menu(ZerofidoApp* app) {
    ...
    snprintf(
        transport_label,
        sizeof(transport_label),
        "Transport: %s",
        runtime_config.transport_mode == ZfTransportModeNfc ? "NFC" : "USB HID");
    submenu_add_item(
        app->settings_menu,
        transport_label,
        ZfSettingsItemTransport,
        zerofido_settings_menu_callback,
        app);
#endif
#if ZF_AUTO_ACCEPT_REQUESTS
    snprintf(
        auto_accept_label,
        sizeof(auto_accept_label),
        "Auto-approve: %s",
        runtime_config.auto_accept_requests ? "On" : "Off");
    submenu_add_item(
        app->settings_menu,
        auto_accept_label,
        ZfSettingsItemAutoAcceptRequests,
        zerofido_settings_menu_callback,
        app);
#endif
#if ZF_DEV_FIDO2_1
    snprintf(
        fido2_profile_label,
        sizeof(fido2_profile_label),
        "FIDO2: %s",
        zf_fido2_profile_name(runtime_config.fido2_profile));
    submenu_add_item(
        app->settings_menu,
        fido2_profile_label,
        ZfSettingsItemFido2Profile,
        zerofido_settings_menu_callback,
        app);
#endif
#if ZF_HAS_ATTESTATION_SETTING
    snprintf(
        attestation_label,
        sizeof(attestation_label),
        "Attest: %s",
        zf_attestation_mode_name(runtime_config.attestation_mode));
    submenu_add_item(
        app->settings_menu,
        attestation_label,
        ZfSettingsItemAttestation,
        zerofido_settings_menu_callback,
        app);
#endif
    submenu_add_item(
        app->settings_menu, "PIN", ZfSettingsItemPin, zerofido_settings_menu_callback, app);
    if(startup_reset_available) {
        submenu_add_item(
            app->settings_menu,
            "Reset app data",
            ZfSettingsItemStartupReset,
            zerofido_settings_menu_callback,
            app);
...
static void zerofido_format_assertion_selection_label(
    ZerofidoApp* app,
...
static bool zerofido_select_next_position(
    uint32_t* values,
    size_t count,
    bool found_selected,
    size_t selected_pos,
...
static bool zerofido_select_next_stored_credential_locked(ZerofidoApp* app, int8_t direction) {
    ...
    if(!zerofido_select_next_position(
           values, count, found_selected, selected_pos, direction, &selected_value)) {
        return false;
    }
...
static void zerofido_refresh_credentials_menu(ZerofidoApp* app) {
    ...
    if(app->store.records && item_count > 0U) {
        ...
        for(size_t i = 0; i < credential_count; ++i) {
            ...
            submenu_add_item(
                app->credentials_menu, label, (uint32_t)i, zerofido_credentials_menu_callback, app);
        }
    } else {
...
static void zerofido_fill_credential_detail_model(
    ZerofidoApp* app,
    const ZfCredentialIndexEntry* entry,
...
static void zerofido_refresh_credential_detail(ZerofidoApp* app) {
    ...
    with_view_model(
        app->credential_detail_view, ZfCredentialDetailModel * model, { *model = snapshot; }, true);
}
...
static void zerofido_settings_menu_callback(void* context, uint32_t index) {
    ...
    case ZfSettingsItemTransport: {
        ...
        if(zf_app_lifecycle_set_transport_mode(app, app->storage, mode)) {
            zerofido_ui_set_status(
                app, mode == ZfTransportModeNfc ? "Transport: NFC" : "Transport: USB HID");
        } else {
            zerofido_notify_error(app);
    ...
    case ZfSettingsItemFido2Profile: {
        ZfFido2Profile profile = runtime_config.fido2_profile ==
        ...
        if(zf_runtime_config_set_fido2_profile(app, app->storage, profile)) {
            zerofido_ui_set_status(
                app,
                profile == ZfFido2ProfileCtap2_1Experimental ? "FIDO2 profile: 2.1 exp" :
                                                               "FIDO2 profile: 2.0");
        } else {
            zerofido_notify_error(app);
            zerofido_ui_set_status(
    ...
    case ZfSettingsItemAttestation: {
        ...
        if(zf_runtime_config_set_attestation_mode(app, app->storage, mode)) {
            zerofido_ui_set_status(
                app, mode == ZfAttestationModePacked ? "Attest: packed" : "Attest: none");
        } else {
            zerofido_notify_error(app);
            zerofido_ui_set_status(app, "Settings save failed");
        }
...
static void zerofido_pin_input_result_callback(void* context) {
    ...
    case ZfPinInputChangeConfirm:
        ...
        status =
            zerofido_pin_verify_plaintext(app->storage, &app->pin_state, pin_buffers->current);
        if(status == ZF_CTAP_SUCCESS) {
            status = zerofido_pin_replace_plaintext(
                app->storage, &app->pin_state, pin_buffers->new_pin);
        }
        break;
    case ZfPinInputRemoveCurrent:
        strncpy(pin_buffers->current, pin_buffers->input, sizeof(pin_buffers->current) - 1);
    ...
    if(result == DialogExResultCenter) {
        ...
        if(!zerofido_begin_local_maintenance(app)) {
            ...
            case ZfPinConfirmActionRemove: {
                ...
                uint8_t status = zerofido_pin_verify_plaintext(
                    app->storage, &app->pin_state, pin_buffers->current);
                if(status == ZF_CTAP_SUCCESS) {
...
static void zerofido_open_delete_credential_confirm(ZerofidoApp* app) {
    zerofido_open_pin_confirm_dialog(
        ...
        ZfViewCredentialDetail,
        "Delete passkey?",
        "This cannot be undone",
        "Delete");
}

static void zerofido_draw_auto_scroll_line(
    Canvas* canvas,
    FuriString* line,
    int32_t x,
    int32_t y,
...
static void zerofido_credential_detail_draw_row(
    Canvas* canvas,
    int32_t y,
    const char* label,
    const char* value,
    FuriString* scroll_line) {
...
static bool zerofido_dev_screenshot_write_ppm(File* file, const uint8_t* frame, size_t size) {
    ...
    for(size_t y = 0; y < ZF_DEV_SCREENSHOT_HEIGHT; ++y) {
        ...
        for(size_t x = 0; x < ZF_DEV_SCREENSHOT_WIDTH; ++x) {
            const uint8_t* color = zerofido_dev_screenshot_pixel(frame, size, x, y) ? ink :
                                                                                      backlight;
...
static bool zerofido_dev_screenshot_save(ZerofidoApp* app, char* path, size_t path_size) {
    ...
    snprintf(
        path,
        path_size,
        ZF_DEV_SCREENSHOT_DIR "/screen_%lu_%lu.ppm",
...
static void zerofido_tick_callback(void* context) {
    ...
    if(active_view == ZfViewStatus) {
        ...
        with_view_model(
            app->credential_detail_view, ZfCredentialDetailModel * model, { UNUSED(model); }, true);
    }
#if ZF_RELEASE_DIAGNOSTICS
...
bool zerofido_ui_ensure_view(ZerofidoApp* app, ZfViewId view_id) {
    ...
    case ZfViewApproval:
        ...
        return zerofido_ui_register_view(
            app, ZfViewApproval, dialog_ex_get_view(app->approval_view));
    ...
    case ZfViewCredentials:
        if(!app->credentials_menu) {
            ...
            view_set_previous_callback(
                submenu_get_view(app->credentials_menu), zerofido_credentials_previous_callback);
        }
        return zerofido_ui_register_view(
            app, ZfViewCredentials, submenu_get_view(app->credentials_menu));
    ...
    case ZfViewCredentialDetail:
        ...
        view_allocate_model(
            app->credential_detail_view, ViewModelTypeLocking, sizeof(ZfCredentialDetailModel));
        view_set_context(app->credential_detail_view, app);
        view_set_draw_callback(
            app->credential_detail_view, zerofido_credential_detail_draw_callback);
        view_set_input_callback(
            app->credential_detail_view, zerofido_credential_detail_input_callback);
        view_set_previous_callback(
            app->credential_detail_view, zerofido_credential_detail_previous_callback);
    ...
    case ZfViewSettings:
        if(!app->settings_menu) {
            ...
            submenu_set_header(app->settings_menu, "Settings");
            view_set_previous_callback(
                submenu_get_view(app->settings_menu), zerofido_settings_previous_callback);
        }
        return zerofido_ui_register_view(
            app, ZfViewSettings, submenu_get_view(app->settings_menu));
    ...
    case ZfViewPinMenu:
        if(!app->pin_menu) {
            ...
            submenu_set_header(app->pin_menu, "PIN");
            view_set_previous_callback(
                submenu_get_view(app->pin_menu), zerofido_pin_menu_previous_callback);
        }
    ...
    case ZfViewPinInput:
        if(!app->pin_input_view) {
            ...
            view_set_previous_callback(
                text_input_get_view(app->pin_input_view), zerofido_pin_input_previous_callback);
        }
        return zerofido_ui_register_view(
            app, ZfViewPinInput, text_input_get_view(app->pin_input_view));
    ...
    case ZfViewPinConfirm:
        if(!app->pin_confirm_view) {
            ...
            dialog_ex_set_result_callback(
                app->pin_confirm_view, zerofido_pin_confirm_result_callback);
            dialog_ex_set_context(app->pin_confirm_view, app);
            view_set_previous_callback(
                dialog_ex_get_view(app->pin_confirm_view), zerofido_pin_confirm_previous_callback);
        }
        return zerofido_ui_register_view(
            app, ZfViewPinConfirm, dialog_ex_get_view(app->pin_confirm_view));
...
bool zerofido_ui_init(ZerofidoApp* app) {
    ...
    view_dispatcher_set_navigation_event_callback(
        app->view_dispatcher, zerofido_navigation_callback);
    view_dispatcher_set_custom_event_callback(
        app->view_dispatcher, zerofido_custom_event_callback);
    ...
    if(app->dev_screenshot_input_events) {
        app->dev_screenshot_input_subscription = furi_pubsub_subscribe(
            app->dev_screenshot_input_events, zerofido_dev_screenshot_input_callback, app);
    }
#endif
...
void zerofido_ui_deinit(ZerofidoApp* app) {
    ...
    if(app->dev_screenshot_input_events && app->dev_screenshot_input_subscription) {
        furi_pubsub_unsubscribe(
            app->dev_screenshot_input_events, app->dev_screenshot_input_subscription);
    ...
    if(app->gui && app->dev_screenshot_framebuffer_registered) {
        gui_remove_framebuffer_callback(
            app->gui, zerofido_dev_screenshot_framebuffer_callback, app);
...
void zerofido_ui_dispatch_custom_event(ZerofidoApp* app, ZfCustomEvent event) {
    ...
    if(app->ui_events_enabled) {
        dispatcher = app->view_dispatcher;
        dispatch_inline = app->ui_thread_id != NULL &&
                          app->ui_thread_id == furi_thread_get_current_id();
    }
    furi_mutex_release(app->ui_mutex);
