...
_Static_assert(
    sizeof(ZfStatusCredentialScratch) <= ZF_UI_SCRATCH_SIZE,
...
static bool zf_status_format_record_item(
    ZfCredentialRecord* record,
    char* title,
    size_t title_size,
    char* subtitle,
...
static bool zf_status_refresh_model(ZerofidoApp* app, bool redraw, bool reload_credentials) {
    ...
    if(zf_status_text_is_error(app->status_text)) {
        ...
        app->approval.state == ZfApprovalPending || app->status_text[0] != '\0' ||
    ...
    for(size_t i = 0; i < snapshot.visible_count; ++i) {
        ...
        if(!formatted) {
            zf_ui_format_passkey_index_title(
                &entries[i], snapshot.items[i].title, sizeof(snapshot.items[i].title));
            zf_ui_format_passkey_index_subtitle(
                &entries[i], snapshot.items[i].subtitle, sizeof(snapshot.items[i].subtitle));
        }
    }
...
static void zf_status_draw_scrollable_line(
    Canvas* canvas,
    FuriString* line,
    int32_t x,
    int32_t y,
    ...
    if(status->item_count == 0) {
        ...
        for(size_t row = 0; row < status->visible_count; ++row) {
            ...
            zf_status_draw_scrollable_line(
                ...
                5,
                y + 7,
                status->items[row].resident_key ? 108U : 118U,
                status->items[row].title,
