    ...
    do {
        ...
        if(is_raw) {
            ...
            if(min_pulse < INT32_MAX && min_pulse > 0) {
                ...
                furi_string_cat_printf(
                    app->text_buffer, "Est. symbol rate: %lu Hz\n", symbol_rate);

                // Estimate data rate for common encodings
                furi_string_cat_printf(app->text_buffer, "\nPossible data rates:\n");
                furi_string_cat_printf(
                    app->text_buffer, "  Manchester: %lu bps\n", symbol_rate / 2);
                ...
                if(protocol && protocol->decoder) {
                    ...
                    if(protocol->decoder->alloc && protocol->decoder->deserialize) {
                        SubGhzProtocolDecoderBase* decoder =
                            protocol->decoder->alloc(app->environment);

                        if(decoder) {
                            flipper_format_rewind(format);
                            SubGhzProtocolStatus status =
                                protocol->decoder->deserialize(decoder, format);
        ...
        if(frequency > 0) {
            ...
            furi_string_cat_printf(
                app->text_buffer,
                "Wavelength: %lu.%02lu%lu m (%lu.%lu cm)\n",
                wavelength_m,
                wavelength_cm,
                wavelength_mm_frac,
                wavelength_mm / 10,
            ...
            furi_string_cat_printf(
                app->text_buffer, "λ/4 antenna: %lu.%lu cm\n", quarter_wave_cm, quarter_wave_mm);
        ...
        if(te > 0) {
            ...
            if(baud == 0) {
                ...
                if(symbols_per_bit == 1 || (symbols_per_bit == 0 && symbols_per_bit_frac >= 9)) {
                    ...
                    symbols_per_bit == 2 || (symbols_per_bit == 1 && symbols_per_bit_frac >= 9)) {
                    encoding = "Manchester";
                } else if(
                    symbols_per_bit == 3 || (symbols_per_bit == 2 && symbols_per_bit_frac >= 9)) {
                    encoding = "3-PWM";
                } else if(
                    symbols_per_bit == 4 || (symbols_per_bit == 3 && symbols_per_bit_frac >= 9)) {
                    encoding = "4-PWM";
                }

                furi_string_cat_printf(
                    app->text_buffer,
                    "Symbols per bit: %lu.%lu\n",
                    symbols_per_bit,
                    symbols_per_bit_frac);
        ...
        if(baud > 0) {
            ...
            furi_string_cat_printf(
                app->text_buffer, "Data rate: %lu.%02lu kbps\n", kbps_int, kbps_frac);

            // Bandwidth estimates
            furi_string_cat_printf(
                app->text_buffer, "Min bandwidth: %lu.%lu kHz\n", baud / 1000, (baud % 1000) / 100);
            furi_string_cat_printf(
                app->text_buffer,
                "Typical BW: %lu.%lu kHz\n",
                (baud * 2) / 1000,
                ((baud * 2) % 1000) / 100);
        }
        ...
        if(bits > 0 && (baud > 0 || te > 0)) {
            ...
            furi_string_cat_printf(
                app->text_buffer, "Single TX: %lu.%02lu ms\n", tx_time_ms, tx_time_us_frac / 10);
            ...
            if(repeat > 1 && delay > 0) {
                ...
                uint32_t avg_current_ua =
                    (30000 * active_time + 1000 * (total_time - active_time)) / total_time;
                furi_string_cat_printf(
                    app->text_buffer,
                    "Avg current: %lu.%lu mA\n",
                    avg_current_ua / 1000,
                    (avg_current_ua % 1000) / 100);
            }
        }
        ...
        if(!furi_string_empty(key_data)) {
            ...
            if(bits > 0) {
                ...
                if(byte_len == expected_bytes) {
                    ...
                    furi_string_cat_printf(
                        app->text_buffer, "Expected bytes: %zu\n", expected_bytes);
                }
            }
            ...
            for(size_t i = 0; i < hex_len - 1; i += 2) {
                ...
                if((hex[i] != 'F' && hex[i] != 'f') || (hex[i + 1] != 'F' && hex[i + 1] != 'f'))
                    all_ff = false;
            }
            ...
            furi_string_cat_printf(
                app->text_buffer,
                "Entropy: %lu.%02lu bits/byte\n",
                entropy_int / 100,
                entropy_int % 100);
            furi_string_cat_printf(
                app->text_buffer, "Unique bytes: %lu/%zu\n", unique_bytes, byte_len);
...
static SubAnalyzerApp* sub_analyzer_app_alloc() {
    ...
    view_set_previous_callback(
        text_box_get_view(app->text_box), sub_analyzer_exit_to_submenu_callback);
    view_set_previous_callback(
        loading_get_view(app->loading), sub_analyzer_exit_to_submenu_callback);

    view_dispatcher_add_view(
        app->view_dispatcher, SubAnalyzerViewSubmenu, submenu_get_view(app->submenu));
    view_dispatcher_add_view(
        app->view_dispatcher, SubAnalyzerViewPopup, popup_get_view(app->popup));
    view_dispatcher_add_view(
        app->view_dispatcher, SubAnalyzerViewTextBox, text_box_get_view(app->text_box));
    view_dispatcher_add_view(
        app->view_dispatcher, SubAnalyzerViewLoading, loading_get_view(app->loading));

    submenu_add_item(
        app->submenu,
        "Select .sub file",
        SubAnalyzerSubmenuIndexSelectFile,
        sub_analyzer_submenu_callback,
        app);
    submenu_add_item(
        app->submenu, "About", SubAnalyzerSubmenuIndexAbout, sub_analyzer_submenu_callback, app);
