    ...
    if(message) {
        ...
        if(message->address == NAME_ADDR) {
            ...
            if(controller->file_name_index < (MAX_FILENAME_SIZE - 1)) {
                ...
                if(received_char == '\0' ||
                   controller->file_name_index == (MAX_FILENAME_SIZE - 1)) {
        ...
        if(message->address == INIT_ADDR) {
            ...
            with_view_model(
                controller->view,
                ReceivingModel * _model,
                { furi_string_set(_model->status, "gettin file size"); },
                redraw);

            if(controller->size_file_count < 4) {
                controller->size_file_buffer[controller->size_file_count] =
                    (uint8_t)(message->command & 0xFF);
                controller->size_file_count++;

                FURI_LOG_I(
                    TAG,
                    "Received byte %d for file size: 0x%02X",
                    controller->size_file_count,
                    (uint8_t)(message->command & 0xFF));
        ...
        if(message->address == REC_ADDR) {
            if(controller->is_receiving == true) {
                ...
                if(controller->file) {
                    storage_file_open(
                        controller->file, FILE_PATH_RECV, FSAM_READ_WRITE, FSOM_OPEN_APPEND);
                    ...
                    FURI_LOG_I(
                        TAG,
                        "Chunk reçu et écrit dans le fichier (%d octets)",
                    ...
                    if(bytes_written == 1) {
                        ...
                        FURI_LOG_I(
                            TAG, "Progression : %.2f%%", (double)(controller->progress * 100.0f));
