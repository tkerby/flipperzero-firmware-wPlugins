...
char* PROTOCOL_NAMES[NfcTechNum] = {
    "ISO14443-3A",
    "ISO14443-3B",
    "ISO15693-3",
    "FELICA"
...
UI* ui_alloc() {
    ...
    view_dispatcher_add_view(ui->view_dispatcher, View_ProtocolSelectDisplay, submenu_get_view(ui->protocol_select_scene->protocol_menu));
    view_dispatcher_add_view(ui->view_dispatcher, View_LogDisplay, text_box_get_view(ui->log_scene->log_display));
    ...
    char path[64];
    snprintf(path, 64, "%s/log_%04d%02d%02d_%02d%02d%02d.txt", LOG_FILE_FOLDER, datetime.year, datetime.month, datetime.day, datetime.hour, datetime.minute, datetime.second);
    ...
    if(!open){
        FURI_LOG_E(LOG_TAG, "Failed to open %s. ERROR: %s", path, storage_file_get_error_desc(ui->log_file));
    }
