...
static void wendigo_scene_device_list_var_list_enter_callback(void* context, uint32_t index) {
    ...
    flipper_bt_device* item = (display_selected_only) ? bt_selected_devices[index] :
                                                        bt_devices[index];
...
static void wendigo_scene_device_list_var_list_change_callback(VariableItem* item) {
    ...
    furi_assert(
        item_index < ((display_selected_only) ? bt_selected_devices_count : bt_devices_count));
    ...
    case WendigoOptionScanType:
        variable_item_set_current_value_text(
            item,
            (menu_item->dev.scanType == SCAN_HCI)  ? "BT Classic" :
            (menu_item->dev.scanType == SCAN_BLE)  ? "BLE" :
            (menu_item->dev.scanType == SCAN_WIFI) ? "WiFi" :
                                                     "Unknown");
        break;
...
bool wendigo_scene_device_list_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        ...
        uint16_t devices_count = (display_selected_only) ? bt_selected_devices_count :
                                                           bt_devices_count;
        char elapsedStr[7];
        ...
        for(int i = 0; i < devices_count; ++i) {
            if(devices != NULL && devices[i] != NULL && devices[i]->view != NULL &&
               variable_item_get_current_value_index(devices[i]->view) == WendigoOptionLastSeen) {
                ...
                _elapsedTime(
                    (uint32_t*)&(devices[i]->dev.lastSeen.tv_sec),
                    &now,
                    elapsedStr,
                    sizeof(elapsedStr));
