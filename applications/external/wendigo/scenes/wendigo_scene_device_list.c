...
void wendigo_scene_device_list_free() {
    ...
    if(stack_counter > 0 && stack != NULL) {
        ...
        for(uint8_t i = 0; i < stack_counter; ++i) {
            if(stack[i].devices_count > 0 && stack[i].devices != NULL && stack[i].free_devices) {
...
bool wendigo_device_is_displayed(wendigo_device* dev) {
    ...
    if((current_devices.devices_mask & DEVICE_CUSTOM) == DEVICE_CUSTOM) {
        wendigo_log(
            MSG_WARN,
            "wendigo_device_is_displayed(): The current device mask includes DEVICE_CUSTOM, results from this function are inconclusive.");
    }
...
uint16_t wendigo_set_current_devices(uint8_t deviceMask) {
    ...
    if((deviceMask & DEVICE_CUSTOM) == DEVICE_CUSTOM) {
        FURI_LOG_T(WENDIGO_TAG, "End wendigo_set_current_devices(): DEVICE_CUSTOM specified.");
    ...
    if(deviceCount > 0) {
        wendigo_device** new_devices =
            realloc(current_devices.devices, sizeof(wendigo_device*) * deviceCount);
        if(new_devices == NULL) {
            return 0;
        }
...
void wendigo_scene_device_list_redraw(WendigoApp* app) {
    ...
    variable_item_list_set_header(
        app->devices_var_item_list,
        (current_devices.devices_msg[0] == '\0') ? NULL : current_devices.devices_msg);
