    ...
    if(selected && !device->dev.tagged) {
        ...
        if(bt_selected_devices_capacity == bt_selected_devices_count) {
            ...
            flipper_bt_device** new_devices = realloc(
                bt_selected_devices,
                sizeof(flipper_bt_device*) * (bt_selected_devices_capacity + 1));
            if(new_devices == NULL) {
        ...
        uint8_t index = (uint8_t)bt_custom_device_index(
            device, bt_selected_devices, bt_selected_devices_count);
...
uint16_t bt_custom_device_index(
    flipper_bt_device* dev,
    flipper_bt_device** array,
    ...
    for(result = 0;
        result < array_count && memcmp(dev->dev.bda, array[result]->dev.bda, MAC_BYTES);
        ++result) {
    }
    return result;
}
...
bool wendigo_update_bt_device(WendigoApp* app, flipper_bt_device* dev) {
    ...
    if(dev->dev.bdname_len > 0 && dev->dev.bdname != NULL) {
        ...
        if(new_name !=
           NULL) { /* If allocation fails target's existing bdname and bdname_len are unmodified */
...
uint16_t parseBufferBluetooth(WendigoApp* app) {
    ...
    memcpy(
        &(dev->dev.lastSeen),
        buffer + WENDIGO_OFFSET_BT_LASTSEEN,
        sizeof(struct timeval)); // TODO: Should lastSeen be removed from here as well?
    memcpy(
        &(dev->dev.bt_services.num_services),
