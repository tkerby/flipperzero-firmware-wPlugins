...
private:
    ...
    uint32_t lastWebsocketSendTime =
        0; // Last time any websocket message was sent (for throttling)
    ...
    void processCompleteMultiplayerMessage(
        const char* message); // Process a complete multiplayer message (after chunk assembly)
    void processWebsocketQueue(
        FlipWorldApp* app); // Process queued websocket messages with 100ms throttling
    bool queueWebsocketMessage(const char* message); // Queue a websocket message for sending
    bool safeWebsocketSend(
        FlipWorldApp* app,
        const char* message); // Send websocket message with 100ms throttling
    void sendMessageWithChunking(
        FlipWorldApp* app,
        const char* message); // Send websocket message with chunking support for large messages
...
public:
    ...
    std::unique_ptr<Level>
        getLevel(LevelIndex index, Game* game = nullptr) const; // Get a level by index
    ...
    void
        processWebsocketMessageQueue(); // Process the websocket message queue (call this regularly)
