...
bool load_settings(
    ...
    if(storage_file_read(file, &wifi_ssid_length, sizeof(size_t)) != sizeof(size_t) ||
       wifi_ssid_length > wifi_ssid_size ||
    ...
    if(storage_file_read(file, &wifi_password_length, sizeof(size_t)) != sizeof(size_t) ||
       wifi_password_length > wifi_password_size ||
    ...
    if(storage_file_read(file, &username_length, sizeof(size_t)) != sizeof(size_t) ||
...
bool save_char(const char* path_name, const char* value) {
    ...
    snprintf(
        file_path,
        sizeof(file_path),
        STORAGE_EXT_PATH_PREFIX "/apps_data/flip_world/data/%s.txt",
        path_name);
...
static FuriString* flip_social_info(char* key) {
    ...
    if(!load_flip_social_settings(
           ssid,
           sizeof(ssid),
           password,
           sizeof(password),
           login_username_logged_out,
           sizeof(login_username_logged_out),
           login_username_logged_in,
           sizeof(login_username_logged_in),
           login_password_logged_out,
           sizeof(login_password_logged_out),
           change_password_logged_in,
           sizeof(change_password_logged_in),
           change_bio_logged_in,
           sizeof(change_bio_logged_in),
           is_logged_in,
           sizeof(is_logged_in))) {
...
bool is_logged_in_to_flip_social() {
    ...
    if(furi_string_cmp(is_logged_in, "true") == 0) {
        ...
        save_settings(
            furi_string_get_cstr(wifi_ssid),
            furi_string_get_cstr(wifi_password),
...
static bool load_flip_social_settings(
    ...
    snprintf(
        file_path,
        sizeof(file_path),
        STORAGE_EXT_PATH_PREFIX "/apps_data/flip_social/settings.bin");
    if(!storage_file_open(file, file_path, FSAM_READ, FSOM_OPEN_EXISTING)) {
    ...
    if(storage_file_read(file, &change_bio_length, sizeof(size_t)) != sizeof(size_t) ||
       change_bio_length > change_bio_size ||
