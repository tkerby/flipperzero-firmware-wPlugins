...
void draw_tree_world(Level* level) {
    ...
    for(int i = 0; i < 2; i++) {
        ...
        spawn_icon_line(
            level, tree_icon->icon, 5, 2 + i * 17, tree_icon->width, tree_icon->height, 22, true);
        // Vertical line of 11 icons
        spawn_icon_line(
            level, tree_icon->icon, 5 + i * 17, 2, tree_icon->width, tree_icon->height, 11, false);
    }
    ...
    for(int i = 20; i < 22; i++) {
        ...
        spawn_icon_line(
            level, tree_icon->icon, 5 + i * 17, 50, tree_icon->width, tree_icon->height, 8, false);
    }

    // Labyrinth lines
    // Third line (14 left, then a gap, then 3 middle)
    spawn_icon_line(
        level, tree_icon->icon, 5, 2 + 2 * 17, tree_icon->width, tree_icon->height, 14, true);
    spawn_icon_line(
        level,
        tree_icon->icon,
        5 + 16 * 17,
    ...
    spawn_icon_line(
        level, tree_icon->icon, 5, 2 + 4 * 17, tree_icon->width, tree_icon->height, 6, true);
    spawn_icon_line(
        level,
        tree_icon->icon,
        5 + 7 * 17,
...
void draw_town_world(Level* level) {
    ...
    if(!house_icon || !fence_icon || !fence_end_icon || !plant_icon || !flower_icon || !man_icon ||
       !woman_icon || !lake_top_left_icon || !lake_top_icon || !lake_top_right_icon ||
       !lake_left_icon || !lake_right_icon || !lake_bottom_left_icon || !lake_bottom_icon ||
       !lake_bottom_right_icon || !tree_icon) {
    ...
    spawn_icon(
        level, fence_end_icon->icon, 180, 72, fence_end_icon->width, fence_end_icon->height);
    ...
    spawn_icon(
        level, fence_end_icon->icon, 126, 72, fence_end_icon->width, fence_end_icon->height);
    ...
    spawn_icon_line(
        level, fence_icon->icon, 8, 100, fence_icon->width, fence_icon->height, 10, true);

    // plants spaced out underneath the fences
    spawn_icon_line(
        level, plant_icon->icon, 40, 110, plant_icon->width, plant_icon->height, 6, true);
    spawn_icon_line(
        level, flower_icon->icon, 40, 140, flower_icon->width, flower_icon->height, 6, true);
    ...
    spawn_icon(
        level,
        lake_top_left_icon->icon,
        240,
        52,
        lake_top_left_icon->width,
        lake_top_left_icon->height);
    spawn_icon(level, lake_top_icon->icon, 264, 52, lake_top_icon->width, lake_top_icon->height);
    spawn_icon(
        level,
        lake_top_right_icon->icon,
        295,
        52,
        lake_top_right_icon->width,
        lake_top_right_icon->height);

    // Middle row
    spawn_icon(
        level, lake_left_icon->icon, 231, 74, lake_left_icon->width, lake_left_icon->height);
    spawn_icon(
        level, lake_right_icon->icon, 317, 74, lake_right_icon->width, lake_right_icon->height);

    // Bottom row
    spawn_icon(
        level,
        lake_bottom_left_icon->icon,
        240,
        105,
        lake_bottom_left_icon->width,
        lake_bottom_left_icon->height);
    spawn_icon(
        level, lake_bottom_icon->icon, 264, 124, lake_bottom_icon->width, lake_bottom_icon->height);
    spawn_icon(
        level,
        lake_bottom_right_icon->icon,
        295,
        105,
        lake_bottom_right_icon->width,
        lake_bottom_right_icon->height);
