...
void player_spawn(Level* level, GameManager* manager) {
    ...
    entity_collider_add_rect(
        player, 10 + PLAYER_COLLISION_HORIZONTAL, 10 + PLAYER_COLLISION_VERTICAL);
...
static void game_start(GameManager* game_manager, void* ctx) {
    ...
    for(int i = 0; i < 10; i++) {
        ...
        levels[i] = game_manager_add_level(
            game_manager, generic_level(furi_string_get_cstr(world_name), i));
