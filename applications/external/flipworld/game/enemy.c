...
static EntityContext* enemy_generic_alloc(
    ...
    if(username != NULL) {
        snprintf(
            enemy_context_generic->username,
            sizeof(enemy_context_generic->username),
            "%s",
...
static void enemy_start(Entity* self, GameManager* manager, void* context) {
    ...
    snprintf(
        enemy_context->username,
        sizeof(enemy_context->username),
        "%s",
        enemy_context_generic->username);
...
static void enemy_render(Entity* self, GameManager* manager, Canvas* canvas, void* context) {
    ...
    if(x_pos + enemy_context->size.x < 0 || x_pos > SCREEN_WIDTH ||
       y_pos + enemy_context->size.y < 0 || y_pos > SCREEN_HEIGHT)
        return;
    ...
    if(game_context->game_mode != GAME_MODE_STORY ||
       (game_context->game_mode == GAME_MODE_STORY && game_context->tutorial_step == 4)) {
...
static void enemy_atk_notify(
    GameContext* game_context,
    EntityContext* enemy_context,
...
static void enemy_collision(Entity* self, Entity* other, GameManager* manager, void* context) {
    ...
    if(entity_description_get(other) == &player_desc) {
        ...
        if(player_is_facing_enemy && game_context->last_button == GameKeyOk &&
           !enemy_is_facing_player) {
            ...
            if(player_context->elapsed_attack_timer >= player_context->attack_timer) {
                ...
                if(enemy_context->health <= 0) {
                    ...
                    enemy_pos.x -=
                        player_context->dx * enemy_context->radius + game_context->icon_offset;
                ...
                FURI_LOG_I(
                    "Game",
                    "Player attack on enemy '%d' is on cooldown: %f seconds remaining",
                    enemy_context->id,
                    (double)(player_context->attack_timer - player_context->elapsed_attack_timer));
            }
        }
        // Handle Enemy Attacking Player (enemy facing player)
            ...
            if(enemy_context->elapsed_attack_timer >= enemy_context->attack_timer) {
                ...
                if(player_context->health <= 0) {
                    ...
                    FURI_LOG_I(
                        "Game",
                        "Player took %f damage from enemy '%d'",
...
static void enemy_pvp_position(GameContext* game_context, EntityContext* enemy, Entity* self) {
    ...
    if(game_context->fhttp->last_response != NULL &&
       strlen(game_context->fhttp->last_response) > 0) {
        // for debugging
