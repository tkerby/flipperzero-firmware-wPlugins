...
static void icon_start(Entity* self, GameManager* manager, void* context) {
    ...
    entity_collider_add_circle(self, (icon_ctx_self->width + icon_ctx_self->height) / 4);
...
static IconContext*
    icon_generic_alloc(const char* id, const Icon* icon, uint8_t width, uint8_t height) {
