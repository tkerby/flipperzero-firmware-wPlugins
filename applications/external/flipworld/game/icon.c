...
static void icon_start(Entity* self, GameManager* manager, void* context) {
    ...
    entity_collider_add_circle(self, (ictx_self->size.x + ictx_self->size.y) / 4);
...
static IconContext*
    icon_generic_alloc(IconID id, const Icon* icon, uint8_t width, uint8_t height) {
...
IconContext* get_icon_context(const char* name) {
    if(is_str(name, "house"))
        ...
        return icon_generic_alloc(
            ICON_ID_FENCE_VERTICAL_END, &I_icon_fence_vertical_end_6x8px, 6, 8);
    else if(is_str(name, "fence_vertical_start"))
        return icon_generic_alloc(
            ICON_ID_FENCE_VERTICAL_START, &I_icon_fence_vertical_start_6x15px, 6, 15);
        ...
        return icon_generic_alloc(
            ICON_ID_LAKE_BOTTOM_LEFT, &I_icon_lake_bottom_left_24x22px, 24, 22);
    else if(is_str(name, "lake_bottom_right"))
        return icon_generic_alloc(
            ICON_ID_LAKE_BOTTOM_RIGHT, &I_icon_lake_bottom_right_24x22px, 24, 22);
