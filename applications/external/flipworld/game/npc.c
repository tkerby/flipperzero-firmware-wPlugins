...
static void npc_start(Entity* self, GameManager* manager, void* context) {
    ...
    snprintf(
        npc_context->message, sizeof(npc_context->message), "%s", npc_context_generic->message);
...
static void npc_render(Entity* self, GameManager* manager, Canvas* canvas, void* context) {
    ...
    if(x_pos + npc_context->size.x < 0 || x_pos > SCREEN_WIDTH ||
       y_pos + npc_context->size.y < 0 || y_pos > SCREEN_HEIGHT)
        return;
...
static void npc_collision(Entity* self, Entity* other, GameManager* manager, void* context) {
    ...
    if(entity_description_get(other) == &player_desc) {
        ...
        if(player_is_facing_npc && game_context->last_button == GameKeyOk) {
            ...
            snprintf(
                game_context->message, sizeof(game_context->message), "%s", npc_context->message);
...
static void npc_update(Entity* self, GameManager* manager, void* context) {
    ...
    case ENTITY_MOVING_TO_START: {
        ...
        Vector target_position = (npc_context->state == ENTITY_MOVING_TO_END) ?
                                     npc_context->end_position :
                                     npc_context->start_position;
...
static const EntityDescription*
    ...
    npc_context_generic->sprite_right =
        game_manager_sprite_load(manager, sprite_context->right_file_name);
    npc_context_generic->sprite_left =
...
void npc_spawn(Level* level, GameManager* manager, FuriString* json) {
    ...
    if(!id || !_index || !start_position || !start_position_x || !start_position_y ||
       !end_position || !end_position_x || !end_position_y || !move_timer || !speed || !message) {
        FURI_LOG_E("Game", "Failed to get JSON values");
        return;
    }

    GameContext* game_context = game_manager_game_context_get(manager);
    if(game_context && game_context->npc_count < MAX_NPCS &&
       !game_context->npcs[game_context->npc_count]) {
        game_context->npcs[game_context->npc_count] = level_add_entity(
            level,
            npc(manager,
