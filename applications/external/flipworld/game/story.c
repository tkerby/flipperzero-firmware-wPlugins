...
void story_draw(Entity* player, Canvas* canvas, GameManager* manager) {
    if(!player || !canvas || !manager) return;
...
void story_update(Entity* player, GameManager* manager) {
    ...
    if(game_context->game_mode == GAME_MODE_STORY) {
        switch(game_context->story_step) {
        case 0:
            if(input.held & GameKeyLeft) game_context->story_step++;
            break;
        case 1:
            if(input.held & GameKeyRight) game_context->story_step++;
            break;
        case 2:
            if(input.held & GameKeyUp) game_context->story_step++;
            break;
        case 3:
            if(input.held & GameKeyDown) game_context->story_step++;
            break;
        case 5:
            if(input.held & GameKeyOk && game_context->is_menu_open) game_context->story_step++;
            break;
        case 6:
            if(input.held & GameKeyBack) game_context->story_step++;
            break;
        case 7:
            if(input.held & GameKeyBack) game_context->story_step++;
            break;
