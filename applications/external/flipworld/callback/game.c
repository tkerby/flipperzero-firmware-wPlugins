        ...
        if(game_mode_index != 1) // not GAME_MODE_PVP
        {
            ...
            bool fetch_pvp_lobbies() {
                ...
                snprintf(
                    directory_path,
                    sizeof(directory_path),
                    STORAGE_EXT_PATH_PREFIX "/apps_data/flip_world/pvp");
                storage_common_mkdir(storage, directory_path);
                snprintf(
                    directory_path,
                    sizeof(directory_path),
                ...
                return flipper_http_request(
                    fhttp,
                    GET,
                    "https://www.jblanked.com/flipper/api/world/pvp/lobbies/2/10/",
                    "{\"Content-Type\":\"application/json\"}",
                    NULL);
            }
            ...
            bool parse_pvp_lobbies() {
                ...
                for(uint32_t i = 0; i < 10; i++) {
                    ...
                    submenu_add_item(
                        ...
                        callback_submenu_lobby_choices,
                        app);
...
bool game_fetch_lobby(FlipperHTTP* fhttp, char* lobby_name) {
    ...
    snprintf(
        url,
        sizeof(url),
        "https://www.jblanked.com/flipper/api/world/pvp/lobby/get/%s/%s/",
        lobby_name,
        username);
    snprintf(
        fhttp->file_path,
