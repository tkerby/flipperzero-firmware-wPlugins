...
static int32_t game_app(void* p) {
    ...
    settings.always_backlight =
        strstr(game_screen_always_on_choices[game_screen_always_on_index], "Yes") != NULL;
    ...
    if(!app->variable_item_list) {
        ...
        case FlipWorldSubmenuIndexGameSettings:
            ...
            if(!app->variable_item_game_screen_always_on) {
                ...
                variable_item_set_current_value_text(
                    app->variable_item_game_screen_always_on, game_screen_always_on_choices[1]);
            }
            char _game_fps[8];
            ...
            if(load_char(
                ...
                int index = strcmp(_game_screen_always_on, "No") == 0  ? 0 :
                            strcmp(_game_screen_always_on, "Yes") == 0 ? 1 :
                                                                         0;
                variable_item_set_current_value_text(
                    app->variable_item_game_screen_always_on,
                    game_screen_always_on_choices[index]);
...
static bool flip_world_fetch_world_list(DataLoaderModel* model) {
    if(model->request_index == 0) {
        ...
        return flipper_http_get_request_with_headers(
            "https://www.flipsocial.net/api/world/list/10/",
        ...
        if(world_exists(furi_string_get_cstr(first_world))) {
            ...
            if(!thread) {
                ...
                easy_flipper_dialog(
                    "Error", "Failed to allocate game thread. Restart your Flipper.");
                return false;
            }
        ...
        snprintf(
            url,
            sizeof(url),
            "https://www.flipsocial.net/api/world/get/world/%s/",
            furi_string_get_cstr(first_world));
        furi_string_free(world_list);
        furi_string_free(first_world);
        return flipper_http_get_request_with_headers(
            ...
            if(fhttp.last_response != NULL &&
               (strstr(fhttp.last_response, "[SUCCESS]") != NULL ||
                ...
                if(!load_char("Flip-Social-Username", username, sizeof(username))) {
                    ...
                    view_dispatcher_switch_to_view(
                        app_instance->view_dispatcher, FlipWorldViewSubmenu);
                    flipper_http_deinit();
...
static bool flip_world_fetch_worlds(DataLoaderModel* model) {
    ...
    fhttp.save_received_data = true;
    return flipper_http_get_request_with_headers(
        "https://www.flipsocial.net/api/world/get/10/", "{\"Content-Type\":\"application/json\"}");
}
