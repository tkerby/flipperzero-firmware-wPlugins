...
FlipWorldApp::FlipWorldApp() {
    ...
    if(!easy_flipper_set_submenu(
           &submenu, FlipWorldViewSubmenu, VERSION_TAG, callbackExitApp, &viewDispatcher)) {
...
void FlipWorldApp::callbackSubmenuChoices(uint32_t index) {
    ...
    case FlipWorldSubmenuRun:
        ...
        if(!hasUserCredentials()) {
            easy_flipper_dialog(
                "No User Credentials", "Please set your User Name\nand Password in Settings.");
            return;
        }
...
bool FlipWorldApp::loadFileChunk(
    const char* filePath,
    char* buffer,
    size_t sizeOfChunk,
    uint8_t iteration) {
    ...
    if(run) {
        if(run->isActive()) {
            ...
            view_dispatcher_switch_to_view(
                app->viewDispatcher, FlipWorldViewSubmenu); // Switch back to the submenu view
...
bool FlipWorldApp::websocketStart(const char* url) {
    ...
    return flipper_http_websocket_start(
        flipperHttp, url, 80, "{\"Content-Type\":\"application/json\"}");
}
