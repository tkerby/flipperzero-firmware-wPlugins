...
public:
    ...
    bool fileExists(const char* path_name)
        const noexcept; // check if a file exists in the app's data directory
    ...
    const char* getLastResponse() const noexcept {
        return flipperHttp ? flipperHttp->last_response : nullptr;
    } // get the last response from the HTTP request
