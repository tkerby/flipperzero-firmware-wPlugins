...
void save_affine_decrypt_result(const char* text) {
    ...
    if(storage_file_open(
           file, APP_DATA_PATH("affine_decrypt.txt"), FSAM_WRITE, FSOM_CREATE_ALWAYS)) {
        storage_file_write(file, text, strlen(text));
    }
    storage_file_close(file);
    storage_file_free(file);
...
void save_atbash_decrypt_result(const char* text) {
    File* file = storage_file_alloc(furi_record_open(RECORD_STORAGE));
    if(storage_file_open(
           file, APP_DATA_PATH("atbash_decrypt.txt"), FSAM_WRITE, FSOM_CREATE_ALWAYS)) {
        storage_file_write(file, text, strlen(text));
    }
    storage_file_close(file);
    storage_file_free(file);
...
void save_baconian_decrypt_result(const char* text) {
    File* file = storage_file_alloc(furi_record_open(RECORD_STORAGE));
    if(storage_file_open(
           file, APP_DATA_PATH("baconian_decrypt.txt"), FSAM_WRITE, FSOM_CREATE_ALWAYS)) {
        storage_file_write(file, text, strlen(text));
    }
    storage_file_close(file);
    storage_file_free(file);
...
void save_beaufort_decrypt_result(const char* text) {
    File* file = storage_file_alloc(furi_record_open(RECORD_STORAGE));
    if(storage_file_open(
           file, APP_DATA_PATH("beaufort_decrypt.txt"), FSAM_WRITE, FSOM_CREATE_ALWAYS)) {
        storage_file_write(file, text, strlen(text));
    }
    storage_file_close(file);
    storage_file_free(file);
...
void save_caesar_decrypt_result(const char* text) {
    File* file = storage_file_alloc(furi_record_open(RECORD_STORAGE));
    if(storage_file_open(
           file, APP_DATA_PATH("caesar_decrypt.txt"), FSAM_WRITE, FSOM_CREATE_ALWAYS)) {
        storage_file_write(file, text, strlen(text));
    }
    storage_file_close(file);
    storage_file_free(file);
...
void save_playfair_decrypt_result(const char* text) {
    File* file = storage_file_alloc(furi_record_open(RECORD_STORAGE));
    if(storage_file_open(
           file, APP_DATA_PATH("playfair_decrypt.txt"), FSAM_WRITE, FSOM_CREATE_ALWAYS)) {
        storage_file_write(file, text, strlen(text));
    }
    storage_file_close(file);
    storage_file_free(file);
...
void save_polybius_decrypt_result(const char* text) {
    File* file = storage_file_alloc(furi_record_open(RECORD_STORAGE));
    if(storage_file_open(
           file, APP_DATA_PATH("polybius_decrypt.txt"), FSAM_WRITE, FSOM_CREATE_ALWAYS)) {
        storage_file_write(file, text, strlen(text));
    }
    storage_file_close(file);
    storage_file_free(file);
...
void save_railfence_decrypt_result(const char* text) {
    File* file = storage_file_alloc(furi_record_open(RECORD_STORAGE));
    if(storage_file_open(
           file, APP_DATA_PATH("railfence_decrypt.txt"), FSAM_WRITE, FSOM_CREATE_ALWAYS)) {
        storage_file_write(file, text, strlen(text));
    }
    storage_file_close(file);
    storage_file_free(file);
...
void save_scytale_decrypt_result(const char* text) {
    File* file = storage_file_alloc(furi_record_open(RECORD_STORAGE));
    if(storage_file_open(
           file, APP_DATA_PATH("scytale_decrypt.txt"), FSAM_WRITE, FSOM_CREATE_ALWAYS)) {
        storage_file_write(file, text, strlen(text));
    }
    storage_file_close(file);
    storage_file_free(file);
...
void save_vigenere_decrypt_result(const char* text) {
    File* file = storage_file_alloc(furi_record_open(RECORD_STORAGE));
    if(storage_file_open(
           file, APP_DATA_PATH("vigenere_decrypt.txt"), FSAM_WRITE, FSOM_CREATE_ALWAYS)) {
        storage_file_write(file, text, strlen(text));
    }
    storage_file_close(file);
    storage_file_free(file);
...
void save_base32_decrypt_result(const char* text) {
    File* file = storage_file_alloc(furi_record_open(RECORD_STORAGE));
    if(storage_file_open(
           file, APP_DATA_PATH("base32_decrypt.txt"), FSAM_WRITE, FSOM_CREATE_ALWAYS)) {
        storage_file_write(file, text, strlen(text));
    }
    storage_file_close(file);
    storage_file_free(file);
...
void save_base58_decrypt_result(const char* text) {
    File* file = storage_file_alloc(furi_record_open(RECORD_STORAGE));
    if(storage_file_open(
           file, APP_DATA_PATH("base58_decrypt.txt"), FSAM_WRITE, FSOM_CREATE_ALWAYS)) {
        storage_file_write(file, text, strlen(text));
    }
    storage_file_close(file);
    storage_file_free(file);
...
void save_base64_decrypt_result(const char* text) {
    File* file = storage_file_alloc(furi_record_open(RECORD_STORAGE));
    if(storage_file_open(
           file, APP_DATA_PATH("base64_decrypt.txt"), FSAM_WRITE, FSOM_CREATE_ALWAYS)) {
        storage_file_write(file, text, strlen(text));
    }
    storage_file_close(file);
    storage_file_free(file);
...
char* load_affine_decrypt() {
    ...
    char* result = NULL;
    if(storage_file_open(
           file, APP_DATA_PATH("affine_decrypt.txt"), FSAM_READ, FSOM_OPEN_EXISTING)) {
        size_t file_size = storage_file_size(file);
        if(file_size > 0) {
            result = malloc(file_size + 1);
            storage_file_read(file, result, file_size);
...
char* load_atbash_decrypt() {
    ...
    char* result = NULL;
    if(storage_file_open(
           file, APP_DATA_PATH("atbash_decrypt.txt"), FSAM_READ, FSOM_OPEN_EXISTING)) {
        size_t file_size = storage_file_size(file);
        if(file_size > 0) {
            result = malloc(file_size + 1);
            storage_file_read(file, result, file_size);
...
char* load_baconian_decrypt() {
    ...
    char* result = NULL;
    if(storage_file_open(
           file, APP_DATA_PATH("baconian_decrypt.txt"), FSAM_READ, FSOM_OPEN_EXISTING)) {
        size_t file_size = storage_file_size(file);
        if(file_size > 0) {
            result = malloc(file_size + 1);
            storage_file_read(file, result, file_size);
...
char* load_beaufort_decrypt() {
    ...
    char* result = NULL;
    if(storage_file_open(
           file, APP_DATA_PATH("beaufort_decrypt.txt"), FSAM_READ, FSOM_OPEN_EXISTING)) {
        size_t file_size = storage_file_size(file);
        if(file_size > 0) {
            result = malloc(file_size + 1);
            storage_file_read(file, result, file_size);
...
char* load_caesar_decrypt() {
    ...
    char* result = NULL;
    if(storage_file_open(
           file, APP_DATA_PATH("caesar_decrypt.txt"), FSAM_READ, FSOM_OPEN_EXISTING)) {
        size_t file_size = storage_file_size(file);
        if(file_size > 0) {
            result = malloc(file_size + 1);
            storage_file_read(file, result, file_size);
...
char* load_playfair_decrypt() {
    ...
    char* result = NULL;
    if(storage_file_open(
           file, APP_DATA_PATH("playfair_decrypt.txt"), FSAM_READ, FSOM_OPEN_EXISTING)) {
        size_t file_size = storage_file_size(file);
        if(file_size > 0) {
            result = malloc(file_size + 1);
            storage_file_read(file, result, file_size);
...
char* load_polybius_decrypt() {
    ...
    char* result = NULL;
    if(storage_file_open(
           file, APP_DATA_PATH("polybius_decrypt.txt"), FSAM_READ, FSOM_OPEN_EXISTING)) {
        size_t file_size = storage_file_size(file);
        if(file_size > 0) {
            result = malloc(file_size + 1);
            storage_file_read(file, result, file_size);
...
char* load_railfence_decrypt() {
    ...
    char* result = NULL;
    if(storage_file_open(
           file, APP_DATA_PATH("railfence_decrypt.txt"), FSAM_READ, FSOM_OPEN_EXISTING)) {
        size_t file_size = storage_file_size(file);
        if(file_size > 0) {
            result = malloc(file_size + 1);
            storage_file_read(file, result, file_size);
...
char* load_scytale_decrypt() {
    ...
    char* result = NULL;
    if(storage_file_open(
           file, APP_DATA_PATH("scytale_decrypt.txt"), FSAM_READ, FSOM_OPEN_EXISTING)) {
        size_t file_size = storage_file_size(file);
        if(file_size > 0) {
            result = malloc(file_size + 1);
            storage_file_read(file, result, file_size);
...
char* load_vigenere_decrypt() {
    ...
    char* result = NULL;
    if(storage_file_open(
           file, APP_DATA_PATH("vigenere_decrypt.txt"), FSAM_READ, FSOM_OPEN_EXISTING)) {
        size_t file_size = storage_file_size(file);
        if(file_size > 0) {
            result = malloc(file_size + 1);
            storage_file_read(file, result, file_size);
...
char* load_base32_decrypt() {
    ...
    char* result = NULL;
    if(storage_file_open(
           file, APP_DATA_PATH("base32_decrypt.txt"), FSAM_READ, FSOM_OPEN_EXISTING)) {
        size_t file_size = storage_file_size(file);
        if(file_size > 0) {
            result = malloc(file_size + 1);
            storage_file_read(file, result, file_size);
...
char* load_base58_decrypt() {
    ...
    char* result = NULL;
    if(storage_file_open(
           file, APP_DATA_PATH("base58_decrypt.txt"), FSAM_READ, FSOM_OPEN_EXISTING)) {
        size_t file_size = storage_file_size(file);
        if(file_size > 0) {
            result = malloc(file_size + 1);
            storage_file_read(file, result, file_size);
...
char* load_base64_decrypt() {
    ...
    char* result = NULL;
    if(storage_file_open(
           file, APP_DATA_PATH("base64_decrypt.txt"), FSAM_READ, FSOM_OPEN_EXISTING)) {
        size_t file_size = storage_file_size(file);
        if(file_size > 0) {
            result = malloc(file_size + 1);
            storage_file_read(file, result, file_size);
