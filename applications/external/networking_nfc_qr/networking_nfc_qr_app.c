...
static bool load_qr_code(NfcQrApp* app) {
    ...
    do {
        ...
        if(qrcode_initBytes(
               ...
               version,
               ECC_MEDIUM,
               (uint8_t*)furi_string_get_cstr(temp_str),
               furi_string_size(temp_str)) != 0) {
        ...
        if(app->emulate_view) {
            with_view_model(
                app->emulate_view, NfcQrAppModel * model, { model->qrcode = app->qrcode; }, true);
        }
    } while(false);
...
static bool custom_event_callback(void* context, uint32_t event) {
    ...
    case NfcQrCustomEventMenuSelectQr: {
        ...
        dialog_file_browser_show(
            app->dialogs, app->qr_file_path, app->qr_file_path, &browser_options);
        load_qr_code(app);
        return true;
    }
    case NfcQrCustomEventMenuSelectNfc: {
        ...
        dialog_file_browser_show(
            app->dialogs, app->nfc_file_path, app->nfc_file_path, &browser_options);
        return true;
    }
...
int32_t networking_nfc_qr_app_entry(void* p) {
    ...
    view_dispatcher_add_view(
        app->view_dispatcher, NfcQrAppViewSubmenu, submenu_get_view(app->submenu));

    // Widget Setup (Credits)
    app->widget = widget_alloc();
    widget_add_text_scroll_element(
        app->widget, 0, 0, 128, 64, "made by: DonJulve\nGitHub: DonJulve\nLinkedIn: julve");
    view_dispatcher_add_view(
        app->view_dispatcher, NfcQrAppViewCredits, widget_get_view(app->widget));
