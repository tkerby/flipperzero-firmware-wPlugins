...
static void morse_app_draw_callback(Canvas* canvas, void* ctx) {
    ...
    case MorseStateMenu: {
        ...
        // Draw menu items
        const char* menu_items[] = {"Learn Morse", "Practice", "Help", "Exit"};
    ...
    case MorseStatePractice: {
        ...
        for(int i = 0; i < 10; i++) {
            if(i < volume_bars) {
                snprintf(
                    volume_str + strlen(volume_str), sizeof(volume_str) - strlen(volume_str), "|");
            } else {
                snprintf(
                    volume_str + strlen(volume_str), sizeof(volume_str) - strlen(volume_str), " ");
            }
...
static void morse_app_input_callback(InputEvent* input_event, void* ctx) {
    ...
    case MorseStateMenu:
        if(input_event->key == InputKeyUp && input_event->type == InputTypeShort) {
            app->menu_selection = (app->menu_selection > 0) ? app->menu_selection - 1 : 3;
        } else if(input_event->key == InputKeyDown && input_event->type == InputTypeShort) {
            app->menu_selection = (app->menu_selection < 3) ? app->menu_selection + 1 : 0;
