...
VideoGameModuleConsoleApp::VideoGameModuleConsoleApp() {
    ...
    view_dispatcher_add_view(
        viewDispatcher, VideoGameModuleConsoleViewSubmenu, submenu_get_view(submenu));

    submenu_add_item(
        submenu, "Run", VideoGameModuleConsoleSubmenuRun, submenuChoicesCallback, this);
    submenu_add_item(
        submenu, "About", VideoGameModuleConsoleSubmenuAbout, submenuChoicesCallback, this);
...
void VideoGameModuleConsoleApp::httpLineCallback(const char* line, void* context) {
    ...
    if(!app || !app->responseQueue || !line || line[0] == '\0') return;
    ...
    if(furi_message_queue_put(app->responseQueue, &copy, 0) != FuriStatusOk) {
        ...
        if(furi_message_queue_put(app->responseQueue, &copy, 0) != FuriStatusOk) free(copy);
    }
}

char* VideoGameModuleConsoleApp::popHttpResponse() {
    if(!responseQueue) return nullptr;
    char* line = nullptr;
    if(furi_message_queue_get(responseQueue, &line, 0) == FuriStatusOk) return line;
    return nullptr;
}
...
bool VideoGameModuleConsoleApp::loadChar(
...
bool VideoGameModuleConsoleApp::saveChar(
    const char* path_name,
    const char* value,
    const char* appId,
    bool overwrite) {
    ...
    if(!storage_file_open(
           file, file_path, FSAM_WRITE, overwrite ? FSOM_CREATE_ALWAYS : FSOM_OPEN_APPEND)) {
        storage_file_free(file);
        furi_record_close(RECORD_STORAGE);
        return false;
    }
