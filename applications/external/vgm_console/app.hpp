...
private:
    ...
    FuriMessageQueue* responseQueue =
        nullptr; // Queue of char* draw-command lines (one per UART line)
    ...
    static bool
        customEventCallback(void* context, uint32_t event); // View dispatcher custom event callback
    void createAppDataPath(const char* appId = APP_ID); // Create the app data path in storage
    static void httpLineCallback(
        const char* line,
        void* context); // Per-line UART callback; enqueues draw commands
    static int32_t
        runThreadCallback(void* context); // Run thread: draws frames and processes input
    static void
        submenuChoicesCallback(void* context, uint32_t index); // Callback for submenu choices
    static void viewPortInputEnqueue(
        InputEvent* event,
        void* context); // Enqueues input events for the run thread
...
public:
    ...
    ViewPort* viewPort =
        nullptr; // ViewPort in GuiLayerFullscreen to capture input during run (blocks view dispatcher)
    //
    void clearHttpBuffer(); // clear the HTTP buffer (for bytes requests)
    void clearHttpResponse(); // clear the last HTTP response
    bool getHttpBuffer(
        uint8_t* buffer,
        size_t buffer_size); // get the buffer of the last response (for bytes requests)
    const char* getHttpResponse() const noexcept {
        return flipperHttp ? flipperHttp->last_response : nullptr;
    } // get the last HTTP response (single-shot; may be stale)
    char*
        popHttpResponse(); // pop one queued UART line (caller must free); returns nullptr if queue empty
    bool loadChar(
