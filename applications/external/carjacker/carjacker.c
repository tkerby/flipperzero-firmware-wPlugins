...
static void draw_main_scene(Canvas* canvas, CarJackerApp* app) {
    ...
    if(app->steal_mode && app->car_count > 0) {
        ...
        if(app->car_count > 0) {
            ...
            for(uint8_t i = 0; i < visible; i++) {
                ...
                if(idx < app->car_count) {
                    canvas_draw_str_aligned(canvas, 64, 20 + i * 7, AlignCenter, AlignBottom, app->captured_cars[idx]);
                }
            }
        ...
        if(app->scanning) {
            ...
            canvas_draw_str_aligned(canvas, 64, 58, AlignCenter, AlignBottom, scan_frames[app->scan_anim_frame % 8]);
    ...
    while(app->running) {
        ...
        if(app->scanning) {
            ...
            if(elapsed_since_last >= detect_interval && app->car_count < MAX_CARS) {
                ...
                const char* selected = is_real
                    ? high_end_cars[rand() % 20]
                    : silly_cars[rand() % 17];
