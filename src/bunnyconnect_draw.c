...
// Error icon 10x10 data
static const uint8_t I_Error_10x10_data[] = {0x01, 0xC0, 0x03, 0x60, 0x06, 0x30, 0x0C,
                                             0x18, 0x08, 0x08, 0x08, 0x08, 0x0C, 0x18,
                                             0x06, 0x30, 0x03, 0x60, 0x01, 0xC0};

// Loading animation data
static const uint8_t I_Loading_24_data[] = {
    0x00, 0x00, 0x00, 0x00, 0x80, 0x01, 0x00, 0xE0, 0x07, 0x00, 0xF0, 0x0F, 0x00, 0xF8, 0x1F,
    0x00, 0x7C, 0x3E, 0x00, 0x3E, 0x7C, 0x00, 0x1F, 0xF8, 0x00, 0x0F, 0xF0, 0x00, 0x07, 0xE0,
    0x00, 0x03, 0xC0, 0x80, 0x01, 0x80, 0xC0, 0x00, 0x00, 0xE0, 0x00, 0x00, 0x60, 0x00, 0x00,
    0x60, 0x00, 0x00, 0xE0, 0x00, 0x00, 0xC0, 0x00, 0x80, 0x80, 0x01, 0x00, 0x03, 0xC0, 0x00,
    0x07, 0xE0, 0x00, 0x0F, 0xF0, 0x00, 0x1F, 0xF8, 0x00, 0x3E, 0x7C, 0x00, 0x7C, 0x3E, 0x00,
    0xF8, 0x1F, 0x00, 0xF0, 0x0F, 0x00, 0xE0, 0x07, 0x00, 0x80, 0x01, 0x00, 0x00, 0x00};

// Error icon 18x18 data
static const uint8_t I_Error_18x18_data[] = {
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xE0, 0x03, 0x00, 0xF8, 0x0F, 0x00, 0xFE, 0x3F,
    0x00, 0xFF, 0x7F, 0x80, 0xFF, 0xFF, 0x80, 0xFF, 0xFF, 0xC0, 0xFF, 0xFF, 0xC0, 0xFF, 0xFF,
    0xC0, 0xFF, 0xFF, 0xC0, 0xFF, 0xFF, 0x80, 0xFF, 0xFF, 0x80, 0xFF, 0xFF, 0x00, 0xFE, 0x3F,
    0x00, 0xF8, 0x0F, 0x00, 0xE0, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

// Define frames arrays
static const uint8_t* const I_Error_10x10_frames[] = {I_Error_10x10_data};
static const uint8_t* const I_Loading_24_frames[] = {I_Loading_24_data};
static const uint8_t* const I_Error_18x18_frames[] = {I_Error_18x18_data};

// Create icon instances - verify frames pointers are not NULL
const Icon I_Error_10x10 =
    {.width = 10, .height = 10, .frame_count = 1, .frame_rate = 0, .frames = I_Error_10x10_frames};

const Icon I_Loading_24 =
    {.width = 24, .height = 24, .frame_count = 1, .frame_rate = 0, .frames = I_Loading_24_frames};

const Icon I_Error_18x18 =
    {.width = 18, .height = 18, .frame_count = 1, .frame_rate = 0, .frames = I_Error_18x18_frames};
...
void bunnyconnect_draw_loading_animation(Canvas* canvas, uint8_t frame, uint8_t x, uint8_t y) {
    ...
    // Calculate angle based on frame - use float literals
    float angle = (float)frame * (2.0f * 3.14159f / frame_count);
    int8_t end_x = x + (int8_t)(sinf(angle) * 5.0f);
    int8_t end_y = y + (int8_t)(cosf(angle) * 5.0f);
