...
static void show_menu(AppState *state, const MenuCommand *commands,
                      size_t command_count, const char *header, Submenu *menu,
                      uint8_t view_id);
...
static const CyclingMenuDef beacon_spam_commands[] = {
    {"< Beacon Spam (List) >", "beaconspam -l\n", "Beacon Spam (List)",
     "Spam SSIDs from list.", false, NULL},
    {"< Beacon Spam (Random) >", "beaconspam -r\n", "Beacon Spam (Random)",
     "Spam random SSIDs.", false, NULL},
    {"< Beacon Spam (Rickroll) >", "beaconspam -rr\n", "Beacon Spam (Rickroll)",
     "Spam Rickroll SSIDs.", false, NULL},
    {"< Beacon Spam (Custom) >", "beaconspam", "Beacon Spam (Custom)",
     "Spam custom SSID.", true, "SSID Name"},
};

// BLE spam command definitions
static const CyclingMenuDef ble_spam_commands[] = {
    {"< BLE Spam (Apple) >", "blespam -apple\n", "BLE Spam (Apple)",
     "Spam Apple BLE devices.", false, NULL},
    {"< BLE Spam (Microsoft) >", "blespam -ms\n", "BLE Spam (Microsoft)",
     "Spam Microsoft BLE devices.", false, NULL},
    {"< BLE Spam (Samsung) >", "blespam -samsung\n", "BLE Spam (Samsung)",
     "Spam Samsung BLE devices.", false, NULL},
    {"< BLE Spam (Google) >", "blespam -google\n", "BLE Spam (Google)",
     "Spam Google BLE devices.", false, NULL},
    {"< BLE Spam (Random) >", "blespam -random\n", "BLE Spam (Random)",
     "Spam random BLE devices.", false, NULL},
};
...
static const CyclingMenuDef rgbmode_commands[] = {
    {"< LED: Rainbow >", "rgbmode rainbow\n", "LED: Rainbow",
     "Cycle rainbow colors.", false, NULL},
    {"< LED: Police >", "rgbmode police\n", "LED: Police",
     "Police light effect.", false, NULL},
    {"< LED: Strobe >", "rgbmode strobe\n", "LED: Strobe",
     "Strobe light effect.", false, NULL},
    {"< LED: Off >", "rgbmode off\n", "LED: Off", "Turn off LED.", false, NULL},
    {"< LED: Red >", "rgbmode red\n", "LED: Red", "Set LED to red.", false,
     NULL},
    {"< LED: Green >", "rgbmode green\n", "LED: Green", "Set LED to green.",
     false, NULL},
    {"< LED: Blue >", "rgbmode blue\n", "LED: Blue", "Set LED to blue.", false,
     NULL},
    {"< LED: Yellow >", "rgbmode yellow\n", "LED: Yellow", "Set LED to yellow.",
     false, NULL},
    {"< LED: Purple >", "rgbmode purple\n", "LED: Purple", "Set LED to purple.",
     false, NULL},
    {"< LED: Cyan >", "rgbmode cyan\n", "LED: Cyan", "Set LED to cyan.", false,
     NULL},
    {"< LED: Orange >", "rgbmode orange\n", "LED: Orange", "Set LED to orange.",
     false, NULL},
    {"< LED: White >", "rgbmode white\n", "LED: White", "Set LED to white.",
     false, NULL},
    {"< LED: Pink >", "rgbmode pink\n", "LED: Pink", "Set LED to pink.", false,
     NULL},
};

static const CyclingMenuDef wifi_scan_modes[] = {
    {"< Scan: (APs) >", "scanap\n", "WiFi AP Scanner", "Scans for WiFi APs...",
     false, NULL},
    ...
    {"< Scan: (Stations) >", "scansta\n", "Station Scanner",
     "Scans for clients...", false, NULL},
    {"< Scan: (AP+STA) >", "scanall\n", "Scan All",
     "Combined AP/Station scan...", false, NULL},
};

static const CyclingMenuDef wifi_list_modes[] = {
    {"< List: (APs) >", "list -a\n", "List Access Points",
     "Shows list of APs found during last scan.", false, NULL},
    {"< List: (Stations) >", "list -s\n", "List Stations",
     "Shows list of clients found during last scan.", false, NULL},
};

static const CyclingMenuDef wifi_select_modes[] = {
    {"< Select: (AP) >", "select -a", "Select Access Point",
     "Select an AP by number from the scanned list.", true, "AP Number"},
    {"< Select: (Station) >", "select -s", "Select Station",
     "Target a station by number from the scan list for attacks.", true,
     "Station Number"},
};

static const CyclingMenuDef wifi_listen_modes[] = {
    {"< Listen Probes (Hop) >", "listenprobes\n", "Listen for Probes",
     "Listen for and log probe requests\nwhile hopping channels.", false, NULL},
    {"< Listen Probes (Chan) >", "listenprobes", "Listen on Channel",
     "Listen for probe requests on a\nspecific channel.", true,
     "Channel (1-165)"},
};
...
static bool cycle_menu_item(CyclingMenuDef *cycling_array, size_t cycling_count,
                            size_t *current_index, MenuCommand *menu_commands,
                            size_t menu_index, Submenu *menu,
                            InputEvent *event) {
  ...
  if (event->key == InputKeyRight) {
    ...
    *current_index =
        (*current_index == 0) ? (cycling_count - 1) : (*current_index - 1);
  }
  submenu_change_item_label(menu, menu_index,
                            cycling_array[*current_index].label);
...
void send_uart_command_with_text(const char *command, char *text,
                                 AppState *state) {
  char buffer[256];
...
void send_uart_command_with_bytes(const char *command, const uint8_t *bytes,
                                  size_t length, AppState *state) {
...
static void confirmation_ok_callback(void *context) {
  ...
  if (cmd_ctx && cmd_ctx->state && cmd_ctx->command) {
    ...
    if (cmd_ctx->command->capture_prefix || cmd_ctx->command->file_ext ||
      ...
      FURI_LOG_I(
          "Capture",
          "Attempting to open PCAP file before sending capture command.");
      file_opened = uart_receive_data(
          cmd_ctx->state->uart_context, cmd_ctx->state->view_dispatcher,
          cmd_ctx->state,
      ...
      if (!file_opened) {
        FURI_LOG_E("Capture",
                   "Failed to open PCAP file. Aborting capture command.");
        free(cmd_ctx);
        return;
      }
      ...
      // view
      send_uart_command(cmd_ctx->command->command, cmd_ctx->state);
      uart_receive_data(cmd_ctx->state->uart_context,
                        cmd_ctx->state->view_dispatcher, cmd_ctx->state, "", "",
...
static void show_command_details(AppState *state, const MenuCommand *command) {
  ...
  confirmation_view_set_header(state->confirmation_view,
                               command->details_header);
...
static void text_input_result_callback(void *context) {
  ...
  if (input_state->connect_input_stage == 1) {
    ...
    if (len >= sizeof(input_state->connect_ssid))
      len = sizeof(input_state->connect_ssid) - 1;
    ...
    if (input_state->input_buffer)
      memset(input_state->input_buffer, 0, INPUT_BUFFER_SIZE);
    text_input_reset(input_state->text_input);
    text_input_set_header_text(input_state->text_input, "PASSWORD");
    text_input_set_result_callback(
        input_state->text_input, text_input_result_callback, input_state,
        input_state->input_buffer, INPUT_BUFFER_SIZE, true);
  ...
  if (input_state->connect_input_stage == 2) {
    ...
    send_uart_command_with_text(input_state->uart_command,
                                input_state->input_buffer, input_state);
  }
  uart_receive_data(input_state->uart_context, input_state->view_dispatcher,
                    input_state, "", "", "");
  if (input_state->input_buffer)
    memset(input_state->input_buffer, 0, INPUT_BUFFER_SIZE);
}
...
static void send_evil_portal_html(AppState *state) {
  ...
  if (ghost_esp_ep_read_html_file(state, &the_html, &html_size)) {
    if (the_html != NULL) {
      ...
      uart_send(state->uart_context, (const uint8_t *)command_str,
                strlen(command_str));
...
static void execute_menu_command(AppState *state, const MenuCommand *command) {
  ...
  if (!uart_is_esp_connected(state->uart_context)) {
    ...
    confirmation_view_set_ok_callback(state->confirmation_view, error_callback,
                                      state);
    confirmation_view_set_cancel_callback(state->confirmation_view,
  ...
  if (command->needs_input && strcmp(command->command, "connect") == 0) {
    ...
    text_input_set_result_callback(
        state->text_input, text_input_result_callback, state,
        state->input_buffer, INPUT_BUFFER_SIZE, true);
  ...
  if (command->needs_confirmation) {
    ...
    confirmation_view_set_ok_callback(state->confirmation_view,
                                      confirmation_ok_callback, cmd_ctx);
  ...
  if (state->current_view == 11 && state->current_index == 0) {
    ...
    if (current_sniff->capture_prefix) {
      ...
      bool file_opened = uart_receive_data(
          state->uart_context, state->view_dispatcher, state,
          current_sniff->capture_prefix, "pcap", GHOST_ESP_APP_FOLDER_PCAPS);
  ...
  if (state->current_view == 12 && state->current_index == 0) {
    const CyclingMenuDef *current_beacon =
        &beacon_spam_commands[current_beacon_index];
  ...
  if (state->current_view == 22 && state->current_index == 0) {
    const CyclingMenuDef *current_ble_spam =
        &ble_spam_commands[current_ble_spam_index];
    // Save view and show terminal log
    state->previous_view = state->current_view;
    uart_receive_data(state->uart_context, state->view_dispatcher, state, "",
                      "", "");
...
void show_wifi_scanning_menu(AppState *state) {
  show_menu(state, wifi_scanning_commands, COUNT_OF(wifi_scanning_commands),
            "Scanning & Probing", state->wifi_scanning_menu, 10);
}

void show_wifi_capture_menu(AppState *state) {
  show_menu(state, wifi_capture_commands, COUNT_OF(wifi_capture_commands),
            "Packet Capture", state->wifi_capture_menu, 11);
}

void show_wifi_attack_menu(AppState *state) {
  show_menu(state, wifi_attack_commands, COUNT_OF(wifi_attack_commands),
            "Attacks", state->wifi_attack_menu, 12);
}

void show_wifi_network_menu(AppState *state) {
  show_menu(state, wifi_network_commands, COUNT_OF(wifi_network_commands),
            "Portal & Network", state->wifi_network_menu, 13);
}

void show_wifi_settings_menu(AppState *state) {
  show_menu(state, wifi_settings_commands, COUNT_OF(wifi_settings_commands),
            "Settings & Hardware", state->wifi_settings_menu, 14);
}

void show_ble_scanning_menu(AppState *state) {
  show_menu(state, ble_scanning_commands, COUNT_OF(ble_scanning_commands),
            "Scanning & Detection", state->ble_scanning_menu, 20);
}

void show_ble_capture_menu(AppState *state) {
  show_menu(state, ble_capture_commands, COUNT_OF(ble_capture_commands),
            "Packet Capture", state->ble_capture_menu, 21);
}

void show_ble_attack_menu(AppState *state) {
  show_menu(state, ble_attack_commands, COUNT_OF(ble_attack_commands),
            "Attacks & Spoofing", state->ble_attack_menu, 22);
}

void show_wifi_menu(AppState *state) {
  submenu_reset(state->wifi_menu);
  submenu_set_header(state->wifi_menu, "WiFi Commands");
  submenu_add_item(state->wifi_menu, "Scanning & Probing > ", 0,
                   submenu_callback, state);
  submenu_add_item(state->wifi_menu, "Packet Capture > ", 1, submenu_callback,
                   state);
  submenu_add_item(state->wifi_menu, "Attacks > ", 2, submenu_callback, state);
  submenu_add_item(state->wifi_menu, "Evil Portal & Network >", 3,
                   submenu_callback, state);
  submenu_add_item(state->wifi_menu, wifi_stop_command.label, 4,
                   submenu_callback, state);
...
void show_ble_menu(AppState *state) {
  ...
  submenu_add_item(state->ble_menu, "Scanning & Detection >", 0,
                   submenu_callback, state);
  submenu_add_item(state->ble_menu, "Packet Capture >", 1, submenu_callback,
                   state);
  submenu_add_item(state->ble_menu, "Attacks & Spoofing >", 2, submenu_callback,
                   state);
  submenu_add_item(state->ble_menu, ble_stop_command.label, 3, submenu_callback,
                   state);
...
bool back_event_callback(void *context) {
  ...
  if (current_view == 5) {
    ...
    if (state->previous_view == 8 || state->previous_view == 4) {
      ...
      case 1:
        show_wifi_menu(state);
        submenu_set_selected_item(state->wifi_menu,
                                  state->last_wifi_category_index);
        break;
      case 10:
        show_wifi_scanning_menu(state);
        submenu_set_selected_item(state->wifi_scanning_menu,
                                  state->last_wifi_scanning_index);
        break;
      case 11:
        show_wifi_capture_menu(state);
        submenu_set_selected_item(state->wifi_capture_menu,
                                  state->last_wifi_capture_index);
        break;
      case 12:
        show_wifi_attack_menu(state);
        submenu_set_selected_item(state->wifi_attack_menu,
                                  state->last_wifi_attack_index);
        break;
      case 13:
        show_wifi_network_menu(state);
        submenu_set_selected_item(state->wifi_network_menu,
                                  state->last_wifi_network_index);
        break;
      case 14:
        show_wifi_settings_menu(state);
        submenu_set_selected_item(state->wifi_settings_menu,
                                  state->last_wifi_settings_index);
        break;
      case 2:
        show_ble_menu(state);
        submenu_set_selected_item(state->ble_menu,
                                  state->last_ble_category_index);
        break;
      case 20:
        show_ble_scanning_menu(state);
        submenu_set_selected_item(state->ble_scanning_menu,
                                  state->last_ble_scanning_index);
        break;
      case 21:
        show_ble_capture_menu(state);
        submenu_set_selected_item(state->ble_capture_menu,
                                  state->last_ble_capture_index);
        break;
      case 22:
        show_ble_attack_menu(state);
        submenu_set_selected_item(state->ble_attack_menu,
                                  state->last_ble_attack_index);
        break;
      case 3:
        show_gps_menu(state);
  ...
  case InputTypeShort:
    ...
    case InputKeyLeft:
      ...
      if (state->current_view == 11 && current_index == 0) {
        ...
        if (event->key == InputKeyRight) {
          current_sniff_index =
              (current_sniff_index + 1) % COUNT_OF(sniff_commands);
        } else {
          current_sniff_index = (current_sniff_index == 0)
                                    ? (size_t)(COUNT_OF(sniff_commands) - 1)
                                    : (current_sniff_index - 1);
        }
        submenu_change_item_label(current_menu, current_index,
                                  sniff_commands[current_sniff_index].label);
        consumed = true;
      }
      // Handle beacon spam command cycling
        ...
        consumed = cycle_menu_item(
            ...
            COUNT_OF(beacon_spam_commands), &current_beacon_index,
            (MenuCommand *)wifi_attack_commands, 0, current_menu, event);
      }
      // Handle rgbmode command cycling (new branch for index 17)
      else if (state->current_view == 14 && current_index == 0) {
        consumed = cycle_menu_item(
            (CyclingMenuDef *)rgbmode_commands, COUNT_OF(rgbmode_commands),
            &current_rgb_index, (MenuCommand *)wifi_settings_commands, 0,
            current_menu, event);
      }
      // Handle BLE spam command cycling
      else if (state->current_view == 22 && current_index == 0) {
        consumed = cycle_menu_item(
            (CyclingMenuDef *)ble_spam_commands, COUNT_OF(ble_spam_commands),
            &current_ble_spam_index, (MenuCommand *)ble_attack_commands, 0,
            current_menu, event);
      }
      // Handle WiFi scan mode cycling
      else if (state->current_view == 10 && current_index == 0) {
        consumed = cycle_menu_item(
            (CyclingMenuDef *)wifi_scan_modes, COUNT_OF(wifi_scan_modes),
            &current_wifi_scan_index, (MenuCommand *)wifi_scanning_commands, 0,
            current_menu, event);
      }
      // List mode cycling
      else if (state->current_view == 10 && current_index == 1) {
        consumed = cycle_menu_item(
            (CyclingMenuDef *)wifi_list_modes, COUNT_OF(wifi_list_modes),
            &current_wifi_list_index, (MenuCommand *)wifi_scanning_commands, 1,
            current_menu, event);
      }
      // Select mode cycling
      else if (state->current_view == 10 && current_index == 2) {
        consumed = cycle_menu_item(
            (CyclingMenuDef *)wifi_select_modes, COUNT_OF(wifi_select_modes),
            &current_wifi_select_index, (MenuCommand *)wifi_scanning_commands,
            2, current_menu, event);
      }
      // Handle listen mode cycling
      else if (state->current_view == 10 && current_index == 3) {
        consumed = cycle_menu_item(
            (CyclingMenuDef *)wifi_listen_modes, COUNT_OF(wifi_listen_modes),
            &current_wifi_listen_index, (MenuCommand *)wifi_scanning_commands,
            3, current_menu, event);
      }
      break;
    case InputKeyMAX:
      break;
