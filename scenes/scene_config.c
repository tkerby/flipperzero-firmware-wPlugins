...
#define ADD_CONFIG_ENTRY(label, name, type, valueIndexFunc, spiTypedefField,   \
                         values, strings)                                      \
...
#define ADD_CONFIG_ENTRY(label, name, type, valueIndexFunc, spiTypedefField,   \
  ...
  static void name##_changed(VariableItem *item) {                             \
    ...
    (variable_item_set_current_value_text)(                                    \
        item, spi_config_##name##_strings[index]);                             \
...
#define ADD_CONFIG_ENTRY(label, name, type, valueIndexFunc, spiTypedefField,   \
                         values, strings)                                      \
  item = variable_item_list_add(app->configScreen, label,                      \
...
static void flipper_spi_terminal_scene_config_alloc_spi_config_items(
...
void display_mode_changed(VariableItem *item) {
  ...
  switch (app->config.print_mode) {
  case FlipperSPITerminalAppPrintModeDynamic:
    variable_item_set_current_value_text(item, "Dynamic");
    break;

  case FlipperSPITerminalAppPrintModeASCII:
    variable_item_set_current_value_text(item, "ASCII");
    break;

  case FlipperSPITerminalAppPrintModeHex:
    variable_item_set_current_value_text(item, "Hex");
    break;

  case FlipperSPITerminalAppPrintModeBinary:
    variable_item_set_current_value_text(item, "Binary");
    break;

  default:
    furi_crash("Bad print mode!");
  }
}

void flipper_spi_terminal_scene_config_on_center_button(void *context,
                                                        uint32_t index) {
  UNUSED(context);
  UNUSED(index);
...
void flipper_spi_terminal_scene_config_alloc(FlipperSPITerminalApp *app) {
  ...
  // Display Mode
  item = variable_item_list_add(app->configScreen, "Display mode",
                                FlipperSPITerminalAppPrintModeMax,
                                display_mode_changed, app);
  ...
  variable_item_list_set_enter_callback(
      app->configScreen, flipper_spi_terminal_scene_config_on_center_button,
      app);
}
...
bool flipper_spi_terminal_scene_config_on_event(void *context,
