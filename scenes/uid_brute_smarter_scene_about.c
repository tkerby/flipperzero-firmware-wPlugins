...
static void uid_brute_smarter_scene_about_draw_callback(Canvas* canvas, void* context) {
    ...
    for(uint8_t i = 0; i < MAX_PARTICLES; i++) {
        if(model->particles[i].life > 0) {
            ...
            canvas_draw_disc(canvas, (uint8_t)model->particles[i].x, (uint8_t)model->particles[i].y, size);
        }
    }
...
View* uid_brute_smarter_scene_about_alloc(void) {
    ...
    view_allocate_model(instance->view, ViewModelTypeLocking, sizeof(UidBruteSmarterSceneAboutModel));
    ...
    instance->timer = furi_timer_alloc(uid_brute_smarter_scene_about_timer_callback, FuriTimerTypePeriodic, instance);
