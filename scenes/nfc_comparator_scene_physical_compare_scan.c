...
void nfc_comparator_physical_compare_scan_scene_on_enter(void* context) {
   ...
   nfc_comparator->reader.worker =
      nfc_comparator_reader_worker_alloc(&nfc_comparator->reader.compare_checks);
   ...
   if(!nfc_comparator_reader_worker_set_loaded_nfc_card(
      ...
      nfc_comparator_reader_worker_start(nfc_comparator->reader.worker);
...
bool nfc_comparator_physical_compare_scan_scene_on_event(void* context, SceneManagerEvent event) {
   ...
   if(event.type == SceneManagerEventTypeBack) {
      force_quit = true;
      nfc_comparator_reader_worker_stop(nfc_comparator->reader.worker);
      scene_manager_search_and_switch_to_previous_scene(
         nfc_comparator->scene_manager, NfcComparatorScene_CompareMenu);
      consumed = true;
   } else if(event.type == SceneManagerEventTypeTick) {
      switch(nfc_comparator_reader_worker_get_state(nfc_comparator->reader.worker)) {
...
void nfc_comparator_physical_compare_scan_scene_on_exit(void* context) {
   ...
   nfc_comparator_reader_worker_free(nfc_comparator->reader.worker);
}
