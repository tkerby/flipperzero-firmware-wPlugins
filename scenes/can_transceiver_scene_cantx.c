...
void CANTransceiver_SceneCANTx_ViewEventCallback(CANTransceiver_ViewCANTx_Event event, void* pContext)
{
...
void CANTransceiver_ViewCANTx_ByteInputCallback_FrameData(void* pContext)
{
	...
	FURI_LOG_I("CAN-Transceiver", "Frame data updated to %hhx%hhx%hhx%hhx%hhx%hhx%hhx%hhx (length = %hhd)",
	...
	CANTransceiver_ViewCANTx_SetFrameData(pInstance->pViewCANTx, pInstance->u8pFrameData, pInstance->pGlobalConfig->u8NumDataBytes);
	...
	if (event.type == SceneManagerEventTypeCustom)
	{
		switch (event.event)
		{
			case CANTransceiver_ViewCANTx_EventEditFrameID:
				...
				byte_input_set_result_callback(pInstance->pByteInput,
				...
				view_dispatcher_switch_to_view(pInstance->pViewDispatcher, CANTransceiverViewByteInput);
				bSuccess = true;
				break;
			...
			case CANTransceiver_ViewCANTx_EventSendMessage:
				...
				if (pInstance->pMCP2515Worker->u32CurTask != u32MCP2515_WORKER_TASK_NONE)
					break;
				...
				memcpy(pInstance->pMCP2515Worker->u8pFrameData, pInstance->u8pFrameData,
					pInstance->pGlobalConfig->u8NumDataBytes);
				// Now send the message!
				MCP2515Worker_GiveTask(pInstance->pMCP2515Worker, u32MCP2515_WORKER_TASK_TX,
							CANTransceiver_SceneCANTx_WorkerCallback, pInstance);
				bSuccess = true;
				break;
			default:
				break;
...
void CANTransceiver_SceneCANTx_ResetGlobalsAndView(CANTransceiverGlobals* pInstance)
{
	...
	CANTransceiver_ViewCANTx_SetFrameData(pInstance->pViewCANTx, u8pNullArray, pInstance->pGlobalConfig->u8NumDataBytes);
	CANTransceiver_ViewCANTx_SetTEC(pInstance->pViewCANTx, 0);
	CANTransceiver_ViewCANTx_SetBusOff(pInstance->pViewCANTx, 0);
	CANTransceiver_ViewCANTx_SetEventCallback(pInstance->pViewCANTx, CANTransceiver_SceneCANTx_ViewEventCallback, pInstance);
}
