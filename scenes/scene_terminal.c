...
void flipper_spi_terminal_scene_terminal_alloc(FlipperSPITerminalApp *app) {
  ...
  furi_string_reserve(app->terminal_screen.output_string_buffer,
                      SPI_TERM_OUTPUT_BUFFER_SIZE);
...
static void flipper_spi_terminal_scene_terminal_dma_rx_isr(void *context) {
  ...
  if (LL_DMA_IsActiveFlag_TC6(SPI_DMA)) { // Second half
    startOfData =
        app->terminal_screen.rx_dma_buffer + app->config.rx_dma_buffer_size;
  ...
  if (startOfData != NULL) {
    furi_stream_buffer_send(app->terminal_screen.rx_buffer_stream, startOfData,
                            app->config.rx_dma_buffer_size, 0);
  }
...
static void
flipper_spi_terminal_scene_terminal_init_spi_dma(FlipperSPITerminalApp *app) {
...
static void
flipper_spi_terminal_scene_terminal_deinit_spi_dma(FlipperSPITerminalApp *app) {
  furi_check(app);
...
void flipper_spi_terminal_scene_terminal_on_enter(void *context) {
  ...
  app->terminal_screen.rx_dma_buffer =
      malloc(app->config.rx_dma_buffer_size * 2);
...
static bool try_append_escape_char(FuriString *str, uint8_t check,
                                   char escape_char, uint8_t byte) {
  if (check != byte) {
    return false;
  }
...
static FuriString *flipper_spi_terminal_scene_terminal_spi_buffer_to_string(
    FlipperSPITerminalApp *app, uint8_t *buffer, size_t size) {
  furi_check(app);
...
bool flipper_spi_terminal_scene_terminal_on_event(void *context,
  ...
  if (event.type == SceneManagerEventTypeTick) {
    ...
    for (size_t i = 0; i < 4; i++) {
      ...
      FuriString *str =
          flipper_spi_terminal_scene_terminal_spi_buffer_to_string(app, buffer,
                                                                   count);
      ...
      if (newOutputBufferLength > SPI_TERM_OUTPUT_BUFFER_SIZE) {
        furi_string_right(app->terminal_screen.output_string_buffer,
