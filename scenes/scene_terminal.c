...
void flipper_spi_terminal_scene_terminal_alloc(FlipperSPITerminalApp* app) {
    ...
    app->terminal_screen.rx_buffer_stream = furi_stream_buffer_alloc(128, 120);
    ...
    if(event.type == SceneManagerEventTypeTick) {
        ...
        for(size_t i = 0; i < 3; i++) {
            ...
            size_t count = furi_stream_buffer_receive(
                app->terminal_screen.rx_buffer_stream, buffer, sizeof(buffer) - 1, 0);
            if(count == 0)
                break;
            else
                addedAny = true;
            ...
            furi_string_cat_printf(app->terminal_screen.output_string_buffer, "%s", buffer);
        }
