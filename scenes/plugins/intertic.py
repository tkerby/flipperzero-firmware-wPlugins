...
def Describe_Usage_1(Usage, ContractMediumEndDate, Certificate):
    ...
    print(
        "  EventDateStamp             : {} ({})".format(
            EventDateStamp,
            (
                datetime(1997, 1, 1)
                + timedelta(days=ContractMediumEndDate - EventDateStamp)
            ).strftime("%Y-%m-%d"),
        )
    )
    print(
        "  EventTimeStamp             : {} ({:02d}:{:02d})".format(
            EventTimeStamp, EventTimeStamp // 60, EventTimeStamp % 60
        )
    )
    print("  unk1...                    :", unk)
    print(
        "  EventValidityTimeFirstStamp: {} ({:02d}:{:02d})".format(
...
def Describe_Usage_1_1(Usage, ContractMediumEndDate, Certificate):
    ...
    print(
        "  TimeStamp             : {} ({:02d}:{:02d})".format(
            EventTimeStamp, EventTimeStamp // 60, EventTimeStamp % 60
        )
    )
    print("  unk0...               :", unk0)
    print(
        "  Code/Nature           : 0x{:x} ({})".format(
            EventCode_Nature, TYPE_EventCode_Nature.get(EventCode_Nature, "?")
        )
    )
    print(
        "  Code/Type             : 0x{:x} ({})".format(
            EventCode_Type, TYPE_EventCode_Type.get(EventCode_Type, "?")
        )
    )
    print("  unk1...               :", unk1)
    ...
    print(
        "  Direction             : {} ({})".format(
            EventGeoRoute_Direction,
            TYPE_EventGeoRoute_Direction.get(EventGeoRoute_Direction, "?"),
        )
    )
    print("  Passengers(?)         : {}".format(EventCountPassengers_mb))
    print(
        "  ValidityTimeFirstStamp: {} ({:02d}:{:02d})".format(
            EventValidityTimeFirstStamp,
            EventValidityTimeFirstStamp // 60,
            EventValidityTimeFirstStamp % 60,
        )
...
def Describe_Usage_1_2(Usage, ContractMediumEndDate, Certificate):
    ...
    print(
        "  Code/Nature(?)        : 0x{:x} ({})".format(
            EventCode_Nature_mb,
            TYPE_EventCode_Nature_Reims.get(EventCode_Nature_mb, "?"),
        )
    )
    print(
        "  Code/Type(?)          : 0x{:x} ({})".format(
            EventCode_Type_mb, TYPE_EventCode_Type.get(EventCode_Type_mb, "?")
        )
    )
    print("  unk1...               :", unk1)
    print("  GeoVehicleId          : {}".format(EventGeoVehicleId))
...
def Describe_Usage_Generic(Usage, ContractMediumEndDate, Certificate):
    print(
        "  !!! GENERIC DUMP - please provide full file dump to benjamin@gentilkiwi.com - especially if NOT empty !!!"
    )
    print("  left...                    :", Usage.nom_bits_left())
    print("  [CER] Usage                : {:04x}".format(Certificate.nom(16)))
...
FRA_OrganizationalAuthority_Contract_Provider = {
    ...
    0x008: {
        15: InterticHelper(
            "Angoulême", "STGA", Describe_Usage_1_1
    ...
    0x057: {
        1: InterticHelper(
            "Lyon", "TCL / Keolis", Describe_Usage_1
    ...
    0x091: {
        1: InterticHelper(
            "Strasbourg", "CTS", Describe_Usage_4
...
def main():
    ...
    """
    # 100100010100000000100000001100000010010100001001000010000010010000000100000011111010101011001101
    if not Distribution_Data.isEmpty():
        ...
        print("DISTRIBUTION")
        print(
            "  CountryCode                     : {:03x} - {}".format(
                CountryCode, ISO_Countries.get(CountryCode, "?")
            )
        )
        print(
            "  OrganizationalAuthority         : {:03x}".format(OrganizationalAuthority)
        )
        ...
        if CountryCode == 0x250:
            oa = FRA_OrganizationalAuthority_Contract_Provider.get(
            ...
            if oa is not None:
                ...
                if s is not None:
                    print(
                        "      ~ Authority & Provider ~    : {} ({})".format(
                            s.OrganizationalAuthority, s.ContractProvider
                        )
                    )
                    Describe_Usage = s.UsageDescribeFunction
        print("  ContractTariff                  :", ContractTariff)
        print(
            "  ContractMediumEndDate           : {} ({})".format(
                ContractMediumEndDate,
                (datetime(1997, 1, 1) + timedelta(days=ContractMediumEndDate)).strftime(
                    "%Y-%m-%d"
                ),
            )
        )
        ...
        print(
            "  [CER] Distribution              : {:08x}".format(
                Distribution_Cer.nom(32)
            )
        )
        print()
        ...
        if COUNTER1 is not None:
            print(
                "[1] Counter: 0x{:06x}   - Reloading available: 0x{:02x}".format(
                    COUNTER1, RELOADING1
                )
            )
        ...
        if SWAP is not None:
            print(
                "[S] SWAP   : 0x{:08x} - last usage on USAGE_{}".format(
                    SWAP, "B" if SWAP & 0b1 else "A"
                )
            )
