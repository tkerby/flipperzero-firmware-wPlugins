...
static void CANTransceiver_ViewCANTx_EnterCallback(void* pContext)
{
	...
	with_view_model(pViewCANTx->pView, CANTransceiver_ViewCANTx_Model* pModel,
	{
...
CANTransceiver_ViewCANTx* CANTransceiver_ViewCANTx_Alloc()
{
	...
	view_allocate_model(pViewCANTx->pView, ViewModelTypeLocking, sizeof(CANTransceiver_ViewCANTx_Model));
	with_view_model(pViewCANTx->pView, CANTransceiver_ViewCANTx_Model* pModel,
	{
		(void)pModel;	// Nothing to do here.
	}, false);
...
void CANTransceiver_ViewCANTx_SetFrameID(CANTransceiver_ViewCANTx* pViewCANTx, uint32_t u32FrameID, uint8_t u8FrameIDLen)
{
	ASSERT(pViewCANTx != NULL);
...
void CANTransceiver_ViewCANTx_SetFrameData(CANTransceiver_ViewCANTx* pViewCANTx, uint8_t* u8pFrameData, uint8_t u8FrameDataLen)
{
	uint8_t i;
...
void CANTransceiver_ViewCANTx_SetEventCallback(CANTransceiver_ViewCANTx* pViewCANTx, CANTransceiver_ViewCANTx_Callback pCallback,
