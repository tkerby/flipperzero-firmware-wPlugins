...
static NfcCommand
    ...
    do {
        ...
        uint8_t config2[] = {0x03, 0xf0, 0xdb, 0x00, 0x00, 0x67, 0xa0, 0x06, 0x00, 0x20, 0x00,
                             128,  0x01, 0x28, 0xa4, 0x01, 0x0c, 0xa5, 0x02, 0x00, 0x0a, 0xa4,
                             0x01, 0x08, 0xa5, 0x02, 0x00, 0x0a, 0xa4, 0x01, 0x0c, 0xa5, 0x02,
                             0x00, 0x0a, 0xa4, 0x01, 0x02, 0xa1, 0x01, 0x12, 0xa4, 0x01, 0x02,
                             0xa1, 0x04, 0x01, 0x27, 0x01, 0x01, 0xa1, 0x02, 0x11, 0x01, 0xa1,
                             0x03, 0x44, 0x00, 0x0f, 0xa1, 0x05, 0x45, 0x27, 0x01, 0x00, 0x00,
                             0xa1, 0x02, 0x3c, 0x05, 0xa1, 0x03, 0x21, 0x00, 0x80, 0xa1, 0x02,
                             0x18, 0x80, 0xa1, 0x02, 0x4e, 0x00, 0xa1, 0x03, 0x4f, 0x27, 0x01,
                             0xa3, 0x01, 0x24, 0xa2, 0x02, 0x22, 0xf7, 0xa2, 0x01, 0x20, 0xa4,
                             0x01, 0x02, 0xa2, 0x02, 0x10, 0x01, 0xa5, 0x02, 0x00, 0x0a};

        const NfcEinkScreenInfo* const info = &screen->data->base;
        uint16_t* width = (uint16_t*)&config2[12];
        uint16_t* height = (uint16_t*)&config2[10];
        *width = __builtin_bswap16(info->width);
        *height = __builtin_bswap16(info->height);

        result = eink_goodisplay_send_data(
            poller, screen, config2, sizeof(config2), eink_goodisplay_validate_0x03_cmd_response);
        if(!result) break;
