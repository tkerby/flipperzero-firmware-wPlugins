...
void kia_protocol_decoder_v6_feed(void* context, bool level, uint32_t duration)
{
    ...
    case KiaV6DecoderStepReset: // case 0
        ...
        if(DURATION_DIFF(duration, kia_protocol_v6_const.te_short) < kia_protocol_v6_const.te_delta) {
    ...
    case KiaV6DecoderStepWaitFirstHigh: // case 1
        ...
        if(DURATION_DIFF(instance->decoder.te_last, kia_protocol_v6_const.te_short) < kia_protocol_v6_const.te_delta) {
            instance->decoder.te_last = duration;
    ...
    case KiaV6DecoderStepWaitLongHigh: // case 2
        ...
        if(DURATION_DIFF(instance->decoder.te_last, kia_protocol_v6_const.te_long) >= kia_protocol_v6_const.te_delta) {
            step_value = KiaV6DecoderStepReset;
            goto LAB_reset;
        }
    ...
    case KiaV6DecoderStepData: // case 3
        ...
        } else if(DURATION_DIFF(duration, kia_protocol_v6_const.te_long) < kia_protocol_v6_const.te_delta) {
            event = level ? 6 : 4;
            goto manchester_process;
        }
        step_value = KiaV6DecoderStepReset;
    ...
    SubGhzRadioPreset* preset)
{
    ...
    do {
        ...
        if(!flipper_format_write_string_cstr(flipper_format, "Preset", furi_string_get_cstr(preset->name))) break;
        ...
        uint64_t key_data = ((uint64_t)instance->stored_part1_high << 32) | instance->stored_part1_low;
        char key_str[20];
...
SubGhzProtocolStatus kia_protocol_decoder_v6_deserialize(void* context, FlipperFormat* flipper_format)
{
    furi_assert(context);
    SubGhzProtocolDecoderKiaV6* instance = context;
    ...
    SubGhzProtocolStatus ret = SubGhzProtocolStatusError;
    do {
        if(!subghz_block_generic_deserialize_check_count_bit(&instance->generic, flipper_format, kia_protocol_v6_const.min_count_bit_for_found)) {
            break;
        }
