...
static bool kia_verify_crc(uint64_t data) {
    ...
    FURI_LOG_D(TAG, "CRC Check - Received: 0x%02X, Calculated: 0x%02X, Match: %s",
...
void *subghz_protocol_encoder_kia_alloc(SubGhzEnvironment *environment)
{
    ...
    instance->encoder.repeat = 10; // High repeat count for continuous transmission while button is held
...
static void subghz_protocol_encoder_kia_update_data(SubGhzProtocolEncoderKIA *instance)
{
    ...
    FURI_LOG_I(TAG, "Data updated - Serial: 0x%07lX, Btn: 0x%X, Cnt: 0x%04X, CRC: 0x%02X",
               instance->serial, instance->button, instance->counter, crc);
...
static void subghz_protocol_encoder_kia_get_upload(SubGhzProtocolEncoderKIA *instance)
{
    ...
    for(uint8_t burst = 0; burst < KIA_TOTAL_BURSTS; burst++) {
        if(burst > 0) {
            instance->encoder.upload[index++] = 
                level_duration_make(false, KIA_INTER_BURST_GAP_US);
        }
        ...
        for(uint8_t bit_num = 0; bit_num < 59; bit_num++) {
            ...
            instance->encoder.upload[index++] = 
                level_duration_make(true, duration);
            instance->encoder.upload[index++] = 
                level_duration_make(false, duration);
        }
    ...
    FURI_LOG_I(TAG, "Upload built: %d bursts, size_upload=%zu, data_count_bit=%u, data=0x%016llX", 
               KIA_TOTAL_BURSTS, instance->encoder.size_upload, instance->generic.data_count_bit, instance->generic.data);
}
...
SubGhzProtocolStatus subghz_protocol_encoder_kia_deserialize(void *context, FlipperFormat *flipper_format)
{
    ...
    do
    {
        ...
        if(!flipper_format_read_uint32(
            ...
            instance->encoder.repeat = 10;
            FURI_LOG_D(TAG, "Repeat not found in file, using default 10 for continuous transmission");
        }
        ...
        FURI_LOG_I(TAG, "Encoder initialized - will send %d bursts, repeat=%u, front=%zu", 
                   KIA_TOTAL_BURSTS, instance->encoder.repeat, instance->encoder.front);
...
LevelDuration subghz_protocol_encoder_kia_yield(void *context)
{
    ...
    if(!instance || !instance->encoder.upload || 
       instance->encoder.repeat == 0 || !instance->encoder.is_running) {
        if(instance) {
            FURI_LOG_D(TAG, "Encoder yield stopped: repeat=%u, is_running=%d, upload=%p", 
                       instance->encoder.repeat, instance->encoder.is_running, instance->encoder.upload);
            instance->encoder.is_running = false;
        }
    ...
    if(instance->encoder.front >= instance->encoder.size_upload) {
        FURI_LOG_E(TAG, "Encoder front out of bounds: %zu >= %zu", 
                   instance->encoder.front, instance->encoder.size_upload);
        instance->encoder.is_running = false;
        instance->encoder.front = 0;
        return level_duration_reset();
    }
    ...
    if(instance->encoder.front < 5 || instance->encoder.front == 0) {
        FURI_LOG_D(TAG, "Encoder yield[%zu]: repeat=%u, size=%zu, level=%d, duration=%lu",
                   instance->encoder.front, instance->encoder.repeat, instance->encoder.size_upload,
                   level_duration_get_level(ret), level_duration_get_duration(ret));
    }

    if(++instance->encoder.front == instance->encoder.size_upload) {
        instance->encoder.repeat--;
        instance->encoder.front = 0;
        FURI_LOG_I(TAG, "Encoder completed one cycle, remaining repeat=%u", instance->encoder.repeat);
    }
...
void subghz_protocol_encoder_kia_increment_counter(void *context)
{
    ...
    subghz_protocol_encoder_kia_update_data(instance);
    subghz_protocol_encoder_kia_get_upload(instance);
    FURI_LOG_I(TAG, "Counter incremented to 0x%04X, upload rebuilt with new CRC", instance->counter);
}
