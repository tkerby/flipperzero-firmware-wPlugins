...
static bool kia_v1_manchester_decode(SubGhzProtocolDecoderKiaV1* instance) {
    ...
    FURI_LOG_D(TAG, "Raw: %02X %02X %02X %02X %02X %02X", 
        instance->raw_bits[0], instance->raw_bits[1], 
        instance->raw_bits[2], instance->raw_bits[3],
        instance->raw_bits[4], instance->raw_bits[5]);
...
void kia_protocol_decoder_v1_feed(void* context, bool level, uint32_t duration) {
    ...
    case KiaV1DecoderStepReset:
        ...
        if((level) && (DURATION_DIFF(duration, kia_protocol_v1_const.te_long) < kia_protocol_v1_const.te_delta)) {
    ...
    case KiaV1DecoderStepCheckPreamble:
        if(level) {
            if(DURATION_DIFF(duration, kia_protocol_v1_const.te_long) < kia_protocol_v1_const.te_delta) {
                instance->decoder.te_last = duration;
    ...
    case KiaV1DecoderStepCollectRawBits:
        if(duration > 2400) {
            ...
            if(kia_v1_manchester_decode(instance)) {
                ...
                FURI_LOG_I(TAG, "DECODE! Key=%014llX Sn=%08lX Btn=%02X Cnt=%02X",
        ...
        if(DURATION_DIFF(duration, kia_protocol_v1_const.te_short) < kia_protocol_v1_const.te_delta) {
            num_bits = 1;
        } else if(DURATION_DIFF(duration, kia_protocol_v1_const.te_long) < kia_protocol_v1_const.te_delta) {
            num_bits = 2;
        } else {
            FURI_LOG_D(TAG, "Invalid pulse: %s %lu, raw_bits=%u", level ? "H" : "L", duration, instance->raw_bit_count);
            instance->decoder.parser_step = KiaV1DecoderStepReset;
            break;
        }
