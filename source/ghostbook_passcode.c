...
static bool passcode_input_cb(InputEvent* event, void* ctx) {
    ...
    PasscodeButton btn;
    switch(event->key) {
        case InputKeyUp:    btn = PasscodeUp; break;
        case InputKeyDown:  btn = PasscodeDown; break;
        case InputKeyLeft:  btn = PasscodeLeft; break;
        case InputKeyRight: btn = PasscodeRight; break;
        case InputKeyOk:    btn = PasscodeOk; break;
        case InputKeyBack:  btn = PasscodeBack; break;
    ...
    if(app->passcode_pos < len) {
        ...
        with_view_model(app->passcode_view, PasscodeViewModel* m, {
            UNUSED(m);
        }, true);
...
View* ghost_passcode_view_alloc(GhostApp* app) {
    ...
    with_view_model(view, PasscodeViewModel* m, {
        m->app = app;
        m->header = "UNLOCK";
