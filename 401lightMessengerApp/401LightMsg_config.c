...
const LightMsg_Orientation lightmsg_orientation_value[] = {LightMsg_OrientationWheelUp, LightMsg_OrientationWheelDown};
...
uint32_t LightMsg_color_pal_nyancat[] = {
    lightmsg_colors_flat[COLOR_RED],    lightmsg_colors_flat[COLOR_RED],    lightmsg_colors_flat[COLOR_RED],
    lightmsg_colors_flat[COLOR_ORANGE], lightmsg_colors_flat[COLOR_ORANGE], lightmsg_colors_flat[COLOR_ORANGE],
    lightmsg_colors_flat[COLOR_YELLOW], lightmsg_colors_flat[COLOR_YELLOW], lightmsg_colors_flat[COLOR_GREEN],
    lightmsg_colors_flat[COLOR_GREEN],  lightmsg_colors_flat[COLOR_CYAN],   lightmsg_colors_flat[COLOR_CYAN],
    lightmsg_colors_flat[COLOR_BLUE],   lightmsg_colors_flat[COLOR_BLUE],   lightmsg_colors_flat[COLOR_PURPLE],
    lightmsg_colors_flat[COLOR_PURPLE],
};

uint32_t LightMsg_color_pal_vaporwave[] = {
    0xf7b900, 0xf88900, 0xf95c00, 0xfa2616, 0xfb0f33, 0xfc0080, 0xfd00a9, 0xfe00d3,
    0xff00f0, 0xd200f2, 0xae00f4, 0x7700f6, 0x4720f7, 0x213ff9, 0x0074fb, 0x00d3ff,
};
...
void update_led(void *ctx)
{
    ...
    for (size_t row = 0; row < LIGHTMSG_LED_ROWS; ++row)
    {
        ...
        SK6805_set_led_color(row, (uint8_t)(((color >> 16) & 0xFF) * brightness),
                             (uint8_t)(((color >> 8) & 0xFF) * brightness), (uint8_t)(((color)&0xFF) * brightness));
    }
    SK6805_update();
}
...
AppConfig *app_config_alloc(void *ctx)
{
    ...
    VariableItem *item = variable_item_list_add(appConfig->list, "Orientation", COUNT_OF(lightmsg_orientation_text),
                                                on_change_orientation, app);
    variable_item_set_current_value_index(item, light_msg_data->orientation); // 0,10,20,30,...
    variable_item_set_current_value_text(item, lightmsg_orientation_text[light_msg_data->orientation]);
// Activate the brightness menu or not.
#if PARAM_BRIGHTNESS_CONFIG_EN == 1
    item = variable_item_list_add(appConfig->list, "Brightness", COUNT_OF(lightmsg_brightness_text),
                                  on_change_brightness, app);
    variable_item_set_current_value_index(item, light_msg_data->brightness); // 0,10,20,30,...
    variable_item_set_current_value_text(item, lightmsg_brightness_text[light_msg_data->brightness]);
#endif
    item = variable_item_list_add(appConfig->list, "Color", COUNT_OF(lightmsg_color_text), on_change_color, app);
...
void app_scene_config_on_enter(void *ctx)
{
    ...
    furi_check(furi_timer_start(appConfig->timer, (furi_kernel_get_tick_frequency() / 100)) == FuriStatusOk);
