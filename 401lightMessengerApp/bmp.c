...
int bmp_read_file_header(File* bitmapFile, BMPFileHeader* file_header) {
  size_t bytes =
      storage_file_read(bitmapFile, file_header, sizeof(BMPFileHeader));
...
int bmp_read_info_header(File* bitmapFile, BMPInfoHeader* info_header) {
  size_t bytes =
      storage_file_read(bitmapFile, info_header, sizeof(BMPInfoHeader));
...
int bmp_load(const char* path, BMPImage* bitmap) {
  ...
  int rowSize =
      (bitmap->info_header.biWidth + 7) / 8;    // Number of bytes per row
...
int bmp_header_check_1bpp(const char* path) {
  ...
  if ((image.info_header.biHeight != 16) || (image.info_header.biWidth > 300)) {
    FURI_LOG_E(TAG, "%ldx%ld height must be 16 and width <= 32",
               image.info_header.biWidth, image.info_header.biHeight);
    return BMP_ERR_BAD_FORMAT;
  }
  if (image.info_header.biBitCount != 1) {
    FURI_LOG_E(TAG, "bits_per_pixel must be 1 %d given",
               image.info_header.biBitCount);
    return BMP_ERR_BAD_FORMAT;
  }
  if (image.info_header.biCompression != 0) {
    FURI_LOG_E(TAG, "compression must be 0 ,%ld given",
               image.info_header.biCompression);
    return BMP_ERR_BAD_FORMAT;
  }
  return BMP_OK;
}
...
int bmp_write(const char* path, bitmapMatrix* bitmap) {
  ...
  fileHeader.bfSize =
      54 + paletteSize + pixelArraySize;  // Headers + palette + pixel data
  fileHeader.bfReserved1 = 0;
  fileHeader.bfReserved2 = 0;
  fileHeader.bfOffBits =
      54 + paletteSize;  // Pixel data offset, accounting for palette
  ...
  uint32_t palette[2] = {0x00000000,
                         0x00FFFFFF};  // Black (0x00RRGGBB), White (0x00RRGGBB)
  if (storage_file_write(bitmapFile, palette, sizeof(palette)) !=
      sizeof(palette)) {
    err = BMP_ERR_WRITE;
    goto fail;
  }
  ...
  for (int h = 0; h < height; h++) {
    ...
    for (int w = 0; w < width; w++) {
      int byteIndex = w / 8;
      int bitIndex = 7 - (w % 8);
      if (bitmap->array[height - h - 1][w] ==
          0x00) {  // Note the inversion of height
