...
#define VL6180X_DEFAULT_SETTINGS_COUNT (sizeof(DEFAULT_SETTINGS) / sizeof(VL6180X_ConfigurationSetting))
...
static void write_register(uint8_t i2c_address, VL6180X_Register vl6180x_register, uint8_t data) {
    ...
    uint8_t command[] = {
        vl6180x_register >> 8,
        vl6180x_register & 0xFF,
        data
    };
    
    if(!furi_hal_i2c_tx(&furi_hal_i2c_handle_external, i2c_address, command, sizeof(command), 100)) {
        return;
    }
}
...
static uint8_t read_register(uint8_t i2c_address, VL6180X_Register vl6180x_register) {
    ...
    if(!furi_hal_i2c_trx(&furi_hal_i2c_handle_external, i2c_address, command, sizeof(command), &response, 1, 100)){
...
uint8_t find_vl6180x_address(){
    ...
    for(address = 0; address != VL6180X_NO_DEVICE_FOUND_ADDRESS; address++) {
        if(furi_hal_i2c_is_device_ready(&furi_hal_i2c_handle_external, address, 5)) {
            ...
            FURI_LOG_D(LOG_TAG, "Found I2C Device at address %02X with Model ID %02X", address, model_id);
...
void configure_vl6180x(uint8_t vl6180x_address) {
    ...
    for(uint8_t i = 0; i < VL6180X_DEFAULT_SETTINGS_COUNT; i++){
        write_register(vl6180x_address, DEFAULT_SETTINGS[i].vl6180x_register, DEFAULT_SETTINGS[i].value);
    }
