...
- `I[n]`: 第n个输入命令
- `O[n]`: 第n个输出响应
- `O[n][start:end]`: 第n个输出响应从start到end位置的切片
- `hex(data)`: 将十六进制字符串转换为可读文本
- `hex(data, "utf-8")`: 将十六进制字符串按UTF-8编码转换为文本
- `h2d(data)`: 将十六进制字符串转换为十进制数字
- `O[n]TAG(tag)`: 从第n个输出响应中提取指定TLV标签的值
- `O[n]TAG(tag, "encoding")`: 从第n个输出响应中提取指定TLV标签的值，并按指定编码转换为文本
- `O[n]TAG(tag)[start:end]`: 提取指定TLV标签的值，然后对结果进行切片
- 数学运算符: `+`, `-`, `*`, `/` (例如: `{h2d(O[1][4:8]) + h2d(O[2][4:8])}`)

### 高级功能

#### 十六进制转十进制转换

`h2d()` 函数将十六进制值转换为十进制数字：

```
余额: {h2d(O[1][4:12])} (单位：分)
余额: {h2d(O[1][4:12])/100.00} 元 (格式化)
```
...
```
总计: {h2d(O[1][4:8]) + h2d(O[2][4:8])}
平均值: {(h2d(O[1][4:8]) + h2d(O[2][4:8]) + h2d(O[3][4:8])) / 3}
```
...
```
卡片分析
余额(元): {h2d(O[1]TAG(9F79))/100.00}
最后交易: {h2d(O[2]TAG(9F02))/100.00}
剩余余额: {h2d(O[1]TAG(9F79))/100.00 - h2d(O[2]TAG(9F02))/100.00}
```
