        ...
        if(context->tutorial_page < 3) {
            for(int i=0; i<3; i++) {
                if(TUTORIAL_TEXT[context->tutorial_page][i]) {
                    canvas_draw_str_aligned(canvas, 64, 12 + (i*10), AlignCenter, AlignTop, TUTORIAL_TEXT[context->tutorial_page][i]);
                }
            }
        }
        ...
        if(furi_message_queue_get(ctx->event_queue, &event, FuriWaitForever) == FuriStatusOk) {
            ...
            if(event.type == EventTypeTick) {
                if(ctx->state == GameStateWaitingForInput) {
                    ...
                    if(!ctx->timeout_replay_done && (now - ctx->last_interaction_time > TIMEOUT_MS)) {
                        ctx->current_input_index = 0;
            ...
            if(event.type == EventTypeKey) {
                ...
                if(input->key == InputKeyBack && input->type == InputTypeShort) {
                    if(ctx->state == GameStateTutorial) ctx->state = GameStateGameOver; 
                    else {
                    ...
                    if(ctx->state == GameStateTutorial) {
                        if(ctx->tutorial_page < 0) {
                            ...
                            if(input->key == InputKeyRight && ctx->tutorial_page < 2) ctx->tutorial_page++;
                            else if(input->key == InputKeyLeft && ctx->tutorial_page > 0) ctx->tutorial_page--;
                            else if(input->key == InputKeyLeft && ctx->tutorial_page == 0) ctx->tutorial_page = -1;
                    ...
                    // 2. GAMEPLAY
                        ...
                        if(ctx->current_input_index >= PATTERN_LENGTH) {
                            ...
                            if(ctx->last_accuracy >= 50.0f) {
                                ...
                                if(ctx->correct_in_a_row > ctx->hi_score) ctx->hi_score = ctx->correct_in_a_row;
                                generate_new_pattern(ctx);
                                view_port_update(ctx->view_port);
                                if(responsive_delay(ctx, 500)) {
                                    play_pattern(ctx);
