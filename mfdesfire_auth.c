    ...
    do {
        ...
        if(flipper_format_read_string(file, furi_string_get_cstr(key_name), temp_str)) {
            if(furi_string_equal(temp_str, instance->selected_card_path)) {
                ...
                if(!flipper_format_read_hex(file, furi_string_get_cstr(key_name), instance->initial_vector, INITIAL_VECTOR_SIZE)) break;

                furi_string_printf(key_name, "Key_%s", filename);
                if(!flipper_format_read_hex(file, furi_string_get_cstr(key_name), instance->key, KEY_SIZE)) break;
...
MfDesApp* mfdesfire_auth_app_alloc() {
    ...
    view_dispatcher_set_custom_event_callback(instance->view_dispatcher, mfdes_app_custom_event_callback);
    view_dispatcher_set_navigation_event_callback(instance->view_dispatcher, mfdes_back_event_callback); // Tohle se zavola kdyz kratce kliknu tlacitko zpet
    ...
    // Přidání views
    instance->file_browser = file_browser_alloc(instance->selected_card_path); // Ulozi vyslednou cestu do selected card path - je to v file_browser.c inner se o to stara
    view_dispatcher_add_view(instance->view_dispatcher, MfDesAppViewBrowser, file_browser_get_view(instance->file_browser));
    
    instance->submenu = submenu_alloc();
    view_dispatcher_add_view(instance->view_dispatcher, MfDesAppViewSubmenu, submenu_get_view(instance->submenu));
