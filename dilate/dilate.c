...
static void format_time_difference(double diff_seconds, char* buffer, size_t buffer_size) {
    if (diff_seconds < SIXTY) {
        ...
        if (years >= HUNDRED) {
            snprintf(buffer, buffer_size, "+%.1gy", years);  // Use %g for automatic scientific notation
        } else {
...
static void dilate_draw_callback(Canvas* canvas, void* ctx) {
    ...
    if (earth_display_time >= HUNDRED) {
        snprintf(earth_time_str, sizeof(earth_time_str), "%.1g%s", earth_display_time, unit_symbols[state->time_unit]);
    } else {
        snprintf(earth_time_str, sizeof(earth_time_str), "%.1f%s", earth_display_time, unit_symbols[state->time_unit]);
    }
