...
#if PICO_GAME_ENGINE_BOARD_TYPE == PICO_GAME_ENGINE_BOARD_TYPE_FLIPPER_VGM                                  // Sets the font for text rendering.
        void setFont(const GFXfont *font = NULL);
#else
        void setFont(int font = 2);
#endif
...
//
...
#else
        TFT_eSPI *display; // Invoke custom library
#endif
    };
    ...
    public:
        ...
        void text(Vector position, const char *text, uint16_t color);    // Draws text on the display at the specified position with the specified color.
#if PICO_GAME_ENGINE_BOARD_TYPE == PICO_GAME_ENGINE_BOARD_TYPE_FLIPPER_VGM
