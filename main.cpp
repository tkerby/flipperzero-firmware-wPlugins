...
static void game_setup() {
    ...
    arduboy.setFrameRate(TARGET_FRAMERATE);   // как в оригинальном .ino
    loadSetEEPROM();
    arduboy.pollButtons();
    arduboy.clear();
    mainGameLoop[gameState]();   // рисует в screen_buffer через Platform
    arduboy.display();    
}

// Один “тик” игры = бывший loop() из .ino
