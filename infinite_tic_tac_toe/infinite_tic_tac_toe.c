...
static void draw_callback(Canvas* const canvas, void* ctx) {
    ...
    if(game_state->inprogress) {
        snprintf(sidebar_buffer, sizeof(sidebar_buffer), "%c\'s Turn", game_state->turn & PLAYER_O_MASK ? 'O' : 'X');
    }else{
        snprintf(sidebar_buffer, sizeof(sidebar_buffer), "%c Wins", game_state->turn & PLAYER_O_MASK ? 'O' : 'X');
    }
        ...
        if(event_status == FuriStatusOk && event.input.type == InputTypePress) {
            ...
            case InputKeyOk:
                if(game_state->inprogress) {
                    ...
                    game_state->board[game_state->pos_r][game_state->pos_c].lifetime = SQUARE_INITIAL_LIFETIME;
                    game_state->board[game_state->pos_r][game_state->pos_c].player = game_state->turn;
                    ...
                    for(int i = 0; i < BOARD_DIM_SIZE; i++){
                        game_state->inprogress &= !(
                            ...
                            game_state->board[i][0].lifetime != 0 && game_state->board[i][1].lifetime != 0 && game_state->board[i][2].lifetime != 0 &&
                            game_state->board[i][0].player == game_state->board[i][1].player && game_state->board[i][1].player == game_state->board[i][2].player
                        );
                            
                        game_state->inprogress &= !(
                            // Check all squares in the column have the same piece and the lifetime is not 0
                            game_state->board[0][i].lifetime != 0 && game_state->board[1][i].lifetime != 0 && game_state->board[2][i].lifetime != 0 &&
                            game_state->board[0][i].player == game_state->board[1][i].player && game_state->board[1][i].player == game_state->board[2][i].player
                        );
                    }
                    
                    game_state->inprogress &= !(
                        // Check all the squares on the major diagonal have the same piece and the lifetime is not 0
                        game_state->board[0][0].lifetime != 0 && game_state->board[1][1].lifetime != 0 && game_state->board[2][2].lifetime != 0 &&
                        game_state->board[0][0].player == game_state->board[1][1].player && game_state->board[1][1].player == game_state->board[2][2].player
                    );
                        
                    game_state->inprogress &= !(
                        // Check all the squares on the minor diagonal have the same piece and the lifetime is not 0
                        game_state->board[0][2].lifetime != 0 && game_state->board[1][1].lifetime != 0 && game_state->board[2][0].lifetime != 0 &&
                        game_state->board[0][2].player == game_state->board[1][1].player && game_state->board[1][1].player == game_state->board[2][0].player
                    );
