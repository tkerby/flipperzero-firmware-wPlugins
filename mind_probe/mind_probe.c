...
#define DB_PATH "/ext/apps_data/mind_probe/database.csv"
#define INDEX_PATH "/ext/apps_data/mind_probe/database.idx"
...
typedef enum { SelectionYes, SelectionNo } Selection;
typedef enum { GameStateStandaloneIntro, GameStatePlaying, GameStateEnd, GameStateError } GameState;
typedef enum { StateSelectMode, StateStandalone, StateConnectedPlaceholder } AppState;
typedef enum { ModeSelectionStandalone, ModeSelectionConnected } ModeSelection;
...
static GameNode* load_node_from_sd(int target_id) {
    ...
    enum { S_ID, S_TYPE, S_TEXT, S_YES, S_NO, S_DONE } state = S_ID;
    for(uint16_t i = 0; i < strlen(line_buf); ++i) {
        char c = line_buf[i];
        switch(state) {
        case S_ID:
            if(c >= '0' && c <= '9') temp_buf[temp_idx++] = c;
            else if(c == ',') { id = atoi(temp_buf); temp_idx = 0; memset(temp_buf, 0, sizeof(temp_buf)); state = S_TYPE; }
            break;
        case S_TYPE:
            if(c != ',') type = c;
            else state = S_TEXT;
            break;
        case S_TEXT:
            if(c == '"' && !in_quotes) in_quotes = true;
            else if(c == '"' && in_quotes) in_quotes = false;
            else if(c == ',' && !in_quotes) { text_buf[temp_idx] = '\0'; temp_idx = 0; state = S_YES; }
            else if(in_quotes) text_buf[temp_idx++] = c;
            break;
        case S_YES:
            if(c >= '0' && c <= '9') temp_buf[temp_idx++] = c;
            else if(c == ',') { yes_id = atoi(temp_buf); temp_idx = 0; memset(temp_buf, 0, sizeof(temp_buf)); state = S_NO; }
            break;
        case S_NO:
            if(c >= '0' && c <= '9') temp_buf[temp_idx++] = c;
            break;
    ...
    if(state == S_NO) {
        ...
        FURI_LOG_D(TAG, "Successfully parsed node: id=%d, yes=%d, no=%d, text='%s'", id, yes_id, no_id, text_buf);
    } else {
...
static void handle_input(GameAppState* state, InputEvent* event, bool* running) {
    ...
    case GameStatePlaying:
        if(event->key == InputKeyLeft || event->key == InputKeyRight) {
            ...
            int next_id = (state->user_selection == SelectionYes) ?
                                state->current_node->yes_id :
