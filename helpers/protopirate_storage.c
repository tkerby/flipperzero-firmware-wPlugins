...
static void protopirate_storage_build_file_list(void)
{
    ...
    if(storage_dir_open(dir, PROTOPIRATE_APP_FOLDER))
    {
        ...
        while(storage_dir_read(dir, &file_info, name, sizeof(name)))
        {
            if(!file_info_is_dir(&file_info) && strstr(name, PROTOPIRATE_APP_EXTENSION))
            {
                ...
                strncpy(g_file_entries[write_index].name, name, sizeof(g_file_entries[write_index].name) - 1);
                g_file_entries[write_index].name[sizeof(g_file_entries[write_index].name) - 1] = '\0';
    ...
    g_file_list_valid = true;
    FURI_LOG_I(TAG, "Built file list with %lu entries (total on disk: %lu)", 
               (unsigned long)g_file_count, (unsigned long)total_found);
}
